...
var webphone_api = (function () {
  ...
  function hangup(isinternal) {
    ...
    else webphone_api.plhandler.Hangup(null, isinternal);
  }
  ...
  function mute(state, direction, isinternal) {
    ...
    else return webphone_api.plhandler.MuteEx(state, direction, isinternal);
  }
  ...
  function hold(state, isinternal) {
    ...
    else return webphone_api.plhandler.Hold(state, null, isinternal);
  }
...
*/
  ...
  function nextcallid(callid) {
    ...
    webphone_api.Log(
      "ERROR, webphone_api: nextcallid webphone_api.plhandler is not defined",
    );
    return "";
  }
  ...
  function Log(msg) {
    if (
      ...
      (webphone_api.parameters["logtoconsole"] == "false" ||
        webphone_api.parameters["logtoconsole"] == false ||
        webphone_api.parameters["loglevel"] == "0" ||
        webphone_api.parameters["loglevel"] == 0)
    ) {
      return;
    }
  ...
  function Log2(msg) {
    if (
      ...
      (webphone_api.parameters["logtoconsole"] == "false" ||
        ...
        webphone_api.parameters["loglevel"] == "0" ||
        webphone_api.parameters["loglevel"] == 0 ||
        webphone_api.parameters["loglevel"] == "1" ||
        webphone_api.parameters["loglevel"] == 1)
    ) {
      return;
    }
      ...
      if (script.readyState) {
        ...
        script.onload = function () {
          ...
          if (url.indexOf("webphone_config.js") >= 0) {
            try {
              if (
                typeof webphone_api.parameters === "undefined" ||
                webphone_api.parameters == null
              ) {
                ...
                if (!found) {
                  webphone_api.parameters["loglevel"] = 5;
                  Log(
                    "WARNING, webphone_config.js parameters are empty or invalid",
                  );
                }
              }
            } catch (err) {
              webphone_api.parameters["loglevel"] = 5;
              LogEx("ERROR, failed to parse webphone_config.js: ", err);
            }
          }
...
webphone_api.getbasedir2 = function (forceauto) {
  try {
        ...
        if (webphone_api.bd_logged !== true) {
          try {
            if (
              ...
              webphone_api.Log2(
                "EVENT, base directory - webphonebasedir(helper): " + wpbdir,
              );
            }
          } catch (err) {}
        }
          ...
          if (webphone_api.bd_logged !== true) {
            try {
              if (
                ...
                console.log(
                  "EVENT, base directory - webphonebasedir(guessed): " + wpbdir,
                );
              }
            } catch (err) {}
          }
    ...
    try {
      webphone_api.Log2("ERROR, webphone_api getbasedir2");
    } catch (err2) {}
  }
  return "";
};
