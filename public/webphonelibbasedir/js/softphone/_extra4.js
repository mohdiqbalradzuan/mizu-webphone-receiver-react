...
*/
  ...
  function onCreate(event) {
    try {
      ...
      webphone_api.$("#btn_extra4_menu").on("click", function () {
        CreateOptionsMenu("#extra4_menu_ul");
      });

      // set page title
      webphone_api.$("#extra4_title").html("Page Extra 4");
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_extra4: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra4: onStart", err);
    }
  }
  ...
  function MeasureExtra4() {
    ...
    var contentHeight =
      ...
      webphone_api.$("#extra4_header").height() -
      3;
    ...
    try {
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra4: CreateOptionsMenu menuid null",
        );
        return;
      }
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra4: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
 ...
 */
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra4: onStop", err);
    }
  }
