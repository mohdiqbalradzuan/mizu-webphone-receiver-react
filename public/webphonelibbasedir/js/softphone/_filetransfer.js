...
webphone_api._filetransfer = (function () {
  function onCreate(event) {
    ...
    try {
      ...
      {
        if (
          webphone_api.$.mobile.activePage.attr("id") === "page_filetransfer"
        ) {
          MeasureFiletransfer();
        }
      });
      ...
      webphone_api
        .$("#filetransfer_menu_ul")
        .on("click", "li", function (event) {
          MenuItemSelected(webphone_api.$(this).attr("id"));
        });
      webphone_api.$("#btn_filetransfer_menu").on("click", function () {
        CreateOptionsMenu("#filetransfer_menu_ul");
      });
      webphone_api
        .$("#btn_filetransfer_menu")
        .attr("title", webphone_api.stringres.get("hint_menu"));

      webphone_api.$("#btn_filetransfpick").on("click", function () {
  ...
  function onStart(event) {
    ...
    try {
      ...
      if (
        ...
        document.getElementById("filetransfer_title").innerHTML =
      ...
      if (
        !webphone_api.common.isNull(
          document.getElementById("filetransfer_btnback"),
        )
      ) {
        document.getElementById("filetransfer_btnback").innerHTML =
          "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("go_back_btn_txt");
      }

      lastoop = 2;

      var destination = webphone_api.common.GetIntentParam(
        webphone_api.global.intentfiletransfer,
        "destination",
      );
      if (webphone_api.common.isNull(destination)) {
        destination = "";
      }
      ...
      if (destination.length < 1)
        destination = webphone_api.common.GetParameter("lastchattarget");
      if (webphone_api.common.isNull(destination)) {
        destination = "";
      }
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#filetransfer_header"),
            -30,
          ),
        );

      webphone_api
        .$("#filetransfpick_input")
        .attr("placeholder", webphone_api.stringres.get("filetransfer_nr"));
      ...
      var useurlshortener = webphone_api.common.GetParameterInt(
        "useurlshortener",
        -1,
      );
      var urlshortener_orig =
        webphone_api.common.GetParameter("urlshortener_orig");
      var urlshortener_replace = webphone_api.common.GetParameter(
        "urlshortener_replace",
      );

      lastoop = 4;

      if (
        webphone_api.common.isNull(urlshortener_orig) ||
        urlshortener_orig.length < 1
      ) {
        urlshortener_orig = "https://www.mizu-voip.com/G";
      }
      if (
        webphone_api.common.isNull(urlshortener_replace) ||
        urlshortener_replace.length < 1
      ) {
        urlshortener_replace = "https://tinyurl.com/qm6oja3";
      }
      lastoop = 5;

      if (
        (useurlshortener === -1 || useurlshortener === 1) &&
        !webphone_api.common.isNull(urlshortener_orig) &&
        urlshortener_orig.length > 0 &&
        !webphone_api.common.isNull(urlshortener_replace) &&
        urlshortener_replace.length > 0 &&
        ...
        if (actionurl.indexOf("https://rtc.mizu-voip.com/mvweb") >= 0) {
          actionurl = actionurl.replace(
            "https://rtc.mizu-voip.com/mvweb",
            "https://tinyurl.com/yxpz9ce2",
          );
        }
      }
      ...
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, filetransfer actionurl: " + actionurl,
      );
      lastoop = 7;
      ...
      var html =
        '<body style="margin 0; padding 0; background: transparent; width: 100%; overflow:hidden; font-size: 1em; color: #cecece;">' +
        "<style>" +
        ...
        actionurl +
        '" method="post" enctype="multipart/form-data" id="frm_filetransf" name="frm_filetransf" onsubmit="return OnFormSubmit()">' +
        '<input type="hidden" id="filepath" name="filepath" value="">' +
        '<input name="filedata" type="file" id="fileinput" /><br />' +
        '<input type="submit" id="btn_filetransf" value="' +
        webphone_api.stringres.get("btn_send") +
        '" title="' +
        webphone_api.stringres.get("hint_filetranf") +
        '" />' +
        "<script>" +
        ...
        function HandleEventFiletransferStart(event) {
          ...
          if (
            ...
            webphone_api.common.ShowToast(
              webphone_api.stringres.get("filetransf_err"),
            );
            return;
          } else {
            ...
            webphone_api.common.PutToDebugLog(
              4,
              "EVENT, filetransfer directory: " + filepath,
            );
          }
        }
      }, 150);
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer: onStart " + lastoop.toString(),
        err,
      );
    } //ERROR, catch on _filetransfer: onStart ReferenceError: isNull is not defined (isNull is not defined)
  }
  ...
  function FileTransferOnSubmit(directory, filename) {
    try {
      webphone_api.common.PutToDebugLog(
        4,
        "EVENT, FileTransferOnSubmit called from iframe form",
      );
      webphone_api.common.PutToDebugLog(
        4,
        "EVENT, FileTransferOnSubmit directory: " +
          directory +
          "; filename: " +
          filename,
      );
      ...
      if (webphone_api.common.isNull(filename) || filename.length < 1) {
        webphone_api.common.PutToDebugLog(
          3,
          "ERROR, FileTransfer send failed: ivalid filename: " + filename,
        );
        webphone_api.common.ShowToast(
          webphone_api.stringres.get("fitransf_failed"),
        );
        return false;
      }
      ...
      var transferpath =
        ...
        directory +
        "/" +
        webphone_api.common.NormalizeFilename(filename);
      webphone_api.common.PutToDebugLog(
        4,
        "EVENT, FileTransferOnSubmit filepath: " + transferpath,
      );

      webphone_api
        .$("#ftranf_status")
        .html(webphone_api.stringres.get("ftrnasf_status_processing"));
      ...
      var ahref =
        ...
        "</a>";
      var msg =
        "[DONT_START_CHAT_WINDOW]" +
        webphone_api.common.GetSipusername() +
        " " +
        webphone_api.stringres.get("fitransf_chat") +
        ": " +
        ahref;

      var to = webphone_api.common.Trim(
      ...
      if (webphone_api.common.GetBrowser() === "Firefox") {
        if (!webphone_api.common.isNull(iframe)) {
          document
            .getElementById("ftranf_iframe_container")
            .removeChild(iframe);
        }
        FileUploaded(null);
        return false;
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer:  FileTransferOnSubmit",
        err,
      );
    }
    return false;
  ...
  function FileUploaded(evt) {
    // actually it's called on iframe.onload
    try {
      if (transf_initiated === false) {
        return;
      }
      ...
      webphone_api
        .$("#ftranf_status")
        .html(webphone_api.stringres.get("ftrnasf_status_waiting"));
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer:  FileUploaded",
        err,
      );
    }
  }

  function MeasureFiletransfer() {
    ...
    try {
      ...
      webphone_api
        .$("#page_filetransfer_content")
        .height(
          webphone_api.common.GetDeviceHeight() -
            webphone_api.$("#filetransfer_header").height() -
            2,
        );
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer: MeasureFiletransfer",
        err,
      );
    }
  }
  ...
  function PickContactResult(number) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer: PickContactResult",
        err,
      );
    }
  }
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _filetransfer: CreateOptionsMenu menuid null",
        );
        return false;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _filetransfer: CreateOptionsMenu can't get reference to Menu",
        );
        return false;
      }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer: CreateOptionsMenu",
        err,
      );
    }
  ...
  function MenuItemSelected(itemid) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_filetransfer: MenuItemSelected",
        err,
      );
    }
  }
