...
*/
  ...
  function onCreate(event) {
    try {
      ...
      webphone_api.$("#btn_extra1_menu").on("click", function () {
        CreateOptionsMenu("#extra1_menu_ul");
      });

      // set page title
      webphone_api.$("#extra1_title").html("Page Extra 1");
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_extra1: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra1: onStart", err);
    }
  }
  ...
  function MeasureExtra1() {
    ...
    var contentHeight =
      ...
      webphone_api.$("#extra1_header").height() -
      3;
    ...
    try {
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra1: CreateOptionsMenu menuid null",
        );
        return;
      }
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra1: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
 ...
 */
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra1: onStop", err);
    }
  }
