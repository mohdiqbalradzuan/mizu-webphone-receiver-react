...
*/
  ...
  function onCreate(event) {
    try {
      ...
      webphone_api.$("#btn_extra5_menu").on("click", function () {
        CreateOptionsMenu("#extra5_menu_ul");
      });

      // set page title
      webphone_api.$("#extra5_title").html("Page Extra 5");
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_extra5: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra5: onStart", err);
    }
  }
  ...
  function MeasureExtra5() {
    ...
    var contentHeight =
      ...
      webphone_api.$("#extra5_header").height() -
      3;
    ...
    try {
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra5: CreateOptionsMenu menuid null",
        );
        return;
      }
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra5: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
 ...
 */
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra5: onStop", err);
    }
  }
