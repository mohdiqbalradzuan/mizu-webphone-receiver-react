...
*/
  ...
  function onCreate(event) {
    try {
      ...
      webphone_api.$("#btn_extra3_menu").on("click", function () {
        CreateOptionsMenu("#extra3_menu_ul");
      });

      // set page title
      webphone_api.$("#extra3_title").html("Page Extra 3");
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_extra3: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra3: onStart", err);
    }
  }
  ...
  function MeasureExtra3() {
    ...
    var contentHeight =
      ...
      webphone_api.$("#extra3_header").height() -
      3;
    ...
    try {
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra3: CreateOptionsMenu menuid null",
        );
        return;
      }
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra3: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
 ...
 */
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra3: onStop", err);
    }
  }
