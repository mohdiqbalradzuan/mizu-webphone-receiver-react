...
webphone_api._callhistorylist = (function () {
  function onCreate(event) {
    ...
    try {
      ...
      {
        if (
          webphone_api.$.mobile.activePage.attr("id") === "page_callhistorylist"
        ) {
      ...
      webphone_api
        .$("#callhistorylist_menu_ul")
        .on("click", "li", function (event) {
          MenuItemSelected(webphone_api.$(this).attr("id"));
        });
      webphone_api.$("#btn_callhistorylist_menu").on("click", function () {
        CreateOptionsMenu("#callhistorylist_menu_ul");
      });
      webphone_api
        .$("#btn_callhistorylist_menu")
      ...
      if (webphone_api.common.UsePresence2() === true) {
        webphone_api
          .$("#callhistorylist_additional_header_left")
          .on("click", function () {
  ...
  function onStart(event) {
    try {
      ...
      if (
        ...
        document.getElementById("app_name_callhistorylist").innerHTML =
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#page_callhistorylist"),
            -30,
          ),
        );

      if (
        !webphone_api.common.isNull(document.getElementById("chlist_title"))
      ) {
        document.getElementById("chlist_title").innerHTML =
          webphone_api.stringres.get("chlist_title");
      }
      webphone_api
        .$("#chlist_title")
      ...
      if (!webphone_api.common.isNull(curruser) && curruser.length > 0) {
        webphone_api.$("#curr_user_callhistorylist").html(curruser);
      }
      // set status width so it's uses all space to curr_user
      var statwidth =
        webphone_api.common.GetDeviceWidth() -
        webphone_api.$("#curr_user_callhistorylist").width() -
        25;
      if (
      ...
      if (webphone_api.common.GetColortheme() === 22) {
        ...
        webphone_api
          .$("#nav_ch_callhistory IMG")
          .attr("src", "images/tab_callog_blue.png");
      }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: onStart",
        err,
      );
    }
  }

  function MeasureCallhistorylist() {
    ...
    try {
      ...
      var notwidth =
        webphone_api.common.GetDeviceWidth() -
        webphone_api.$("#callhistorylist_additional_header_left").width() -
        webphone_api.$("#callhistorylist_additional_header_right").width();
      ...
      webphone_api
        .$("#callhistorylist_notification")
        .height(
          Math.floor(
            webphone_api.$("#callhistorylist_additional_header_left").height(),
          ),
        );
      ...
      var heightTemp =
        ...
        webphone_api.$("#callhistorylist_header").height() -
        3;

      if (webphone_api.$("#footertext_callhistorylist").is(":visible")) {
        heightTemp =
          heightTemp - webphone_api.$("#footertext_callhistorylist").height();
      }

      if (webphone_api.$("#advert_callhistorylist").is(":visible")) {
        heightTemp =
          heightTemp - webphone_api.$("#advert_callhistorylist").height();
      }
      ...
      var brandW =
        webphone_api.$("#callhistorylist_additional_header_left").width() - 5;
      if (webphone_api.$("#callhistorylist_presence").is(":visible")) {
        brandW = brandW - webphone_api.$("#callhistorylist_presence").width();
      }
      webphone_api.$("#app_name_callhistorylist").width(brandW);
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: MeasureCallhistorylist",
        err,
      );
    }
  }
        ...
        webphone_api.common.ReadCallhistoryFile(function (success) {
          if (!success) {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _callhistorylist: Load call history failed",
            );
          }
    ...
    try {
      if (
        webphone_api.common.isNull(
          document.getElementById("callhistorylist_list"),
        )
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist: PopulateList listelement is null",
        );
        return;
      }
        ...
        webphone_api
          ...
          .html(
            '<span style="text-shadow:0 0 0;">' +
              webphone_api.stringres.get("no_history") +
              "</span>",
          );
        webphone_api.common.PutToDebugLog(
          2,
          "EVENT, _callhistorylist: PopulateList no history",
        );
        return;
      }
      ...
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _callhistorylist Starting populate list",
      );
      ...
      for (var i = 0; i < webphone_api.global.chlist.length; i++) {
        ...
        // handle filter
        if (
          webphone_api.global.callhistoryfilter === 0 &&
          item[webphone_api.common.CH_TYPE] !== "0"
        ) {
          continue;
        } else if (
          webphone_api.global.callhistoryfilter === 1 &&
          item[webphone_api.common.CH_TYPE] !== "1"
        ) {
          continue;
        } else if (
          webphone_api.global.callhistoryfilter === 2 &&
          item[webphone_api.common.CH_TYPE] !== "2" &&
        ...
        try {
          durationint = webphone_api.common.StrToInt(
            webphone_api.common.Trim(item[webphone_api.common.CH_DURATION]),
          );
        } catch (errin1) {
          webphone_api.common.PutToDebugLogException(
            2,
            "_callhistorylist: PopulateList convert duration",
            errin1,
          );
        }
        ...
        lisitem = lisitem.replace(
          "[HINT_DISC_REASON]",
          item[webphone_api.common.CH_REASON],
        );
  ...
  function OnListItemClick(id) {
    ...
    try {
      if (webphone_api.common.isNull(id) || id.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist OnListItemClick id is NULL",
        );
        return;
      }
      ...
      if (pos < 2) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist OnListItemClick invalid id",
        );
        return;
      }
      ...
      webphone_api.$.mobile.changePage("#page_callhistorydetails", {
        transition: "none",
        role: "page",
      });
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: OnListItemClick",
        err,
      );
    }
  }
  ...
  function OnListItemLongClick(id) {
    ...
    try {
      if (webphone_api.common.isNull(id) || id.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist OnListItemLongClick id is NULL",
        );
        return;
      }
      ...
      if (pos < 2) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist OnListItemLongClick invalid id 1",
        );
        return;
      }
      ...
      if (
        webphone_api.common.isNull(chid) ||
        chid.length < 1 ||
        !webphone_api.common.IsNumber(chid)
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist OnListItemLongClick invalid id 2: " + chid,
        );
        return;
      }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: OnListItemLongClick",
        err,
      );
    }
  }
  ...
  function CreateContextmenu(chid, popupafterclose) {
    try {
      ...
      if (
        webphone_api.common.GetContactIdFromNumber(
          chentry[webphone_api.common.CH_NUMBER],
        ) < 0
      ) {
        ...
        itemTemp = itemTemp.replace(
          "[ITEMTITLE]",
          webphone_api.stringres.get("menu_createcontact"),
        );
        list = list + itemTemp;
        itemTemp = "";
      } else {
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ch_delete"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      var template =
        "" +
        '<div id="ch_contextmenu" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        "px; min-width: " +
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: CreateContextmenu",
        err,
      );
    }
  }
  ...
  function CreateContact(chid) {
    try {
      webphone_api.$("#ch_contextmenu").on("popupafterclose", function (event) {
        ...
        webphone_api.global.intentaddeditct[1] =
          "numbertoadd=" + chentry[webphone_api.common.CH_NUMBER];
        var name = chentry[webphone_api.common.CH_NAME];
        if (
          webphone_api.common.isNull(name) ||
          name.length < 1 ||
          name === chentry[webphone_api.common.CH_NUMBER]
        ) {
          name = "";
        }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: CreateContact",
        err,
      );
    }
  }
  ...
  function EditContact(chid) {
    try {
      ...
      if (ctid < 0) {
        // means there is no contact found
        CreateContact(chid);
        return;
      }
  ...
  var MENUITEM_CALLHISTORYLIST_LASTCALLDETAILS =
    "#menuitem_callhistorylist_lastcalldetails";
  ...
  var MENUITEM_CALLHISTORYLIST_CUSTOM_LINK =
    "menuitem_callhistorylist_custom_link";
  var MENUITEM_CALLHISTORYLIST_CUSTOM_LINK2 =
    "menuitem_callhistorylist_custom_link2";
  var MENUITEM_CALLHISTORYLIST_CUSTOM_LINK3 =
    "menuitem_callhistorylist_custom_link3";
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist: CreateOptionsMenu menuid null",
        );
        return;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _callhistorylist: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
      ...
      if (
        !webphone_api.common.isNull(webphone_api.global.chlist) &&
        webphone_api.global.chlist.length > 0
      ) {
        webphone_api
          .$(menuId)
          .append(
            '<li id="' +
              MENUITEM_CALLHISTORYLIST_CLEAR +
              '"><a data-rel="back">' +
        ...
        webphone_api
          ...
          .append(
            '<li id="' +
              MENUITEM_CALLHISTORYLIST_FILTER +
              '"><a data-rel="back">' +
      ...
      if (
        featureset > 0 &&
        !webphone_api.common.isNull(webphone_api.global.lastcalldetails) &&
        webphone_api.global.lastcalldetails.length > 0
      ) {
        webphone_api
          .$(menuId)
          .append(
            '<li id="' +
              MENUITEM_CALLHISTORYLIST_LASTCALLDETAILS +
              '"><a data-rel="back">' +
              webphone_api.stringres.get("menu_lastcalldetails") +
              "</a></li>",
          )
          .listview("refresh");
      }
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_CALLHISTORYLIST_SETTINGS +
            '"><a data-rel="back">' +
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: CreateOptionsMenu",
        err,
      );
    }
  ...
  function MenuItemSelected(itemid) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: MenuItemSelected",
        err,
      );
    }
  }
  ...
  function Filter() {
    try {
      ...
      if (webphone_api.global.callhistoryfilter === -1) {
        allchecked = 'checked="checked"';
      } else if (webphone_api.global.callhistoryfilter === 0) {
        outchecked = 'checked="checked"';
      } else if (webphone_api.global.callhistoryfilter === 1) {
        inchecked = 'checked="checked"';
      } else if (webphone_api.global.callhistoryfilter === 2) {
        missedchecked = 'checked="checked"';
      }

      var radiogroup =
        '<input name="chfilter_all" id="chfilter_all" value="-1" ' +
        allchecked +
        ' type="radio">' +
        '<label for="chfilter_all">' +
        webphone_api.stringres.get("chfilter_all") +
        "</label>" +
        '<input name="chfilter_out" id="chfilter_out" value="0" ' +
        outchecked +
        ' type="radio">' +
        '<label for="chfilter_out">' +
        webphone_api.stringres.get("chfilter_out") +
        "</label>" +
        '<input name="chfilter_in" id="chfilter_in" value="1" ' +
        inchecked +
        ' type="radio">' +
        '<label for="chfilter_in">' +
        webphone_api.stringres.get("chfilter_in") +
        "</label>" +
        '<input name="chfilter_missed" id="chfilter_missed" value="2" ' +
        missedchecked +
        ' type="radio">' +
        '<label for="chfilter_missed">' +
        webphone_api.stringres.get("chfilter_missed") +
        "</label>";
      ...
      var template =
        "" +
        '<div id="ch_filter" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        "px; min-width: " +
        Math.floor(popupWidth * 0.6) +
        'px;">' +
        ...
        '<fieldset id="ch_filter_select" data-role="controlgroup">' +
        radiogroup +
      ...
      webphone_api.$(":radio").on("change", function (event) {
        ...
        if (
          webphone_api.common.isNull(valstr) ||
          valstr.length < 1 ||
          webphone_api.common.IsNumber(valstr) === false
        ) {
          webphone_api.common.PutToDebugLog(
            2,
            "ERROR,_callhistorylist: Filter invalid value: " + valstr,
          );
          return;
        }
  ...
  function ManuallyClosePopup(popupelement) {
    ...
    try {
      if (webphone_api.common.isNull(popupelement)) {
        return;
      }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: ManuallyClosePopup",
        err,
      );
    }
  }
  ...
  function ClearCallhistory(popupafterclose) {
    try {
      ...
      var template =
        "" +
        '<div id="adialog_clearcallhistory" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        '<div data-role="header" data-theme="b">' +
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        ...
        "<span> " +
        webphone_api.stringres.get("clear_callhistory_msg") +
        " </span>" +
        //        '<a href="javascript:;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back">' + webphone_api.stringres.get('btn_close') + '</a>' +
        //        '<a href="javascript:;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back" data-transition="flow">Delete</a>' +
        "</div>" +
      ...
      webphone_api.$("#btn_adialog_ok").on("click", function () {
        ...
        webphone_api.common.SaveCallhistoryFile(function (issaved) {
          webphone_api.common.PutToDebugLog(
            4,
            "EVENT, _callhistorylist: ClearCallhistory SaveCallhistoryFile: " +
              issaved.toString(),
          );
          PopulateList();
        });
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_callhistorylist: ClearCallhistory",
        err,
      );
    }
  }
