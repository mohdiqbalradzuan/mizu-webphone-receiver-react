...
webphone_api._logview = (function () {
  function onCreate(event) {
    ...
    try {
      ...
      webphone_api.$("#btn_logview_menu").on("click", function () {
        CreateOptionsMenu("#logview_menu_ul");
      });
      webphone_api
        .$("#btn_logview_menu")
      ...
      webphone_api.$("#sendtosupport").on("click", function () {
        var additionalinfo =
          "Build date: " + webphone_api.common.GetParameter("codegenerated");
        webphone_api.common.SendLog(
      ...
      webphone_api.$("#btn_loghelp").on("click", function () {
        webphone_api.common.AlertDialog(
          ...
          webphone_api.stringres.get("logview_help") +
            " " +
            webphone_api.common.GetParameter("support_email"),
          null,
          null,
          false,
        );
      ...
      webphone_api.$("#btn_sendlog").on("click", function () {
        ...
        if (webphone_api.common.isNull(pdesc) || pdesc.length < 3) {
          webphone_api.common.ShowToast(
            webphone_api.stringres.get("log_desc_error"),
          );
          return false;
        } else {
          ...
          webphone_api
            .$("#log_text")
            .html(pd + webphone_api.$("#log_text").html());
      ...
      webphone_api.common.PutToDebugLogException(2, "_logview: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      if (
        !webphone_api.common.isNull(document.getElementById("logview_title"))
      ) {
        document.getElementById("logview_title").innerHTML =
      ...
      if (
        !webphone_api.common.isNull(document.getElementById("logview_btnback"))
      ) {
        document.getElementById("logview_btnback").innerHTML =
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#logview_header"),
            -30,
          ),
        );
      ...
      webphone_api
        .$("#label_disable_logs")
        .html(webphone_api.stringres.get("disable_logs"));
      ...
      var logmanualemail = webphone_api.common.GetParameterInt(
        "logmanualemail",
        0,
      );
      ...
      if (webphone_api.common.isNull(email) || email.length < 1) {
        webphone_api.common.GetConfig("supportmail");
      }
      if (webphone_api.common.isNull(email) || email.length < 1) {
        webphone_api.common.GetConfig("log_email");
      }
      ...
      if (webphone_api.common.isNull(subject) || subject.length < 1) {
        subject = encodeURIComponent("WebPhone Log");
      }

      if (
        (logsendto === 2 || logmanualemail > 0) &&
        !webphone_api.common.isNull(email) &&
        email.length > 3
      ) {
        // send in email
        ...
        webphone_api
          .$("#sendtosupport_link")
          .html(webphone_api.stringres.get("sendtosupport"));
        ...
        var href =
          "mailto:" +
          webphone_api.common.Trim(email) +
          "?subject=" +
          subject +
          "&body=" +
          webphone_api.stringres.get("support_email_body");
        webphone_api.$("#sendtosupport_link").attr("href", href);
      ...
      if (!webphone_api.common.isNull(document.getElementById("filename"))) {
        ...
        try {
          if (
            ...
            !isNull(webphone_api.parameters.serveraddress)
          ) {
            srv = webphone_api.parameters.serveraddress;
          }
        } catch (errin) {}
        if (srv.length < 2) {
          srv = webphone_api.common.GetConfig("serveraddress");
        }
        if (webphone_api.common.isNull(srv)) {
          srv = "";
        }
        srv = srv.replace("://", "");
        ...
        if (webphone_api.common.isNull(logfilename) || logfilename.length < 1) {
          ...
          if (
            !webphone_api.common.isNull(
              webphone_api.common.GetParameter("brandname"),
            )
          ) {
            logfilename =
              logfilename +
              "_" +
              encodeURIComponent(webphone_api.common.GetParameter("brandname"));
          }
          if (!webphone_api.common.isNull(srv)) {
            logfilename = logfilename + "_" + encodeURIComponent(srv);
          }
        }

        webphone_api.common.PutToDebugLog(
          2,
          "EVENT, _logview filename: " + logfilename,
        );
      ...
      if (!webphone_api.common.isNull(document.getElementById("wplocation"))) {
        try {
          ...
          if (!webphone_api.common.isNull(loc) && loc.length > 0) {
            ...
            if (webphone_api.common.isNull(loc)) {
              loc = "";
            }
          ...
          webphone_api.common.PutToDebugLogException(
            2,
            "_logview: onStart Inner",
            err,
          );
        }
      }
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_logview: onStart", err);
    }
  }

  function MeasureLogview() {
    ...
    try {
      ...
      webphone_api
        .$("#page_logview_content")
        .height(
          webphone_api.common.GetDeviceHeight() -
            webphone_api.$("#logview_header").height() -
            2,
        );
      var ltheight =
        webphone_api.common.GetDeviceHeight() -
      ...
      if (webphone_api.$("#sendtosupport_container").is(":visible")) {
        ltheight =
          ltheight - webphone_api.$("#sendtosupport_container").height();
      }
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _logview: CreateOptionsMenu menuid null",
        );
        return;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _logview: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_logview: onStop", err);
    }
  }
