...
webphone_api._message = (function () {
  ...
  var composePlaceHolder =
    '<span style="color: #ccc;">' +
    webphone_api.stringres.get("messagepl") +
    "</span>";
  ...
  function onCreate(event) {
    ...
    try {
      ...
      webphone_api.$("#btn_message_menu").on("click", function () {
        CreateOptionsMenu("#message_menu_ul");
      });
      webphone_api
        .$("#btn_message_menu")
      ...
      webphone_api.$("#msg_textarea").keyup(function () {
        ...
        webphone_api
          .$("#msg_charcount")
          .text(webphone_api.$(this).html().length);
        ...
        if (
          ...
          webphone_api.common.GetTickCount() - lastiscomposingsenttick > 10000
        ) {
          //send in every 10 second on typing
      ...
      webphone_api.$("#msg_btn_sendfile").on("click", function () {
        webphone_api.common.FileTransfer(
          webphone_api.$("#msgpick_input").val(),
        );
      });
      webphone_api
        .$("#msg_btn_sendfile")
        .attr("title", webphone_api.stringres.get("hint_filetranf"));

      webphone_api
        .$("#btn_msgsend")
        .attr("title", webphone_api.stringres.get("hint_sendmsg"));

      webphone_api.$("#msg_btn_smiley").on("click", function () {
        OpenSmileys();
      });
      webphone_api
        .$("#msg_btn_smiley")
        .attr("title", webphone_api.stringres.get("hint_smiley"));

      webphone_api.$("#btn_msgpick").on("click", function () {
        webphone_api.common.PickContact(PickContactResult);
      });
      webphone_api
        .$("#btn_msgpick")
        .attr("title", webphone_api.stringres.get("hint_choosect"));

      webphone_api
        .$("#msg_btnback")
        .attr("title", webphone_api.stringres.get("hint_btnback"));
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_message: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#page_message"),
            -30,
          ),
        );
      ...
      mAction = webphone_api.common.GetIntentParam(
        webphone_api.global.intentmsg,
        "action",
      );
      webphone_api.global.mto = webphone_api.common.GetIntentParam(
        webphone_api.global.intentmsg,
        "to",
      );
      mMessage = webphone_api.common.GetIntentParam(
        webphone_api.global.intentmsg,
        "message",
      );

      if (
        !webphone_api.common.isNull(document.getElementById("msg_title")) &&
        !webphone_api.common.isNull(mAction)
      ) {
        if (mAction === "sms") {
          document.getElementById("msg_title").innerHTML =
            webphone_api.stringres.get("msg_title_sms");
        } else {
          document.getElementById("msg_title").innerHTML =
            webphone_api.stringres.get("msg_title_chat");
        }
      }
      webphone_api
        .$("#msg_title")
      ...
      if (!webphone_api.common.isNull(document.getElementById("msg_btnback"))) {
        document.getElementById("msg_btnback").innerHTML =
          "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("go_back_btn_txt");
      }

      if (webphone_api.common.isNull(webphone_api.global.mto)) {
      ...
      webphone_api
        .$("#msgpick_input")
        .attr("placeholder", webphone_api.stringres.get("chat_nr"));
      ...
      setTimeout(function () {
        ...
        if (
          webphone_api.common.isNull(tovalTmp) ||
          webphone_api.common.Trim(tovalTmp).length < 1
        ) {
      ...
      webphone_api.common.PutToDebugLogException(2, "_message: onStart", err);
    }
  }
  ...
  function HandleChatSms() {
    try {
      ...
      if (webphone_api.common.isNull(sms) || sms.length < 3) {
        return;
      }
      /*
      ...
      if (
        webphone_api.common.isNull(nr) ||
        webphone_api.common.Trim(nr).length < 1
      ) {
        return;
      }
...
//                    ChooseChatOrSms(number, msg);
  ...
  function MeasureMessage() {
    ...
    try {
      ...
      var heightTemp =
        ...
        webphone_api
          .$("#message_header")
          .height() /*- webphone_api.$('#msg_spacer').height()*/ -
        webphone_api.$("#msg_textarea_container").height();
  ...
  function LoadMessage() {
    try {
      ...
      if (
        !webphone_api.common.isNull(webphone_api.global.mto) &&
        webphone_api.global.mto.length > 0
      ) {
        ...
        webphone_api.File.ReadFile(
          currfile,
          webphone_api.global.STORAGE_LOCAL,
          function (content) {
            if (
              webphone_api.common.isNull(content) ||
              webphone_api.common.Trim(content).length < 1
            ) {
              webphone_api.common.PutToDebugLog(
                2,
                "ERROR, _message: LoadMessage no content",
              );
              content = "";
              document.getElementById("msgpick_container").style.display =
                "block";
              MeasureMessage();
            }
            mContent = content;
            ...
            if (
              !webphone_api.common.isNull(mMessage) &&
              mMessage.length > 0 &&
              mContent.indexOf("<p>" + mMessage + "</p>") < 0
            ) {
              AddMessage("1", true);
            }
    ...
    try {
      if (
        webphone_api.common.isNull(
          document.getElementById("page_message_content"),
        )
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: PopulateList listelement is null",
        );
        return;
      }
      // filenames: sms/chat_username_number
  ...
  function DisplayPresence(presence) {
    try {
      ...
      if (
        ...
        var ptitle = "";
        if (mAction === "sms") {
          ptitle = webphone_api.stringres.get("msg_title_sms");
        } else {
          ptitle = webphone_api.stringres.get("msg_title_chat");
        }
        ...
        if (
          !webphone_api.common.isNull(presenceimg) &&
          presenceimg.length > 0 &&
          !webphone_api.common.isNull(webphone_api.global.mto) &&
          webphone_api.global.mto.length > 0
        ) {
          ptitle =
  ...
  function RemoveNotification() {
    // remove new message notification (number) from filenames list for the opened message thread
    try {
      ...
      if (webphone_api.common.isNull(files) || files.length < 3) {
        webphone_api.common.PutToDebugLog(
          3,
          "EVENT, _message: RemoveNotification no message files",
        );
        return;
      }
      ...
      for (var i = 0; i < msglist.length; i++) {
        if (webphone_api.common.isNull(msglist[i]) || msglist[i].length < 3) {
          continue;
        }
        ...
        if (msglist[i].indexOf(currfile) === 0) {
          ...
          if (files.lastIndexOf(",") === files.length - 1) {
  ...
  function AddToGroupChat(dest) {
    try {
      if (
        webphone_api.common.isNull(dest) ||
        webphone_api.common.Trim(dest).length < 1
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: AddToGroupChat invalid destination: " + dest,
        );
        return;
      }
      ...
      if (webphone_api.common.isNull(currval)) {
        currval = "";
      }
      ...
      if (sendrec === true) {
        if (mAction === "chat") {
          ...
          for (var i = 0; i < dstlist.length; i++) {
            if (
              webphone_api.common.isNull(dstlist[i]) ||
              webphone_api.common.Trim(dstlist[i]).length < 1
            ) {
              continue;
            }
          ...
          var chatwith =
            "[" +
            webphone_api.stringres.get("gc_message") +
            ": " +
            currval +
            "]";
      ...
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _message: AddToGroupChat: " + dest,
      );
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_message: AddToGroupChat",
        err,
      );
    }
  }
  ...
  function SendMessage() {
    // validate and send chat/sms
    try {
      webphone_api.common.PutToDebugLog(
        5,
        "EVENT, _message: SendMessage onclick",
      );
      ...
      if (webphone_api.common.isNull(textarea)) {
        ...
        if (webphone_api.common.isNull(textarea)) {
          webphone_api.common.PutToDebugLog(
            2,
            "ERROR, _message: SendMessage textarea is NULL",
          );
          return;
        }
      }
      ...
      if (!webphone_api.common.isNull(mMessage)) {
        mMessage = mMessage.replace(composePlaceHolder, "");
      }
      ...
      if (
        webphone_api.common.isNull(tofrom) ||
        webphone_api.common.Trim(tofrom).length < 1
      ) {
        if (mAction === "sms") {
          webphone_api.common.ShowToast(
            webphone_api.stringres.get("err_msg_5"),
          );
        } else {
          webphone_api.common.ShowToast(
            webphone_api.stringres.get("err_msg_6"),
          );
        }
        return;
      } else {
      ...
      if (
        webphone_api.common.isNull(mMessage) ||
        webphone_api.common.Trim(mMessage).length < 1
      ) {
        //textarea.value = '';
        textarea.innerHTML = "";
        return;
  ...
  function SendAction(to, msg) {
    ...
    try {
      ...
      if (
        mAction === "sms" &&
        !webphone_api.common.isNull(smsapi) &&
        smsapi.length > 0
      ) {
        webphone_api.common.PutToDebugLog(
          5,
          "EVENT, _message SMS SendMessage to: " +
            webphone_api.global.mto +
            "; message: " +
            mMessage,
        );
          ...
          for (var i = 0; i < dstlist.length; i++) {
            ...
            if (sendrec === false) {
              ...
              webphone_api.common.UriParser(
                ...
                "",
                webphone_api.common.GetSipusername(),
                webphone_api.common.Trim(dstlist[i]),
                chatwith,
                "sendsms",
              );
            }
  ...
  function AddMessage(sentStatus, isincoming) {
    ...
    try {
      if (webphone_api.common.isNull(mMessage)) {
        webphone_api.common.PutToDebugLog(
          3,
          "ERROR, _message: AddMessage message is empty (null)",
        );
        return;
      }
      ...
      if (climit < 0) {
        if (
          webphone_api.common.GetParameter2("filterchatduplicates") === "true"
        ) {
          climit = 10000; // 10 sec
        } else {
          climit = 1000;
      ...
      var ctname = "";
      if (isincoming) {
        ctname = webphone_api.common.GetContactNameFromNumber(
          webphone_api.global.mto,
        );
      } else {
        ...
        if (mMessage.indexOf("<a href=") >= 0) {
          ...
          if (formattedmsg.indexOf('target="_blank"') >= 0) {
            formattedmsg = formattedmsg.replace(
              'target="_blank"',
              'target="_blank" onclick="try{webphone_api.filetransfercallback(\'' +
                webphone_api.global.mto +
                "')}catch(e){;}\"",
            );
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT,xxxxxx a: " + formattedmsg,
            );
          } else {
            formattedmsg = formattedmsg.replace(
              ">",
              " onclick=\"try{webphone_api.filetransfercallback('" +
                webphone_api.global.mto +
                "')}catch(e){;}\" >",
            );
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT,xxxxxx b: " + formattedmsg,
            );
          }
        } else {
          // try to get url
          ...
          if (
            !webphone_api.common.isNull(filename) &&
            filename.length > 0 &&
            !webphone_api.common.isNull(furl) &&
            furl.length > 10
          ) {
            ...
            var start_of_msg = webphone_api.common.Trim(
              mMessage.substring(0, pos),
            );

            webphone_api.common.PutToDebugLog(2, "EVENT,xxxxxx url: " + furl);
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT,xxxxxx fname: " + filename,
            );
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT,xxxxxx mto: " + webphone_api.global.mto,
            );
            ...
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT,xxxxxx formatted: " + formattedmsg,
            );
          } else {
            formattedmsg = mMessage;
          }
      ...
      if (webphone_api.common.isNull(formattedmsg) || formattedmsg.length < 1) {
        formattedmsg = mMessage;
      }
      ...
      if (
        webphone_api.common.isNull(webphone_api.global.mto) ||
        webphone_api.global.mto.length < 1
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: AddMessage destination number is NULL",
        );
        return;
      }
      ...
      for (var i = 0; i < msglist.length; i++) {
        if (webphone_api.common.isNull(msglist[i])) {
          continue;
        }
      ...
      if (mContent.length < 1) {
        ...
        if (webphone_api.common.IsWindowsSoftphone()) {
          webphone_api.common.ApiWinLoadFile(currfile, function (content) {
            ...
            webphone_api.common.ApiWinSaveFile(
              currfile,
              mContent,
              function (success) {
                if (success) {
                  webphone_api.common.PutToDebugLog(
                    2,
                    "ERROR, _message: AddMessage cannot save message file (1) WinApi",
                  );
                }
              },
            );
              ...
              webphone_api.File.SaveFile(
                ...
                function (success) {
                  if (!success) {
                    webphone_api.common.PutToDebugLog(
                      2,
                      "ERROR, _message: AddMessage cannot save message file (1)",
                    );
                  }
                },
              );
          ...
          webphone_api.common.ApiWinSaveFile(
            ...
            function (success) {
              if (!success) {
                webphone_api.common.PutToDebugLog(
                  2,
                  "ERROR, _message: AddMessage cannot save message file (2) WinApi",
                );
              }
            },
          );
          ...
          webphone_api.File.SaveFile(
            ...
            function (success) {
              if (!success) {
                webphone_api.common.PutToDebugLog(
                  2,
                  "ERROR, _message: AddMessage cannot save message file (2)",
                );
              }
            },
          );
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_message: AddMessage (" + lastoop.toString() + ")",
        err,
      );
    }
  }
  ...
  function SaveMissedIncomingMessage(action, from, name, msg) {
    try {
      ...
      if (webphone_api.common.isNull(from) || from.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: SaveMissedIncomingMessage source number is NULL",
        );
        return;
      }

      var currfile =
        action + "_" + webphone_api.common.GetSipusername() + "_" + from;
      ...
      for (var i = 0; i < msglist.length; i++) {
        ...
        if (tempmsgfile === currfile) {
          if (pos > 0) {
            ...
            if (temp.length > 0) {
              try {
                nrmissed = webphone_api.common.StrToInt(temp);
              } catch (err) {}
            }
          }
          nrmissed = nrmissed + 1;
      ...
      if (webphone_api.common.IsWindowsSoftphone()) {
        webphone_api.common.ApiWinLoadFile(currfile, function (content) {
          if (webphone_api.common.isNull(content)) {
            content = "";
          }
          ...
          webphone_api.common.ApiWinSaveFile(
            ...
            function (success) {
              if (!success) {
                webphone_api.common.PutToDebugLog(
                  2,
                  "ERROR, _message: SaveMissedIncomingMessage cannot save message file (1) WinApi",
                );
              }
            },
          );
            ...
            webphone_api.File.SaveFile(
              ...
              function (success) {
                if (!success) {
                  webphone_api.common.PutToDebugLog(
                    2,
                    "ERROR, _message: SaveMissedIncomingMessage cannot save message file (1)",
                  );
                }
              },
            );
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_message: SaveMissedIncomingMessage",
        err,
      );
    }
  }
  ...
  function ShowIncomingMessage(action, from, msg) {
    try {
      ...
      if (enablepres === true) {
        ...
        if (
          !webphone_api.common.isNull(lastpres_stat) &&
          lastpres_stat !== "0"
        ) {
        ...
        if (webphone_api.global.mto.indexOf(",") > 0) {
          ...
          for (var i = 0; i < dstlist.length; i++) {
            ...
            if (
              mAction === "sms" &&
              !webphone_api.common.isNull(
                webphone_api.common.GetParameter("sms"),
              ) &&
              webphone_api.common.GetParameter("sms").length > 0
            ) {
              webphone_api.common.UriParser(
                webphone_api.common.GetParameter("sms"),
                "",
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: CreateOptionsMenu menuid null",
        );
        return;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_MESSAGE_DELETE +
            '"><a data-rel="back">' +
            webphone_api.stringres.get("delete_text") +
            "</a></li>",
        )
        .listview("refresh");
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_MESSAGE_GROUPCHAT +
            '"><a data-rel="back">' +
            webphone_api.stringres.get("menu_groupchat") +
            "</a></li>",
        )
        .listview("refresh");
  ...
  function StartCall(number, isvideo) {
    try {
      if (webphone_api.common.isNull(number) || number.length < 1) {
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message: StartCall number is NULL",
        );
        return;
      }
      ...
      if (isvideo === true) {
        webphone_api.common.PutToDebugLog(
          4,
          "EVENT, _message initiate video call to: " + number,
        );
        webphone_api.videocall(number);
      } else {
        webphone_api.common.PutToDebugLog(
          4,
          "EVENT, _message initiate call to: " + number,
        );
        webphone_api.call(number, -1);
      }
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_message: StartCall", err);
    }
  }
  ...
  function ClearHistory(popupafterclose) {
    try {
      ...
      if (
        webphone_api.common.isNull(files) ||
        files.length < 3 ||
        webphone_api.common.isNull(webphone_api.global.mto) ||
        webphone_api.global.mto.length < 1
      ) {
      ...
      var template =
        ...
        "<span> " +
        webphone_api.stringres.get("delete_msg_alert") +
        " </span>" +
        ...
        webphone_api.$.mobile.activePage
          ...
          .on("popupafterclose", function (event) {
            ...
            webphone_api.File.DeleteFile(currfile, function (success) {
              webphone_api.common.PutToDebugLog(
                3,
                "EVENT, _message: ClearHistory DeleteFile: " +
                  currfile +
                  " status: " +
                  success.toString(),
              );
  ...
  function OpenSmileys() {
    try {
      ...
      if (webphone_api.common.isNull(scont)) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _message OpenSmileys: container is NULL",
        );
        return;
      }
      ...
      webphone_api.$("#msg_btn_smiley_close").on("click", function () {
        SmileyClose();
      });
      webphone_api
        .$("#msg_btn_smiley_close")
        .attr("title", webphone_api.stringres.get("btn_close"));
      ...
      if (!webphone_api.common.isNull(shint)) {
        webphone_api
          ...
          .on("mouseover", function () {
            shint.innerHTML = "Smiling :)";
          })
          .on("mouseout", function () {
            shint.innerHTML = "";
          });
        webphone_api.$("#btn_emoti_sad").on("mouseover", function () {
          shint.innerHTML = "Sad :(";
        });
        webphone_api.$("#btn_emoti_sad").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_laughing").on("mouseover", function () {
          shint.innerHTML = "Laughing :))";
        });
        webphone_api.$("#btn_emoti_laughing").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_winking").on("mouseover", function () {
          shint.innerHTML = "Winking ;)";
        });
        webphone_api.$("#btn_emoti_winking").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_surprised").on("mouseover", function () {
          shint.innerHTML = "Surprised :-O";
        });
        webphone_api.$("#btn_emoti_surprised").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_straightface").on("mouseover", function () {
          shint.innerHTML = "StraightFace :|";
        });
        webphone_api.$("#btn_emoti_straightface").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_worried").on("mouseover", function () {
          shint.innerHTML = "Worried :-S";
        });
        webphone_api.$("#btn_emoti_worried").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_crying").on("mouseover", function () {
          shint.innerHTML = "Crying :((";
        });
        webphone_api.$("#btn_emoti_crying").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_cool").on("mouseover", function () {
          shint.innerHTML = "Cool B-)";
        });
        webphone_api.$("#btn_emoti_cool").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_angel").on("mouseover", function () {
          shint.innerHTML = "Angel :-O)";
        });
        webphone_api.$("#btn_emoti_angel").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_kiss").on("mouseover", function () {
          shint.innerHTML = "Kiss :x";
        });
        webphone_api.$("#btn_emoti_kiss").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_idea").on("mouseover", function () {
          shint.innerHTML = "Idea :-I";
        });
        webphone_api.$("#btn_emoti_idea").on("mouseout", function () {
          shint.innerHTML = "";
        });
        webphone_api.$("#btn_emoti_thinking").on("mouseover", function () {
          shint.innerHTML = "Thinking :-?";
        });
        webphone_api.$("#btn_emoti_thinking").on("mouseout", function () {
          shint.innerHTML = "";
        });
      }
      ...
      webphone_api.$("#btn_emoti_smiling").on("click", function () {
        AddSmiley("Smiling");
      });
      webphone_api.$("#btn_emoti_sad").on("click", function () {
        AddSmiley("Sad");
      });
      webphone_api.$("#btn_emoti_laughing").on("click", function () {
        AddSmiley("Laughing");
      });
      webphone_api.$("#btn_emoti_winking").on("click", function () {
        AddSmiley("Winking");
      });
      webphone_api.$("#btn_emoti_surprised").on("click", function () {
        AddSmiley("Surprised");
      });
      webphone_api.$("#btn_emoti_straightface").on("click", function () {
        AddSmiley("StraightFace");
      });
      webphone_api.$("#btn_emoti_worried").on("click", function () {
        AddSmiley("Worried");
      });
      webphone_api.$("#btn_emoti_crying").on("click", function () {
        AddSmiley("Crying");
      });
      webphone_api.$("#btn_emoti_cool").on("click", function () {
        AddSmiley("Cool");
      });
      webphone_api.$("#btn_emoti_angel").on("click", function () {
        AddSmiley("Angel");
      });
      webphone_api.$("#btn_emoti_kiss").on("click", function () {
        AddSmiley("Kiss");
      });
      webphone_api.$("#btn_emoti_idea").on("click", function () {
        AddSmiley("Idea");
      });
      webphone_api.$("#btn_emoti_thinking").on("click", function () {
        AddSmiley("Thinking");
      });
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_message: OpenSmileys",
        err,
      );
    }
  }

  function AddEmoticon(txtin) {
    // convert emoticon text to image:  :) =>  image
    try {
      if (webphone_api.common.isNull(txtin) || txtin.length < 1) {
        return txtin;
      }
      var txt = txtin;

      txt = webphone_api.common.ReplaceAll(
        txt,
        ":))",
        '<img src="' +
          webphone_api.common.GetElementSource() +
      ...
      txt = webphone_api.common.ReplaceAll(
        ...
        '<img src="' +
          webphone_api.common.GetElementSource() +
          'images/smiley/StraightFace.gif">',
      );
      txt = webphone_api.common.ReplaceAll(
        txt,
        ":-S",
  ...
  function RemoveEmoticon(txtin) {
    // convert emoticon image to text:  image  => :)
    try {
      if (
        webphone_api.common.isNull(txtin) ||
        txtin.length < 1 ||
        txtin.indexOf("<img") < 0
      ) {
        return txtin;
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.SaveContactsFile(function (issaved) {
        webphone_api.common.PutToDebugLog(
          4,
          "EVENT, _message: onDestroy SaveContactsFile: " + issaved.toString(),
        );
      });
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_message: onStop", err);
    }
  }
