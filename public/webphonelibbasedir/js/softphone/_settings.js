...
webphone_api._settings = (function () {
  ...
  function onCreate(event) {
    // called only once - bind events here
    try {
      if (webphone_api.common.CanLog(4)) {
      ...
      webphone_api.$("#page_settings").keydown(function (event) {
        if (event.which === 13) {
          ...
          var active_popups =
            webphone_api.$.mobile.activePage.find(".messagePopup");
          if (
            !webphone_api.common.isNull(active_popups) &&
            active_popups.length > 0
          ) {
      ...
      webphone_api.$("#btn_settings_menu").on("click", function () {
        CreateOptionsMenu("#settings_menu_ul");
      });
      webphone_api
        .$("#btn_settings_menu")
        .attr("title", webphone_api.stringres.get("hint_menu"));

      webphone_api
        .$("#btn_back_settings")
        .attr("title", webphone_api.stringres.get("hint_btnback"));

      webphone_api
        .$("#lp_btn_login")
        .attr("title", webphone_api.stringres.get("hint_settlogin"));

      webphone_api.$("#btn_back_settings").on("click", function (event) {
        BackOnClick(event);
      });

      webphone_api
        .$("#btn_settings_engine_close")
      ...
      webphone_api.$("#a_forgotpassword").on("click", function (event) {
        if (webphone_api.common.IsWindowsSoftphone()) {
          var forgotpasswordurl =
            webphone_api.common.GetConfig("forgotpasswordurl");
          if (
            !webphone_api.common.isNull(forgotpasswordurl) &&
            forgotpasswordurl.length > 3
          ) {
      ...
      webphone_api.$("#lp_btn_custom").on("click", function (event) {
        CustomBtn();
      });
      ...
      webphone_api.$("#btn_settings_engine").on("click", function (event) {
        ...
        if (
          webphone_api.common.isNull(chooseenginetouse) ||
          chooseenginetouse.length < 1
        ) {
          return;
        }
        MeasureSettingslist();
        ...
        } else if (chooseenginetouse === "webrtc") {
          webphone_api.common.EngineSelect(1, 2);
        ...
        if (!webphone_api.common.isNull(engine)) {
          ...
          webphone_api.common.ShowToast(
            webphone_api.common.GetEngineDisplayName(chooseenginetouse) +
              " " +
              webphone_api.stringres.get("ce_use"),
            function () {
      ...
      //BRANDSTART
      if (webphone_api.common.GetConfigInt("brandid", -1) === 58) {
        // enikma
        var logodiv = document.getElementById("logologinpage");
        if (!webphone_api.common.isNull(logodiv)) {
          logodiv.innerHTML =
            '<img src="' +
            webphone_api.common.GetElementSource() +
            'images/logo.png" style="border: 0;">&nbsp;&nbsp;<div style="color:#000; font-size: .6em; display: inline-block; position: relative; top: -0.6em;"><b>eNikMa</b> Unified Comm</div>';
        }
      }
      //BRANDEND
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_settings: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      if (webphone_api.global.isdebugengine) {
        setTimeout(function () {
          webphone_api.common.ChooseEngineLogic(
            function () {
              console.log("StartUp");
            },
            false,
      ...
      document.getElementById("phone_app_main_container").style.display =
        "block";
      webphone_api.common.ShowModalLoader(
        webphone_api.stringres.get("loading"),
      );
      if (webphone_api.common.CanLog(3)) {
        webphone_api.common.PutToDebugLogSpecial(
          3,
          "EVENT, _settings: onStart display Loading... modal loader_1",
          false,
          "",
        );
      }
      ...
      if (webphone_api.common.IsWindowsSoftphone()) {
        webphone_api.common.WinAPI("API_GuiStarted", function (answer) {
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, _settings API_GuiStarted response: " + answer,
          );
        });
      }
      ...
      if (
        !webphone_api.common.isNull(
          document.getElementById("settings_page_title"),
        )
      ) {
        document.getElementById("settings_page_title").innerHTML =
          webphone_api.stringres.get("settings_title");
      }
      webphone_api
        .$("#settings_page_title")
        .attr("title", webphone_api.stringres.get("hint_page"));

      webphone_api
        .$(".separator_line_thick")
        .css(
          "background-color",
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#settings_header"),
            -30,
          ),
        );

      // fix for IE 10
      if (webphone_api.common.IsIeVersion(10)) {
        webphone_api
          .$("#settings_list")
          .children()
      ...
      webphone_api
        .$("#settings_page_title")
        .html(webphone_api.stringres.get("settings_login"));

      startedfrom = webphone_api.common.GetIntentParam(
        webphone_api.global.intentsettings,
        "startedfrom",
      );
      if (webphone_api.common.isNull(startedfrom)) {
        startedfrom = "";
      }
      ...
      if (startedfrom === "app") {
        ...
        webphone_api
          .$("#btn_back_settings")
          .html("<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("btn_cancel"));
      } else {
      ...
      if (ShowLoginPage()) {
        ...
        //OPSSTART
        if (
          !webphone_api.common.IsWindowsSoftphone() &&
          webphone_api.global.usestorage === true
        ) {
        ...
        if (webphone_api.common.GetConfigInt("brandid", -1) === 50) {
          ...
          webphone_api
            .$("#lp_btn_custom")
            .html(webphone_api.stringres.get("favafone_new"));
          webphone_api.$("#lp_btn_custom").show();
        }
        //BRANDEND
        else {
          if (
            !webphone_api.common.isNull(newuseruri) &&
            newuseruri.length > 3
          ) {
            webphone_api.$("#a_newuser").show();
            webphone_api
              .$("#a_newuser")
              .html(webphone_api.stringres.get("newuser_a"));
        ...
        if (
          ...
          webphone_api
            .$("#a_forgotpassword")
            .html(webphone_api.stringres.get("forgotpassword_a"));
      ...
      webphone_api.$("#lp_btn_login").on("click", function () {
        ...
        if (
          !webphone_api.common.isNull(srv_field) &&
          webphone_api
            .$("#lp_serveraddress")
            .closest("div.ui-input-text")
            .is(":visible") == true
        ) {
          var lpsrv = srv_field.value;
          if (webphone_api.common.isNull(lpsrv)) {
            lpsrv = "";
          }
        ...
        if (
          !webphone_api.common.isNull(document.getElementById("lp_username"))
        ) {
          var lpusr = document.getElementById("lp_username").value;
          if (webphone_api.common.isNull(lpusr)) {
            lpusr = "";
          }
        ...
        if (
          !webphone_api.common.isNull(document.getElementById("lp_password"))
        ) {
          var lppwd = document.getElementById("lp_password").value;
          if (webphone_api.common.isNull(lppwd)) {
            lppwd = "";
          }
      ...
      if (
        !webphone_api.common.isNull(document.getElementById("lp_serveraddress"))
      ) {
        document.getElementById("lp_serveraddress").placeholder =
          webphone_api.stringres.get("sett_display_name_serveraddress_user");
      }
      if (!webphone_api.common.isNull(document.getElementById("lp_username"))) {
        document.getElementById("lp_username").placeholder =
          webphone_api.stringres.get("sett_display_name_sipusername");
      }
      if (!webphone_api.common.isNull(document.getElementById("lp_password"))) {
        document.getElementById("lp_password").placeholder =
          webphone_api.stringres.get("sett_display_name_password");
      }

      var logo = webphone_api.common.GetConfig("logo");
      var logodiv = document.getElementById("logologinpage"); //logo
      if (
        !webphone_api.common.isNull(logo) &&
        logo.length > 2 &&
        !webphone_api.common.isNull(logodiv)
      ) {
        var isimg = false;
        if (
          logo.toLowerCase().indexOf(".jpg") > 0 ||
          logo.toLowerCase().indexOf(".jpeg") > 0 ||
          logo.toLowerCase().indexOf(".png") > 0 ||
          logo.toLowerCase().indexOf(".gif") > 0
        ) {
          isimg = true;
        }
      ...
      setTimeout(function () {
        if (
          ...
          if (
            webphone_api.common.GetOs() ===
            "Android" /* && webphone_api.common.GetBrowser() === 'Chrome'*/
          ) {
            var ep_webrtc = webphone_api.common.StrToInt(
              webphone_api.common.GetParameter2("enginepriority_webrtc"),
            );
            var ep_app = webphone_api.common.StrToInt(
              webphone_api.common.GetParameter2("enginepriority_app"),
            );
            ...
            if (
              ep_app < 1 ||
              ep_webrtc < 2 ||
              (ep_webrtc >= ep_app && usewebrtc === true)
            ) {
              return;
            }
            ...
            if (
              !webphone_api.common.isNull(active_popups) &&
              active_popups.length > 0 &&
              webphone_api.global.dontclosecurrpopup !== true
            ) {
              webphone_api.$.mobile.activePage
                .find(".messagePopup")
                .popup("close")
                .bind({
      ...
      //BRANDSTART
      if (
        webphone_api.common.GetConfigInt("brandid", -1) === 50 &&
      ...
      webphone_api.common.PutToDebugLogException(2, "_settings: onStart", err);
    }
  }
  ...
  function OfferNativeEngine(popupafterclose) {
    try {
      if (
        webphone_api.common.GetParameter2("enginepriority_webrtc") === "0" ||
        webphone_api.common.GetParameter2("enginepriority_app") === "0"
      ) {
        return;
      }
      ...
      if (
        !webphone_api.common.isNull(popupWidth) &&
        webphone_api.common.IsNumber(popupWidth) &&
        popupWidth > 100
      ) {
      ...
      var message =
        '<a href="javascript:;" id="pusenative">' +
        webphone_api.stringres.get("usenative_option_native") +
        "</a><br /><br />";
      message =
        message +
        '<a href="javascript:;" id="pusewebrtc">' +
        webphone_api.stringres.get("usenative_option_webrtc") +
        "</a>";

      var template =
        "" +
        '<div data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        '<div data-role="header" data-theme="b">' +
        //--        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' +
        webphone_api.stringres.get("usenative_title") +
        "</h1>" +
        "</div>" +
        '<div role="main" class="ui-content adialog_content adialog_alert">' +
        '<span style="-ms-user-select: text; -moz-user-select: text; -khtml-user-select: text; -webkit-user-select: text;  user-select: text;"> ' +
        message +
        " </span>" +
      ...
      webphone_api.$.mobile.activePage.find(".messagePopup").bind({
        popupbeforeposition: function () {
          ...
          var maxHeight = Math.floor(
            webphone_api.common.GetDeviceHeight() * 0.6,
          ); //-- webphone_api.$(window).height() - 120;
      ...
      webphone_api
        .$("#pusewebrtc")
        .on("click", function () //-- !!! also set in UserChooseEnginePopupSDK()
        {
  ...
  function MeasureSettingslist() {
    // resolve window height size change
    try {
      ...
      var pageheight =
        webphone_api.common.GetDeviceHeight() -
        webphone_api.$("#settings_header").height() -
        3;

      if (filtervisible) {
        var margin = webphone_api.common.StrToIntPx(
          webphone_api.$(".ui-input-search").css("margin-top"),
        );
        pageheight =
          pageheight -
          webphone_api.$("#settings_list").prev("form.ui-filterable").height() -
          margin -
          margin;
      }
      ...
      if (webphone_api.$("#settings_engine").is(":visible")) {
        pageheight =
          pageheight - webphone_api.$("#settings_engine").height() - 2;
      }

      if (webphone_api.$("#settings_copyright").is(":visible")) {
        var margin = webphone_api.common.StrToIntPx(
          webphone_api.$("#settings_copyright").css("margin-bottom"),
        );
        pageheight =
          pageheight - webphone_api.$("#settings_copyright").height() - margin;
      }
  ...
  function GetAccounts() {
    ...
    try {
      ...
      if (waitmaxloop > 18) {
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings GetAccounts File class failed to load",
        );
      }
      ...
      if (!webphone_api.common.isNull(filewaittimer)) {
        clearTimeout(filewaittimer);
      }
      filewaittimer = null;
      ...
      if (
        webphone_api.common.isNull(webphone_api.global.aclist) ||
        webphone_api.global.aclist.length < 1
      ) {
        ...
        webphone_api.common.ReadAccountsFile(function (accisread) {
          ...
          } // if we have accounts file, then we need to get settings filename and read setttings from file
          else {
      ...
      } // if we have accounts file, then read
      else {
  ...
  function GetSettings() {
    ...
    try {
      ...
      if (
        webphone_api.common.isNull(webphone_api.global.settmap) ||
        webphone_api.common.isNull(webphone_api.global.settmap["magicnumber"])
      ) {
        if (webphone_api.common.CanLog(5))
          webphone_api.common.PutToDebugLog(5, "EVENT, get settings file");
        var settfilename = webphone_api.common.GetActiveAccSettingsFilename();
        if (webphone_api.common.CanLog(5))
          webphone_api.common.PutToDebugLog(
            5,
            "EVENT, got settings file: " + settfilename,
          );

        //--DBG         webphone_api.common.PutToDebugLog(3, 'DEBUG, _settings GetSettings settfilename: ' + settfilename);

        if (
          webphone_api.common.isNull(settfilename) ||
          settfilename.length < 2
        ) {
          ...
          webphone_api.plhandler.ConfigChanged();
          webphone_api.init.SkinLoaded();
        } // we got the active account settings filename, we can read settings
        else {
          //--DBG             webphone_api.common.PutToDebugLog(3, 'DEBUG, _settings GetSettings ReadSettingsFile');
          if (webphone_api.common.CanLog(5))
            webphone_api.common.PutToDebugLog(5, "EVENT, read settings file");
          webphone_api.common.ReadSettingsFile(
            settfilename,
            function (settisread) {
              if (webphone_api.common.CanLog(5))
                webphone_api.common.PutToDebugLog(
                  5,
                  "EVENT, settings file readed.",
                );
  ...
  function AutoStart() {
    //if server, username and password is set, then auto start (don't show the settings)
    ...
    try {
      ...
      if (startedfrom !== "app") {
        ...
        if (webphone_api.common.isNull(upperSrvTemp) || upperSrvTemp.length < 1)
          upperSrvTemp = webphone_api.common.GetParameter("upperserverdomain");
        if (webphone_api.common.isNull(upperSrvTemp) || upperSrvTemp.length < 1)
          upperSrvTemp = webphone_api.common.GetParameter("upperserverip");
        var usrTemp = webphone_api.common.GetSipusername();
        var pwdTemp = webphone_api.common.GetParameter("password");

        if (
          webphone_api.common.GetParameter("serverinputisupperserver") ===
          "true"
        ) {
          if (
            !webphone_api.common.isNull(upperSrvTemp) &&
            upperSrvTemp.length > 2 &&
            !webphone_api.common.isNull(usrTemp) &&
            usrTemp.length > 2 &&
            !webphone_api.common.isNull(pwdTemp) &&
            pwdTemp.length > 2
          ) {
            autologin = true;
          }
        } else if (
          webphone_api.common.GetParameterBool("customizedversion", true) ===
          true
        ) {
          if (
            !webphone_api.common.isNull(usrTemp) &&
          ...
          if (
            !webphone_api.common.isNull(srvTemp) &&
            srvTemp.length > 2 &&
            !webphone_api.common.isNull(usrTemp) &&
            usrTemp.length > 2 &&
            !webphone_api.common.isNull(pwdTemp) &&
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "EVENT, _settings before StartWin",
        );
        ...
        if (autostart === 0) {
          webphone_api.common.PutToDebugLog(
            3,
            "EVENT, settings autologin disabled because autostart is 0",
          );
          autologin = false;
        }
        ...
        if (IsUsernameFixed() === true && IsPasswordFixed() === true) {
          ...
          if (
            autostart < 2 &&
            webphone_api.common.GetParameterInt("lastsucc", -1) === 0 &&
            webphone_api.common.IsSDK() === false
          ) {
            // only for skin
            autologin = false;
            webphone_api.common.PutToDebugLog(
              2,
              "WARNING,_settings AutoStart disabled, because last login was not successfull",
            );
            ...
            if (webphone_api.common.isNull(srvtmp) || srvtmp.length < 1) {
              srvtmp = webphone_api.common.GetParameter("serveraddress_orig");
            }
            if (webphone_api.common.isNull(srvtmp) || srvtmp.length < 1) {
              srvtmp = webphone_api.common.GetParameter("serveraddress");
            }
            ...
            if (
              usrtmp.length > 0 &&
              pwdtmp.length > 0 &&
              ((webphone_api.common.ShowServerInput() && srvtmp.length > 0) ||
                webphone_api.common.ShowServerInput() === false)
            ) {
        ...
        if (autologin) {
          ...
          if (webphone_api.common.CanLog(3)) {
            webphone_api.common.PutToDebugLogSpecial(
              3,
              "EVENT, _settings: onStart display Loading... modal loader_2",
              false,
              "",
            );
          }
          ...
          webphone_api.$.mobile.changePage("#page_dialpad", {
            transition: "pop",
            role: "page",
          });
          ...
          if (webphone_api.common.IsWindowsSoftphone()) {
            setTimeout(function () {
              ...
              webphone_api.startInner(
                webphone_api.common.GetSipusername(),
                webphone_api.common.GetParameter("password"),
              );
            }, 100);
          } else {
            setTimeout(function () {
        ...
        //OPSSTART
        currautoprovsrv =
          webphone_api.common.GetParameter("serveraddress_user");
        //OPSEND
        PopulateList();
      }
    } catch (err) {
  ...
  function EngineSelectStage1() {
    //-- call EngineSelect(stage  1) before login, if we are staying on settings page
    try {
      if (webphone_api.common.IsWindowsSoftphone()) {
        return;
      }
      ...
      eselect_called = true;
      if (
        webphone_api.global.engineselectstage !== 0 ||
        webphone_api.common.GetTickCount() -
          webphone_api.global.engineselecttime >
          webphone_api.global.ENGINE_DELAY ||
        webphone_api.global.webrtcavailable < 0
      ) {
    ...
    try {
      ...
      engineSelectTimer = setInterval(function () {
        ...
        if (limitmaxloop > 50) {
          webphone_api.common.PutToDebugLog(
            2,
            "ERROR, _settings: EngineSelect timeout",
          );
          clearInterval(engineSelectTimer);
        }
          ...
          if (ret < 1 || webphone_api.global.webrtcavailable < 0) {
            if (ret < 1) {
              if (webphone_api.common.CanLog(2)) {
                webphone_api.common.PutToDebugLog(
                  2,
                  "EVENT, _settings: waiting for EngineSelect (" +
                    limitmaxloop +
                    ")",
                );
              }
            ...
            if (webphone_api.global.webrtcavailable < 0) {
              if (webphone_api.common.CanLog(2)) {
                webphone_api.common.PutToDebugLog(
                  2,
                  "EVENT, _settings: waiting for WebRTC to load (" +
                    limitmaxloop +
                    ")",
                );
              }
            ...
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _settings: selected engine(" +
                webphone_api.global.engineselectstage +
                "): " +
                webphone_api.common.TestEngineToString(
                  webphone_api.common.GetSelectedEngine(),
                  false,
                ),
            );
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _settings: recommended engine(" +
                webphone_api.global.engineselectstage +
                "): " +
                webphone_api.common.TestEngineToString(
                  webphone_api.common.GetRecommendedEngine(),
                  false,
                ),
            );
            ...
            if (
              webphone_api.common.getuseengine() ===
                webphone_api.global.ENGINE_WEBRTC ||
              webphone_api.common.GetSelectedEngineName() ===
                webphone_api.global.ENGINE_WEBRTC
            ) {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "settings: WaitForEngineSelectSetting",
        err,
      );
    }
  }
  ...
  function ShowEngineOptionSettings(force_show) {
    try {
      ...
      if (
        webphone_api.common.isNull(webphone_api.global.useengine) ||
        webphone_api.global.useengine.length < 2
      ) {
        ...
        if (
          webphone_api.common.GetSelectedEngineName() === "java" &&
          webphone_api.common.GetBrowser() === "MSIE" &&
          webphone_api.common.IsJavaInstalled() < 2
        ) {
          //-- don't set webphone_api.global.useengine, because in plhandler.StartEngine it will  start recommended engine
        } else {
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, Settings set useengine: " + selengine,
          );
      ...
      webphone_api.common.ShowEngineOptionOnPage(function (msg, enginetouse) {
        ...
        var epwebrtc = webphone_api.common.GetParameter2(
        ...
        html_engineoption =
          '<li data-icon="carat-d" id="settingitem_chooseengine"><a class="noshadow mlistitem"><div class="sett_text"><span class="sett_display_name">' +
          webphone_api.stringres.get("sett_chooseengine_title") +
          '</span><br><span id="sett_comment_engineservice" class="sett_comment">' +
          webphone_api.stringres.get("sett_chooseengine_comment") +
          "</span></div></a></li>";
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: ShowEngineOptionSettings",
        err,
      );
    }
    return false;
  ...
  if (webphone_api.common.GetOs() !== "Android") {
    aspeak = "";
  }
  ...
  var settOrderReduced =
    "serveraddress_user,sipusername,password,username,displayname,email,hidemyidentity,textmessaging,voicemailnum,callbacknumber,loglevel";
  ...
  var settOrderReducedCustomized =
    "hidemyidentity,realm,use_fast_ice,use_stun,use_rport,register,registerinterval,keepaliveival,keepalive,natopenpackets,prack,capabilityrequest,setfinalcodec";
  var settorderAdvancedLoginSett =
    "serveraddress_user,proxyaddress,transport,displayname,username,sipusername,password,realm,settobasefunctionality,accounts,operation_mode,fine_tune,submenu_general,submenu_sipsettings,submenu_media,submenu_calldivert,submenu_profile,usetunneling,autoanswer_forward,submenu_integrate,hidemyidentity,use_fast_ice,use_stun,use_rport,register,registerinterval,acceptsrvexpire,enablepresence,enableblf,keepalive,natopenpackets,ringtimeout,calltimeout,dtmfmode,textmessaging,offlinechat,prack,sendearlymedia,sendrtponmuted,defmute,changesptoring,signalingport,rtpport,capabilityrequest,customsipheader,checksrvrecords,devtest,filters,numrewriterulesadv,techprefix,callforwardonbusy,rejectonvoipbusy,rejectonphonebusy,rejectphoneonvoipbusy,voicemail,callback_accessnumber,callback_mode,accessnumber,accessnumbercalltype,transfertype,transfwithreplace,automute,autohold,holdtypeonhold,enabledirectcalls,normalizenumber,ringincall,beeponconnect,vibrate,keypadfeedback,cpualwayspartiallock,forcewifi,wifionly,keepdeviceawakeincall,unlockphone,proximitysensor,dialerintegration,ringtone,theme,profilepicture,email,info,presencestatus,share_location,integrateifwifionly,nativefilterallow,nativefilterblock,displaynotification,screenrotation,savetocontacts,extraoption,voicerecording,keepcallhistory,sendchatonenter,editbeforecall,recentcontacts,removefromtask,log,loglevel,playring,cfgvideo,callbackonincomingvideo,video_bandwidth,androidspeaker,video_width,video_height,use_h263,use_h264,use_vp8,use_vp9,cfgcpuspeed,cfgnetworkspeed,submenu_video,codec,prefcodec,alwaysallowlowcodec,setfinalcodec,disablewbforpstn,aec_orig,denoise_orig,agc_orig,plc_orig,silencesupress,hardwaremedia,volumein,volumeout,mediaencryption,setqos,codecframecount,doublesendrtp,jittersize,audiobufferlength,speakerphoneoutput,audiorecorder,audioplayer,speakerphoneplayer,incomingcallalert,autousebluetooth,speakermode,focusaudio,useroutingapi,scheduledwakeup,autorestart,runservice,enablefcm,backexit,killonexit,reset_settings";

  //-- for windows softphone
  var settOrderWin =
    "serveraddress_user,sipusername,password,submenu_sipsettings,advancedloginsettings2,submenu_media,submenu_calldivert,submenu_general,submenu_profile,theme,language,startwithos," +
    exregacc +
    "voicerecording,keeprecfiles,proxyaddress,realm,username,profilepicture,displayname,email,presencestatus,share_location,profilestatustext,flash,beeponincoming,sendchatonenter,importonlywithnum,hidemyidentity," +
    utops +
  ...
  var basicSettings =
    "settobasefunctionality,theme,username,displayname," +
    utops +
    "transport,proxyaddress,register,ringtone,voicerecording,startonboot,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,cpualwayspartiallock,forcewifi,email,loglevel,startwithos";
  var basicSettingsCustomized =
    "settobasefunctionality,theme,displayname,ringtone,voicerecording,startonboot,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,cpualwayspartiallock,forcewifi,email,loglevel,startwithos";
  ...
  function PopulateList() {
    //will return true if auto switched inside Settings...
    try {
      ...
      if (ShowLoginPage()) {
        if (webphone_api.common.ShowServerInput()) {
          ...
          //--             !!!! HACK !!!
          if (
            webphone_api.common.GetParameter("showserverinput") === "1" &&
            webphone_api.common.GetConfigInt("upperserverfromuser", 2) < 2
          ) {
          } else {
            webphone_api
              .$("#lp_serveraddress")
              .val(webphone_api.common.GetParameter("serveraddress_user"));
        ...
        if (webphone_api.common.GetConfigInt("brandid", -1) !== 58) {
          ...
          webphone_api
            .$("#lp_username")
            .val(webphone_api.common.GetSipusername());
          webphone_api
            .$("#lp_password")
            .val(webphone_api.common.GetParameter("password"));
        }
        //BRANDEND
      ...
      if (
        webphone_api.common.isNull(document.getElementById("settings_list"))
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings: PopulateList listelement is null",
        );
        return false;
      }
      ...
      webphone_api.common.PutToDebugLog(
        5,
        "EVENT, _settings Starting populate list",
      );
      webphone_api.global.sipstackstarted = false;
      ...
      if (webphone_api.common.IsWindowsSoftphone()) {
        if (printdevice) {
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, settings: Device: Windows softphone",
          );
        }
        ...
        if (
          webphone_api.common.ParameterIsDefault("startwithos", true) === false
        ) {
        ...
        if (printdevice) {
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, settings: Device: browser webphone",
          );
        }
        ...
        if (
          webphone_api.common.getuseengine() ===
            webphone_api.global.ENGINE_WEBRTC ||
          webphone_api.common.getuseengine() ===
            webphone_api.global.ENGINE_FLASH
        ) {
        ...
        if (
          webphone_api.common.GetOs() !== "Android" &&
          webphone_api.common.GetOs() !== "iOS"
        ) {
      ...
      if (webphone_api.common.GetConfigInt("brandid", -1) === 50) {
        // favafone
        basicSettingsCustomized =
          "startwithos,loglevel,displayvolumecontrols,displayaudiodevice,savetocontacts";
        settOrderTmp =
          "startwithos,loglevel,displayvolumecontrols,displayaudiodevice,savetocontacts";
      }
      //BRANDEND
      ...
      var settOrderReducedCustomizedArray =
        settOrderReducedCustomized.split(",");
      ...
      if (webphone_api.common.isNull(extraLits)) {
        extraLits = [];
      }
      ...
      while (settOrderTmp.length > 0 && loopindex < 5000) {
        ...
        if (pos + 1 < settOrderTmp.length) {
          settOrderTmp = webphone_api.common.Trim(
            settOrderTmp.substring(pos + 1),
          );
        } else {
          settOrderTmp = "";
        }
        ...
        if (webphone_api.common.isNull(value)) {
          continue;
        }
        ...
        try {
          settGroupingInt = webphone_api.common.StrToInt(
            webphone_api.common.Trim(value[webphone_api.common.SETT_GROUP]),
          );
          if (webphone_api.common.isNull(settGroupingInt)) {
            settGroupingInt = 0;
          }
        } catch (errin) {
          webphone_api.common.PutToDebugLogException(
            3,
            "_settings: populateList settGrouping can't be converted to integer",
            errin,
          );
        }
        ...
        var settDisplayName = webphone_api.stringres.get(
          "sett_display_name_" + settName,
        );
        var settComment = webphone_api.stringres.get(
          "sett_comment_" + settName,
        );
        var settCommentShort = webphone_api.stringres.get(
          "sett_comment_short_" + settName,
        );
        ...
        if (!filtervisible) {
          ...
          if (isSettLevelBasic) {
            if (
              currGroup ===
              webphone_api.common.StrToInt(webphone_api.common.GROUP_LOGIN)
            ) {
              if (
                settGroupingInt !== currGroup &&
                settName !== "accounts" &&
                extraLits.indexOf(settName) < 0
              ) {
                continue;
              }
            } else {
              if (
                webphone_api.common.GetParameterBool(
                  "customizedversion",
                  true,
                ) === true
              ) {
            ...
            if (
              currGroup ===
              webphone_api.common.StrToInt(webphone_api.common.GROUP_PROFILE)
            ) {
              if (
                settGroupingInt !== currGroup &&
                settName !== "displayname" &&
                settName !== "email" &&
                settName !== "presencestatus"
              ) {
                continue;
              }
        ...
        //BRANDSTART
        if (webphone_api.common.GetConfigInt("brandid", -1) === 60) {
          //-- voipmuch
          if (settName === "username") {
            continue;
          } //-- hide caller ID
        }
        //BRANDEND
        ...
        if (settName === "accounts") {
          ...
          if (
            currGroup ===
              webphone_api.common.StrToInt(webphone_api.common.GROUP_SIP) ||
            (currGroup ===
              webphone_api.common.StrToInt(webphone_api.common.GROUP_LOGIN) &&
              !webphone_api.common.isNull(webphone_api.global.sipaccounts) &&
              webphone_api.global.sipaccounts.length > 1)
          ) {
          } else {
            continue;
          }
        ...
        if (
          settGroupingInt ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_INTEGRATE)
        ) {
          continue;
        }

        if (
          webphone_api.common.isNull(settType) ||
          settType.length < 1 ||
          (settType !== "0" &&
            settType !== "1" &&
            settType !== "2" &&
          ...
          if (settType !== "-1") {
            webphone_api.common.PutToDebugLog(
              ...
              "ERROR, _settings: " +
                settType +
                " is incorrectly defined in InitializeSettings",
            );
          }
          continue;
        }
        ...
        if (
          webphone_api.common.HideSettings(
            settName,
            settDisplayName,
            settName,
          ) === true
        ...
        if (settName === "serveraddress_user") {
          if (webphone_api.common.ShowServerInput()) {
            ...
            if (
              settDisplayName.toLowerCase().indexOf("op code") >= 0 ||
              settDisplayName.toLowerCase().indexOf("operator code") >= 0
            ) {
              settComment = webphone_api.stringres.get(
                "sett_comment_serveraddress_user_operator",
              );
            }
          } else {
        ...
        if (
          settName === "theme" &&
          (webphone_api.common.GetConfig("showtheme") === "false" ||
            webphone_api.common.GetParameterBool("customizedversion", false) ===
              true)
        ) {
          continue;
        }
        ...
        if (settName === "telsearchkey") {
          if (
            webphone_api.common.isNull(
              webphone_api.common.GetConfig("telsearchurl"),
            ) ||
            webphone_api.common.GetConfig("telsearchurl").length < 3
          ) {
            continue;
          }
        }
        ...
        if (settName === "icetimeout") {
          ...
          var ep_webrtc = webphone_api.common.GetParameterInt(
            "enginepriority_webrtc",
            2,
          );
          if (ep_webrtc < 1) {
          ...
          var ep_java = webphone_api.common.GetParameterInt(
            "enginepriority_java",
            2,
          );
          var ep_ns = webphone_api.common.GetParameterInt(
            "enginepriority_ns",
            2,
          );
          var ep_flash = webphone_api.common.GetParameterInt(
            "enginepriority_flash",
            2,
          );
          var ep_app = webphone_api.common.GetParameterInt(
            "enginepriority_app",
            2,
          );
          var ep_p2p = webphone_api.common.GetParameterInt(
            "enginepriority_p2p",
            2,
          );
          var ep_accessnum = webphone_api.common.GetParameterInt(
            "enginepriority_accessnum",
            2,
          );
          var ep_nativedial = webphone_api.common.GetParameterInt(
        ...
        if (
          settIsdefault === "0" ||
          settName === "sipusername" ||
          settName === "password" ||
          settName === "serveraddress_user"
        ) {
          // means the default value was changed
          comment =
            GetSettComment(settName) + " " + GetSettFormattedValue(settName);
        } else {
        ...
        if (
          ...
          if (settName === "serveraddress_user") {
            //BRANDSTART
            if (webphone_api.common.GetParameterInt("brandid", -1) === 2)
              // gmsdialergold
              comment =
                webphone_api.stringres.get(
                  "sett_comment_serveraddress_user_gmsdialer",
                ) +
                " " +
                GetSettFormattedValue(settName);
            //BRANDEND
            else
              comment =
                GetSettComment(settName) +
                " " +
                GetSettFormattedValue(settName);
          }
        }
        ...
        if (settName === "serveraddress_user") {
          if (webphone_api.common.GetConfig("server_comment").length > 1) {
            comment =
              webphone_api.common.GetConfig("server_comment") +
              " " +
              GetSettFormattedValue(settName);
          }
        }
        ...
        try {
          if (
            window.location.href.indexOf("isdemopage=true") > 0 &&
            (settName === "sipusername" ||
              settName === "username" ||
              settName === "password" ||
              settName === "sippassword" ||
              settName === "serveraddress_user" ||
              settName === "serveraddress_orig" ||
              settName === "serveraddress" ||
              settName === "upperserver" ||
              settName === "callto" ||
              settName === "destination")
          ) {
            var tmp = webphone_api.getparameter(settName);
            if (settName === "sipusername" && tmp.length < 1) {
              tmp = webphone_api.getparameter("username");
            }
            if (
              settName === "serveraddress_user" &&
              tmp.length < 1 &&
              webphone_api.common.RequestUserServerInput() === false
            ) {
              tmp = webphone_api.getparameter("serveraddress");
            }
            if (
              !webphone_api.common.isNull(tmp) &&
              tmp.length > 0 &&
        ...
        listitem =
          '<li data-icon="[SETTICON]" id="[ITEMID]" title="[SETT_TITLE]"><a class="noshadow mlistitem"><div class="sett_text"><span class="sett_display_name">[DISPLAYNAME]</span><br><span id="[COMMENTID]" class="sett_comment">[COMMENT]</span></div>[TRUEFALSEIMG]</a></li>';
        ...
        if (settType === "0") {
          ...
          if (
            ...
            if (settValue === "-1" || settValue === "1" || settValue === "2") {
              settValue = "true";
            } else {
              settValue = "false";
            }
          ...
          if (settName === "loglevel") {
            // handle loglevel separatelly, because values range from 1 to 5 or more
            var tmplevel = webphone_api.common.StrToInt(settValue);
            if (tmplevel > 1) {
              listitem = listitem.replace(
                "[TRUEFALSEIMG]",
                '<div class="sett_image"><img src="' +
                  webphone_api.common.GetElementSource() +
                  'images/checkbox_true.png" id="img_' +
                  settName +
                  '" /></div>',
              );
            } else {
              listitem = listitem.replace(
                ...
                '<div class="sett_image"><img src="' +
                  webphone_api.common.GetElementSource() +
                  'images/checkbox_false.png" id="img_' +
                  settName +
                  '" /></div>',
              );
            }
    ...
    if (filtervisible === false) {
      if (
        (startedfrom !== "app" ||
          webphone_api.common.GetParameter("sipusername").length < 1 ||
          webphone_api.common.GetParameter("password").length < 1) &&
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_LOGIN)
      ) {
        ...
        var settitem =
          '<li data-icon="carat-r" id="settingitem_advancedloginsettings" title="' +
          webphone_api.stringres.get("sett_comment_advancedloginsettings") +
          '"><a class="noshadow mlistitem"><div class="sett_text"><span class="sett_display_name">' +
          webphone_api.stringres.get(
            "sett_display_name_advancedloginsettings",
          ) +
          '</span><br><span id="sett_comment_advancedloginsettings" class="sett_comment">' +
          webphone_api.stringres.get("sett_comment_advancedloginsettings") +
          "</span></div></a></li>";
      ...
      if (
        (webphone_api.common.GetParameterBool("customizedversion", true) ===
          false ||
          startedfrom === "app" ||
          webphone_api.common.GetParameter("newuser_second").length > 3) &&
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_LOGIN)
      ) {
        if (show_showSetitngsEntry) {
          var settenteritem =
            '<li data-icon="carat-r" id="settingitem_entersettings" title="' +
            webphone_api.stringres.get("sett_comment_entersettings") +
            '"><a class="noshadow mlistitem"><div class="sett_text"><span class="sett_display_name">' +
            webphone_api.stringres.get("sett_display_name_entersettings") +
            '</span><br><span id="sett_comment_advancedsettings" class="sett_comment">' +
            webphone_api.stringres.get("sett_comment_entersettings") +
            "</span></div></a></li>";
      ...
      if (
        /*showadvacedsettings && */ isSettLevelBasic &&
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_MAIN) &&
        currfeatureset > 5 &&
        isAdvancedLoginSett < 1
      ) {
        var advanceditem =
          '<li data-icon="carat-r" id="settingitem_advancedsettings" title="' +
          webphone_api.stringres.get("sett_comment_submenu_advanced") +
          '"><a class="noshadow mlistitem"><div class="sett_text"><span class="sett_display_name">' +
          webphone_api.stringres.get("sett_display_name_advancedsettings") +
          '</span><br><span id="sett_comment_advancedsettings" class="sett_comment">' +
          webphone_api.stringres.get("sett_comment_advancedsettings") +
          "</span></div></a></li>";
      ...
      if (
        ...
        var moreitem =
          '<li data-icon="carat-r" id="settingitem_moresettings" title="' +
          webphone_api.stringres.get("sett_comment_moresettings") +
          '"><a class="noshadow mlistitem"><div class="sett_text"><span class="sett_display_name">' +
          webphone_api.stringres.get("sett_display_name_moresettings") +
          '</span><br><span id="sett_comment_moresettings" class="sett_comment">' +
          webphone_api.stringres.get("sett_comment_moresettings") +
          "</span></div></a></li>";
    ...
    //--    -ha megvan adva a configban a serveraddress-username-passsword, akkor menjen be a basic settingsbe (ne a login oldalon alljon meg ahol csak egy settings item van)
    if (
      webphone_api.common.IsWindowsSoftphone() === false &&
      (webphone_api.global.isdebugversion_showengineselection === true ||
        webphone_api.common.GetParameterInt("enproblem", 0) > 0 ||
        (webphone_api.common.GetParameterBool("last_login_failed", false) ===
          true &&
          currGroup ===
            webphone_api.common.StrToInt(webphone_api.common.GROUP_MAIN)))
    ) {
      html_engineoption =
        ...
        webphone_api.stringres.get("sett_chooseengine_comment") +
        " (" +
        webphone_api.common.GetEngineDisplayName(
          webphone_api.common.GetSelectedEngineName(),
        ) +
        ")" +
        "</span></div></a></li>";
      ...
      if (
        (currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_LOGIN) &&
          webphone_api.common.GetParameter("lastsessionsuccess") === "1") ||
        (currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_SIP) &&
          isSettLevelBasic === false)
      ) {
        if (webphone_api.common.isNull(html_engineoption)) {
          html_engineoption = "";
        }
        listview = html_engineoption + listview;
        hasonlysettings = 0;
    ...
    var footer =
      '<li id="settings_footer"><button id="btn_login" title="' +
      webphone_api.stringres.get("hint_settlogin") +
      '" class="ui-btn ui-btn-corner-all ui-btn-b noshadow">' +
      btnlogintitle +
      "</button></li>";

    var usrtmp = webphone_api.common.GetSipusername();
    if (webphone_api.common.isNull(usrtmp)) {
      usrtmp = "";
    }
    var pwdtmp = webphone_api.common.GetParameter("password");
    if (webphone_api.common.isNull(pwdtmp)) {
      pwdtmp = "";
    }
    ...
    if (webphone_api.common.IsWindowsSoftphone() === true) {
      ...
      if (qrcode_login == 1) {
        if (usrtmp.length < 1 || pwdtmp.length < 1) {
          qrlayout =
            '<li id="qrcode_layout"><button id="btn_qrcode_login" class="ui-btn ui-btn-corner-all ui-btn-b noshadow">' +
            webphone_api.stringres.get("btn_qrcode") +
            "</button></li>";
        }
    ...
    if (webphone_api.common.isNull(newuseruri) || newuseruri.length < 1)
      newuseruri = webphone_api.common.GetParameter("newuserurl");
    if (
      !webphone_api.common.isNull(newuseruri) &&
      newuseruri.length > 2 &&
      startedfrom !== "app" &&
      ...
      listview =
        listview +
        '<a href="javascript:;" id="al_newuser" title="' +
        webphone_api.stringres.get("hint_btnnewuser") +
        '" class="settings_links" target="_blank">' +
        webphone_api.stringres.get("newuser") +
        "</a>";
      hasonlysettings = 0;
    ...
    webphone_api.$("#al_newuser").on("click", function (event) {
      OnNewUserClicked();
      event.preventDefault();
    });
    ...
    webphone_api.$("#btn_qrcode_login").on("click", function () {
      ...
      webphone_api.common.PutToDebugLog(
        3,
        "EVENT, settings button QRcode login clicked",
      );
    ...
    webphone_api.$("#btn_login").on("click", function () {
      ...
      webphone_api.common.PutToDebugLog(
        3,
        "EVENT, settings button login clicked",
      );
    ...
    if (hasonlysettings == 2) {
      ...
      webphone_api
        .$("#settings_page_title")
        .html(webphone_api.stringres.get("settings_title"));
      webphone_api
        .$("#btn_back_settings")
        .html(
          "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("settings_login"),
        );
      return true;
    }
    return false;
  ...
  function OnListItemClick(id) {
    ...
    try {
      if (webphone_api.common.CanLog(3)) {
        webphone_api.common.PutToDebugLog(
          3,
          "EVENT, _settings: OnListItemClick click, id: " + id,
        );
      }
      ...
      if (webphone_api.common.isNull(id) || id.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings OnListItemClick id is NULL",
        );
        return;
      }
      ...
      if (
        id === "settingitem_advancedloginsettings" ||
        id === "settingitem_advancedloginsettings2"
      ) {
        ShowLoginSettings();
        return;
      }
      ...
      if (id === "settingitem_chooseengine") {
        var enginelist = webphone_api.common.GetEngineList();
        if (webphone_api.common.isNull(enginelist) || enginelist.length < 2) {
          return;
        }
        ...
        for (var i = 0; i < enginelist.length; i++) {
          ...
          if (
            webphone_api.common.isNull(engine) ||
            webphone_api.common.isNull(selname) ||
            selname.length < 1 ||
            (engine.name !== "java" &&
              engine.name !== "webrtc" &&
              engine.name !== "ns" &&
          ...
          var type = "0";
          if (engine.name === selname) {
            type = "3";
            defpriority = defpriority * 10;
          } else if (engine.name === recname) {
            type = "2";
            defpriority = defpriority * 4;
          } else if (webphone_api.common.EngineIsSupported(engine.name) > 0) {
            type = "1";
          }
        ...
        if (webphone_api.common.isNull(ulist) || ulist.length < 1) {
          return;
        }
        ...
        for (var i = 0; i < ulist.length; i++) {
          var oneen = ulist[i];
          if (webphone_api.common.isNull(oneen) || oneen.length < 1) {
            continue;
          }
          ...
          var item =
            '<input name="' +
            mCurrSettName +
            '" id="[INPUTID]" value="[VALUE]" [CHECKED] [DISABLED] type="radio">' +
          ...
          if (oneen[1] === "3") {
            ...
            item = item.replace(
              "[LABEL]",
              webphone_api.common.GetEngineDisplayName(oneen[0]) +
                " (" +
                webphone_api.stringres.get("sett_ce_highly") +
                ")",
            );
            ...
            item = item.replace(
              ...
              webphone_api.common.GetEngineDisplayName(oneen[0]) +
                " (" +
                webphone_api.stringres.get("sett_ce_recommended") +
                ")",
            );
            item = item.replace("[NOTBOLD]", "");
          } else if (oneen[1] === "0") {
        ...
        if (
          !webphone_api.common.isNull(pWidth) &&
          webphone_api.common.IsNumber(pWidth) &&
          pWidth > 100
        ) {
        ...
        var template =
          "" +
          '<div id="settings_user_ce_select" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
          pWidth +
          "px; min-width: " +
          Math.floor(pWidth * 0.6) +
          'px;">' +
          '<div data-role="header" data-theme="b">' +
          '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
          '<h1 class="adialog_title">' +
          webphone_api.stringres.get("sett_chooseengine_popup_title") +
          "</h1>" +
          "</div>" +
          '<div role="main" class="ui-content adialog_content_select">' +
          //--        '<form id="settings_select_2">' +
          '<fieldset id="settings_select_2" data-role="controlgroup">' +
          radiogroup +
          ...
          "</div>" +
          '<div data-role="footer" data-theme="b" class="adialog_footer">' +
          '<a href="javascript:;" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' +
          webphone_api.stringres.get("btn_ok") +
          "</a>" +
          '<a href="javascript:;" id="adialog_negative" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back">' +
          webphone_api.stringres.get("btn_cancel") +
          "</a>" +
          "</div>" +
          "</div>";
        ...
        webphone_api.$("#adialog_positive").on("click", function (event) {
          ...
          if (webphone_api.common.CanLog(3)) {
            webphone_api.common.PutToDebugLog(
              3,
              "EVENT, _settings: selected engine by user: " + newen,
            );
          }

          webphone_api
            .$("#sett_comment_engineservice")
            .html(
              webphone_api.stringres.get("sett_chooseengine_comment") +
                "(" +
                webphone_api.common.GetEngineDisplayName(newen) +
                ")",
            );
          ...
          if (newen === "java") {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _settings: chooseengine reset useengine_6 java",
            );
          } else if (newen === "webrtc") {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _settings: chooseengine reset useengine_1 webrtc: " +
                newen +
                " / " +
                currengine.name,
            );
            ...
            if (!webphone_api.common.isNull(currengine)) {
              webphone_api.common.PutToDebugLog(
                2,
                "EVENT, _settings: chooseengine reset useengine_2 ns: " +
                  newen +
                  " / " +
                  currengine.name,
              );
            ...
            if (srvc_installed === false) {
              //-- this should not be in IsServiceInstalled:callback, because window.open will only work on user interaction (click)
              ...
              var downloadurl = webphone_api.common.GetNPLocation();
              if (
                !webphone_api.common.isNull(downloadurl) &&
            ...
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _settings: chooseengine reset useengine_3 flash",
            );
            webphone_api.global.useengine = ""; //-- must be reset, otherwise plhandler.StartEngine() interprets it like the selected engine was not working, and starts recommended engine
            webphone_api.global.last_useengine = "";
          } else if (newen === "app") {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, _settings: chooseengine reset useengine_5 app",
            );
          ...
          if (newen !== "ns") {
            ...
            if (!webphone_api.common.isNull(currengine)) {
              webphone_api.common.PutToDebugLog(
                2,
                "EVENT, _settings: chooseengine reset useengine_4 nonns: " +
                  newen +
                  " / " +
                  currengine.name,
              );
      ...
      if (id === "settings_footer" || id === "newuser_container") {
        return;
      } // don't handle Save/Cancel buttons
      ...
      if (webphone_api.common.isNull(value) || value.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings OnListItemClick settings NULL: " + mCurrSettName,
        );
        return;
      }
      ...
      var settDisplayName = webphone_api.stringres.get(
        "sett_display_name_" + mCurrSettName,
      );
      var settComment = webphone_api.stringres.get(
        "sett_comment_" + mCurrSettName,
      );
      ...
      if (webphone_api.common.GetConfigInt("brandid", -1) === 50) {
        ...
        if (mCurrSettName === "sipusername") {
          settComment = "Your Favafone Username";
        }
        if (mCurrSettName === "password") {
          settComment = "Your Favafone Password";
        }
      }
      //BRANDEND
      ...
      if (
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings OnListItemClick invalid type",
        );
        return;
      }
      ...
      if (mSettType === "0") {
        ...
        //--         handle 2 as TRUE and 1 as FALSE
          ...
          mCurrSettName === "flash" ||
          mCurrSettName === "sscontrol" ||
          mCurrSettName === "ssscroll" ||
          mCurrSettName === "sstop"
        ) {
          if (mSettValue === "2") {
            mSettValue = "1";
          ...
          if (mSettValue === "-1" || mSettValue === "1" || mSettValue === "2") {
            mSettValue = "true";
          } else {
            mSettValue = "false";
          }
      ...
      if (mSettType === "1") {
        ...
        var template =
          "" +
          '<div id="settings_type_1" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
          popupWidth +
          'px;">' +
          '<div data-role="header" data-theme="b">' +
          '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        ...
        if (
          !webphone_api.common.isNull(mSettValue) &&
          mSettValue.length > 0 &&
          !webphone_api.common.isNull(textBox)
        ) {
          if (mCurrSettName === "password") {
            textBox.value = "*****";
          }
          ...
          // treat the followings as seconds instead of milliseconds if the value is between 2 and 999: ringtimeout,calltimeout,icetimeout,keepaliveival,wskeepaliveival,closecall_timeout
            ...
            mCurrSettName === "ringtimeout" ||
            mCurrSettName === "calltimeout" ||
            mCurrSettName === "icetimeout" ||
            mCurrSettName === "keepaliveival" ||
            mCurrSettName ===
              "wskeepaliveival" /*|| mCurrSettName === 'closecall_timeout'*/
          ) {
          ...
          // handle username / sipusername
            ...
            if (
              webphone_api.common.isNull(mSettValue) ||
              mSettValue.length <
                1 /* && webphone_api.common.GetParameter('username').length > 0*/
            ) {
        ...
        setTimeout(function () {
          if (!webphone_api.common.isNull(textBox)) {
            textBox.focus();
        ...
        webphone_api.$("#adialog_positive").on("click", function (event) {
          ...
          webphone_api.common.PutToDebugLog(
            5,
            "EVENT, settings onListItemClick 1 ok (" + mCurrSettName + ")",
          );
          ...
          if (
            ...
            if (mCurrSettName === "loglevel") {
              ...
              try {
                valint = webphone_api.common.StrToInt(mSettValue);
              } catch (errinner) {}
      ...
      if (mSettType === "2") {
        ...
        while (allNamesTmp2.indexOf(",") > 0) {
          //get number of options
          allNamesTmp2 = allNamesTmp2.substring(
            allNamesTmp2.indexOf(",") + 1,
            allNamesTmp2.length,
          );
          allCount2++;
        }
        ...
        while (countIdx2 < allCount2) {
          //get options names in array
          if (allNamesTmp2.indexOf(",") > 0) {
            arrayNames2[countIdx2] = allNamesTmp2.substring(
              0,
              allNamesTmp2.indexOf(","),
            );
            allNamesTmp2 = allNamesTmp2.substring(
              allNamesTmp2.indexOf(",") + 1,
              allNamesTmp2.length,
        ...
        while (countIdx2 < allCount2) {
          //get options values in array
          if (allValuesTmp2.indexOf(",") > 0) {
            arrayValues2[countIdx2] = allValuesTmp2.substring(
              0,
              allValuesTmp2.indexOf(","),
            );
            allValuesTmp2 = allValuesTmp2.substring(
              allValuesTmp2.indexOf(",") + 1,
              allValuesTmp2.length,
            );
          } else {
            arrayValues2[countIdx2] = allValuesTmp2.substring(
              0,
        ...
        for (var i = 0; i < arrayNames2.length; i++) {
          var item =
            '<input name="' +
            mCurrSettName +
            '" id="[INPUTID]" value="[VALUE]" [CHECKED] type="radio">' +
        ...
        if (
          !webphone_api.common.isNull(popupHeight) &&
          webphone_api.common.IsNumber(popupHeight) &&
          popupHeight > 100
        ) {
        ...
        var template =
          "" +
          '<div id="settings_type_2" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
          popupWidth +
          "px; min-width: " +
          Math.floor(popupWidth * 0.6) +
          'px;">' +
          '<div data-role="header" data-theme="b">' +
          '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
          '<h1 class="adialog_title">' +
          ...
          "</h1>" +
          "</div>" +
          '<div role="main" class="ui-content adialog_content_select" style="max-height: ' +
          popupHeight +
          'px;">' +
          //'<form id="settings_select_2">' +
          '<fieldset id="settings_select_2" data-role="controlgroup">' +
        ...
        webphone_api.$(":radio").on("change", function (event) {
          ...
          if (
            webphone_api.common.IsWindowsSoftphone() &&
            webphone_api.common.GetConfig("needactivation") == "true" &&
            webphone_api.common.GetParameter("canshowlickeyinput") !== "true" &&
            ((mCurrSettName === "transport" && mSettValue === "2") ||
              (mCurrSettName === "mediaencryption" && mSettValue === "2"))
          ) {
            webphone_api.common.ShowToast(
              webphone_api.stringres.get("warning_feature"),
              6000,
            );
            return;
          }
          ...
          if (mCurrSettName === "theme") {
            ...
            if (webphone_api.common.IsWindowsSoftphone()) {
              var url =
                webphone_api.common.AddJscommport(
                  webphone_api.global.WIN_SOFTPHONE_URL,
                ) +
                "?extcmd_theme=" +
                mSettValue;
              webphone_api.common.WinSoftphoneHttpReq(
                url,
                "GET",
                "",
                "",
                function (resp) {
                  webphone_api.common.PutToDebugLog(
                    2,
                    "EVENT, send theme to softphone response: " + resp,
                  );
                },
              );
            }
      ...
      if (mSettType === "3") {
        if (mCurrSettName === "videocodec") {
          var radiogroup =
            ...
            '<input name="' +
            mCurrSettName +
            '" id="videocodec_optimal" [CHECKED_OPTIMAL] type="checkbox">' +
            '<label for="videocodec_optimal">' +
            webphone_api.stringres.get("videocodec_optimal") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="videocodec_h264" [CHECKED_H264] type="checkbox">' +
            '<label for="videocodec_h264">H264</label>' +
            '<input name="' +
            mCurrSettName +
            '" id="videocodec_vp8" [CHECKED_VP8] type="checkbox">' +
            '<label for="videocodec_vp8">VP8</label>' +
            '<input name="' +
            mCurrSettName +
            '" id="videocodec_vp9" [CHECKED_VP9] type="checkbox">' +
          ...
          if (mSettValue === "-1") {
            ...
            var ch_h264 = 'checked="checked"';
            if (
              webphone_api.common.GetParameterBool("use_h264", true) === false
            ) {
              ch_h264 = "";
            }
            radiogroup = radiogroup.replace("[CHECKED_H264]", ch_h264);

            var ch_vp8 = 'checked="checked"';
            if (
              webphone_api.common.GetParameterBool("use_vp8", true) === false
            ) {
              ch_vp8 = "";
            }
            radiogroup = radiogroup.replace("[CHECKED_VP8]", ch_vp8);

            var ch_vp9 = 'checked="checked"';
            if (
              webphone_api.common.GetParameterBool("use_vp9", true) === false
            ) {
              ch_vp9 = "";
            }
          ...
          var template =
            "" +
            '<div id="settings_type_3" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" data-dismissible="false" style="max-width:' +
            popupWidth +
            "px; min-width: " +
            Math.floor(popupWidth * 0.6) +
            'px;">' +
            ...
            "</div>" +
            '<div data-role="footer" data-theme="b" class="adialog_footer">' +
            '<a href="javascript:;" style="width: 98%;" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' +
            webphone_api.stringres.get("btn_ok") +
            "</a>" +
            "</div>" +
            "</div>";
          ...
          webphone_api.$.mobile.activePage.find(".messagePopup").bind({
            popupbeforeposition: function () {
              webphone_api.$(this).unbind("popupbeforeposition"); //.remove();
              var maxHeight = Math.floor(
                webphone_api.common.GetDeviceHeight() * 0.7,
              ); // webphone_api.$(window).height() - 120;
          ...
          webphone_api.$("#adialog_positive").on("click", function (event) {
            ...
            webphone_api.common.PutToDebugLog(
              5,
              "EVENT, settings onListItemClick videocodec ok",
            );
            ...
            if (webphone_api.$("#videocodec_optimal").prop("checked")) {
              ...
              var cdh264 = "false";
              if (webphone_api.$("#videocodec_h264").prop("checked")) {
                cdh264 = "true";
              }
              webphone_api.common.SaveParameter("use_h264", cdh264);

              var cdvp8 = "false";
              if (webphone_api.$("#videocodec_vp8").prop("checked")) {
                cdvp8 = "true";
              }
              webphone_api.common.SaveParameter("use_vp8", cdvp8);

              var cdvp9 = "false";
              if (webphone_api.$("#videocodec_vp9").prop("checked")) {
                cdvp9 = "true";
              }
          ...
          for (var i = 0; i < arrayNames3.length; i++) {
            var item =
              '<input name="' +
              mCurrSettName +
              '" id="[INPUTID]" value="[VALUE]" [CHECKED] type="checkbox">' +
              '<label for="[INPUTID]">[LABEL]</label>';
          ...
          webphone_api.$(":checkbox").on("change", function (event) {
            ...
            if (val === "-1") {
              if (webphone_api.$("#" + mCurrSettName + "_0").prop("checked")) {
              ...
              if (
                ...
                webphone_api.common.IsWindowsSoftphone() &&
                webphone_api.common.GetConfig("needactivation") == "true" &&
                webphone_api.common.GetParameter("canshowlickeyinput") !==
                  "true"
              ) {
                //--                        setTimeout(function ()
                //--                        {
          ...
          webphone_api.$("#adialog_positive").on("click", function (event) {
            ...
            webphone_api.common.PutToDebugLog(
              5,
              "EVENT, settings onListItemClick 3 ok",
            );
            ...
            for (var i = 0; i < arrayNames3.length; i++) {
              if (
                webphone_api.$("#" + mCurrSettName + "_" + i).prop("checked")
              ) {
                var sep = "";
                ...
                mSettValue =
                  mSettValue +
                  sep +
                  document.getElementById(mCurrSettName + "_" + i).value;
              }
            }
            value[webphone_api.common.SETT_VALUE] = mSettValue;
            value[webphone_api.common.SETT_ISDEFAULT] = "0";
      ...
      if (mSettType === "4") {
        var template =
          "" +
          '<div id="settings_type_4" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
          popupWidth +
          'px;">' +
          '<div data-role="header" data-theme="b">' +
          '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        ...
        webphone_api.$("#adialog_positive").on("click", function (event) {
          ...
          webphone_api.common.PutToDebugLog(
            5,
            "EVENT, settings onListItemClick 2 ok (" + mSettValue + ")",
          );
          ...
          if (
            !webphone_api.common.isNull(textBoxVal) &&
            textBoxVal.length > 0
          ) {
            mSettValue = webphone_api.common.Trim(textBoxVal);
      ...
      if (mSettType === "5") {
        ...
        if (mCurrSettName === "filters") {
          ...
          webphone_api.$.mobile.changePage("#page_filters", {
            transition: "slide",
            role: "page",
          });
        } else if (mCurrSettName === "accounts") {
          webphone_api.$.mobile.changePage("#page_accounts", {
            transition: "slide",
            role: "page",
          });
        } else {
      ...
      //6 = submenu
      if (mSettType === "6") {
        //submenu_sipsettings,submenu_media,submenu_integrate,submenu_calldivert,submenu_general,submenu_integrate
        //--        webphone_api.$('#app_name_settings').hide();

        if (mCurrSettName === "submenu_sipsettings") {
          SubmenuSipSettings();
        } else if (mCurrSettName === "submenu_media") {
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_MEDIA),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          ...
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_VIDEO),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          ...
          webphone_api
            ...
            .html(
              "<b>&LT;</b>&nbsp;" +
                webphone_api.stringres.get("sett_display_name_submenu_media"),
            );
        } else if (mCurrSettName === "submenu_calldivert") {
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_CALLDIVERT),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          ...
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_GENERAL),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          ...
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_PROFILE),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          ...
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_INTEGRATE),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          ...
          currGroup = webphone_api.common.StrToInt(
            webphone_api.common.Trim(webphone_api.common.GROUP_SCRSHARE),
          );
          PopulateList();
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
        ...
        } else if (mCurrSettName === "submenu_advanced_artcl") {
          // customized for ARTCL
          webphone_api.$("#btn_back_settings").show();
          webphone_api.$("#app_name_settings").hide();
          currGroup = 6;
          PopulateList();
      ...
      if (mSettType === "8") {
        ...
        if (mCurrSettName === "audiodevices") {
          webphone_api.common.AlertDialog(
            webphone_api.stringres.get("sett_display_name_audiodevices"),
            webphone_api.stringres.get("sett_comment_audiodevices"),
            null,
            null,
            false,
          );
        ...
        if (mCurrSettName === "reset_settings") {
          var template =
            ...
            '<div role="main" class="ui-content adialog_content">' +
            "<span>" +
            webphone_api.stringres.get("reset_settings_msg") +
            "</span>" +
          ...
          webphone_api.$("#adialog_positive").on("click", function (event) {
            ...
            webphone_api.common.PutToDebugLog(
              5,
              "EVENT, settings onListItemClick Reset Settings ok",
            );
            ...
            var serveraddress_origTemp =
              webphone_api.common.GetParameter("serveraddress_orig");
            var serveraddress_userTemp =
              webphone_api.common.GetParameter("serveraddress_user");

            var proxyaddressTemp =
              webphone_api.common.GetParameter("proxyaddress");
            var sipusernameTemp =
              webphone_api.common.GetParameter("sipusername");
            var displaynameTemp =
              webphone_api.common.GetParameter("displayname");
            var emailTemp = webphone_api.common.GetParameter("email");
            var voicemailTemp =
              webphone_api.common.GetParameter("voicemailnum");
            var forwardnumberTemp =
              webphone_api.common.GetParameter("callforwardonbusy");
            ...
            webphone_api.common.SaveParameter(
              "serveraddress_orig",
              serveraddress_origTemp,
            );
            webphone_api.common.SaveParameter(
              "serveraddress_user",
              serveraddress_userTemp,
            );
            ...
            webphone_api.common.ShowToast(
              webphone_api.stringres.get("reset_settings_msg2"),
            );
          });
        ...
        if (mCurrSettName === "aec") {
          ...
          var template =
            "" +
            '<div id="settings_type_aec" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" data-dismissible="false" style="max-width:' +
            popupWidth +
            "px; min-width: " +
            Math.floor(popupWidth * 0.6) +
            'px;">' +
            ...
            ' type="checkbox">' +
            '<label for="aec_auto">' +
            webphone_api.stringres.get("aec_auto") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="aec_none" ' +
            ...
            ' type="checkbox">' +
            '<label for="aec_none">' +
            webphone_api.stringres.get("aec_none") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="aec_software" ' +
            cbSoftware +
            ' type="checkbox">' +
            '<label for="aec_software">' +
            webphone_api.stringres.get("aec_software") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="aec_native" ' +
            cbNative +
            ' type="checkbox">' +
            '<label for="aec_native">' +
            webphone_api.stringres.get("aec_native") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="aec_fast" ' +
            ...
            ' type="checkbox">' +
            '<label for="aec_fast">' +
            webphone_api.stringres.get("aec_fast") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="aec_decrease_volume" ' +
            cbDecreaseVolume +
            ' type="checkbox">' +
            '<label for="aec_decrease_volume">' +
            webphone_api.stringres.get("aec_decrease_volume") +
            "</label>" +
            "</fieldset>" +
            //--    '</form>' +
          ...
          webphone_api.$(":checkbox").on("change", function (event) {
            ...
            if (checkbox === "aec_software") {
              if (webphone_api.$("#" + checkbox).prop("checked")) {
                ...
                webphone_api.$("#aec_native").prop("checked") === false &&
            ...
            if (checkbox === "aec_native") {
              if (webphone_api.$("#" + checkbox).prop("checked")) {
                ...
                webphone_api.$("#aec_software").prop("checked") === false &&
                webphone_api.$("#aec_fast").prop("checked") === false &&
                webphone_api.$("#aec_decrease_volume").prop("checked") === false
              ) {
                webphone_api
          ...
          webphone_api.$("#adialog_positive").on("click", function (event) {
            ...
            webphone_api.common.PutToDebugLog(
              5,
              "EVENT, settings onListItemClick aec ok",
            );
            ...
            if (webphone_api.$("#aec_auto").prop("checked")) {
              mSettValue += "-1,";
            }
            if (webphone_api.$("#aec_none").prop("checked")) {
              mSettValue += "0,";
            }
            if (webphone_api.$("#aec_software").prop("checked")) {
              mSettValue += "1,";
            }
            if (webphone_api.$("#aec_native").prop("checked")) {
              mSettValue += "2,";
            }
            if (webphone_api.$("#aec_fast").prop("checked")) {
              mSettValue += "3,";
            }
            if (webphone_api.$("#aec_decrease_volume").prop("checked")) {
              mSettValue += "4,";
            }
        ...
        if (mCurrSettName === "serveraddress_user") {
          // has different popup (with help button)
          settDisplayName = webphone_api.common.GetParameter("server_label");
          ...
          var srvaddrpart = "";
          var widthclass = "";
          var sdsrv = webphone_api.common.GetParameter(
            "displaytopdomainserveraddress",
          );
          if (webphone_api.common.isNull(sdsrv)) {
            sdsrv = "";
          }
          if (sdsrv.length > 1) {
            srvaddrpart =
              '<span id="setting_item_srvaddrpart">' + sdsrv + "</span>";
            widthclass = 'data-wrapper-class="setting_item_input_class"';
          }

          var template =
            "" +
            '<div id="settings_type_server" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
            popupWidth +
            'px;">' +
            '<div data-role="header" data-theme="b">' +
            '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
            ...
            '<input type="text" ' +
            widthclass +
            ' id="setting_item_input" name="setting_item" data-theme="a" autocapitalize="off"/>' +
            srvaddrpart +
            '<button id="btn_srvhelp" class="btn_nexttoinput ui-btn ui-btn-corner-all ui-btn-b noshadow"><img src="' +
            webphone_api.common.GetElementSource() +
            'images/icon_help_mark.png"></button>' +
            "</div>" +
            '<div data-role="footer" data-theme="b" class="adialog_footer">' +
            '<a href="javascript:;" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' +
            webphone_api.stringres.get("btn_ok") +
          ...
          if (
            webphone_api.common.GetParameter("serverinputisupperserver") ===
              "true" &&
            (webphone_api.common.GetParameter("autoprovisioning").length < 1 ||
              webphone_api.common.GetParameter("autoprovisioning") === "0")
          ) {
          ...
          if (
            ...
            if (sdsrv.length > 1) {
              if (
                !webphone_api.common.isNull(valtmp) &&
                valtmp.length > 0 &&
                valtmp.indexOf(sdsrv) >= 0
              ) {
          ...
          webphone_api.$("#adialog_positive").on("click", function (event) {
            ...
            webphone_api.common.PutToDebugLog(
              5,
              "EVENT, settings onListItemClick serveraddress ok",
            );
            ...
            if (
              ...
              mSettValue = webphone_api.common.Trim(textBoxVal);
              if (webphone_api.common.isNull(mSettValue)) {
                mSettValue = "";
              }
              ...
              if (
                ...
                //OPSSTART
                if (
                  webphone_api.common.GetParameter("autoprovisioning") ===
                    "1" ||
                  webphone_api.common.GetParameter("autoprovisioning") === "2"
                ) {
                  webphone_api.common.SaveParameter(
                    "serveraddress_user",
                    mSettValue,
                ...
                if (
                  ...
                  if (
                    webphone_api.global.autoServerDeployVersion &&
                    mSettValue.indexOf(".") > 0
                  ) {
                    //-- IP or domain name is not accepted as serveraddress, only autoprov filename
                    webphone_api.common.AlertWindow(
                      webphone_api.stringres.get("warning"),
                      webphone_api.stringres.get("warning_msg_2"),
                    );
                    return;
                  }
                  ...
                  if (
                    mSettValue.indexOf(".") > 0 ||
                    mSettValue.toLowerCase() === "mizu"
                  ) {
                    //-- means it's IP address or domain; not autoprovisioning filename
                    webphone_api.common.SaveParameter(
                      "serveraddress_orig",
                      mSettValue,
                    );
          ...
          webphone_api.$("#btn_srvhelp").on("click", function (event) {
            ...
            var btn_findprovider =
              '<br /><a href="https://www.mizu-voip.com/VoIPServiceProviders.aspx" onclick="webphone_api.common.OpenLinkInExternalBrowser(\'https://www.mizu-voip.com/VoIPServiceProviders.aspx\')" id="btn_srvhelp" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b " target="_blank">' +
              webphone_api.stringres.get("help_provider") +
              "</a>";
            ...
            webphone_api.common.AlertDialog(
              webphone_api.stringres.get("help"),
              webphone_api.stringres.get("srvaddr_help") + btn_findprovider,
              null,
              null,
              false,
            );
  ...
  function HandleProfilePicture(popupafterclose) {
    ...
    try {
      ...
      if (!webphone_api.common.isNull(picsrc) || picsrc.length > 10) {
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "EVENT, HandleProfilePicture: display picture: " + picsrc,
        );
      }
      ...
      var template =
        ...
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' +
        webphone_api.stringres.get("sett_display_name_profilepicture") +
        "</h1>" +
        "</div>" +
        ...
        "</div>" +
        '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="javascript:;" id="btn_adialog_alert_ok" style="width: 98%;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' +
        webphone_api.stringres.get("btn_cancel") +
        "</a>" +
        "</div>" +
        "</div>";
      ...
      webphone_api.$.mobile.activePage.find(".messagePopup").bind({
        popupbeforeposition: function () {
          ...
          if (!webphone_api.common.isNull(document.getElementById("ppmypic"))) {
            document.getElementById("ppmypic").style.width =
              Math.floor(popupWidth / 3).toString() + "px";
          }
      ...
      webphone_api.$.mobile.activePage
        .find(".messagePopup")
        .popup()
        .popup("open", { positionTo: "#settings_header" })
        .bind({
          popupafterclose: function () {
            webphone_api.$(this).unbind("popupafterclose").remove();
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: HandleProfilePicture",
        err,
      );
    }
  }
  ...
  function BuildPPIframe() {
    try {
      ...
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, BuildPPIframe actionurl: " + actionurl,
      );
      ...
      var html =
        '<html><body style="margin 0; padding 0; background: transparent; width: 100%; overflow:hidden; font-size: 1em; color: #cecece;">' +
        "<style>" +
        ...
        '<form style=" width: 100%; margin: 0; padding: 0;" action="' +
        actionurl +
        '" method="post" enctype="multipart/form-data" id="frm_ppupload" name="frm_ppupload" onsubmit="return OnPPFormSubmit()">' +
        '<input type="hidden" id="filepath" name="filepath" value="">' +
        '<input name="filedata" type="file" id="fileinput" /><br />' +
        '<input type="submit" id="btn_uploadpic" value="' +
        webphone_api.stringres.get("btn_set") +
        '" title="' +
        webphone_api.stringres.get("hint_setprofpic") +
        '" />' +
        "</form>" +
        "</body></html>";
      ...
      if (
        webphone_api.common.isNull(
          document.getElementById("prifilep_container"),
        )
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, BuildPPIframe container element is NULL",
        );
        return;
      }
      ...
      setTimeout(function () {
        ...
        if (ifrmDoc.addEventListener) {
          ifrmDoc.addEventListener(
            "click",
            HandleEventFiletransferStart,
            false,
          );
        ...
        function HandleEventFiletransferStart(event) {
          ...
          webphone_api.common.PutToDebugLog(
            4,
            "EVENT, profilepic directory: " + filepath,
          );
        }
      }, 150);
    } catch (err) {
  ...
  function ProfilePicOnSubmit(directory, filename) {
    try {
      webphone_api.common.PutToDebugLog(
        4,
        "EVENT, ProfilePicOnSubmit directory: " +
          directory +
          "; filename: " +
          filename,
      );

      if (webphone_api.common.isNull(directory)) {
        directory = "";
      } else {
        directory = "/" + directory;
      }
      if (webphone_api.common.isNull(filename) || filename.length < 1) {
        webphone_api.common.PutToDebugLog(
          3,
          "ERROR, ProfilePicOnSubmit send failed: ivalid filename: " + filename,
        );
        webphone_api.common.ShowToast(
          webphone_api.stringres.get("ppupload_failed"),
        );
        return false;
      }
      ...
      var transferpath =
        ...
        directory +
        "/" +
        encodeURIComponent(webphone_api.common.NormalizeFilename(filename));
      webphone_api.common.PutToDebugLog(
        4,
        "EVENT, ProfilePicOnSubmit filepath: " + transferpath,
      );

      if (webphone_api.common.GetBrowser() === "Firefox") {
        if (
          !webphone_api.common.isNull(iframe) &&
          !webphone_api.common.isNull(
            document.getElementById("prifilep_container"),
          )
        ) {
  ...
  function SubmenuSipSettings() {
    try {
      currGroup = webphone_api.common.StrToInt(
        webphone_api.common.Trim(webphone_api.common.GROUP_SIP),
      );
      PopulateList();
      webphone_api.$("#btn_back_settings").show();
      webphone_api.$("#app_name_settings").hide();
      ...
      webphone_api
        .$("#settings_page_title")
        .html(
          webphone_api.stringres.get("sett_display_name_submenu_sipsettings"),
        );
      webphone_api
        .$("#btn_back_settings")
        .html(
          "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("settings_title"),
        );
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: SubmenuSipSettings",
        err,
      );
    }
  }

  function ManuallyClosePopup(popupelement) {
    // workaround for IE, sometimes popups are not closed simply by clicking the button, so we close it manually
    try {
      if (
        webphone_api.common.isNull(popupelement) ||
        webphone_api.common.isNull(popupelement.popup)
      ) {
        return;
      }
      if (webphone_api.common.GetBrowser() === "MSIE") {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: ManuallyClosePopup",
        err,
      );
    }
  }

  function ShowSettValue(settname) {
    // display sett value (if not default) in comment
    try {
      if (webphone_api.common.isNull(settname) || settname.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "WARNING, _settings: ShowSettValue settname is NULL",
        );
        return;
      }
      ...
      if (
        settname === "serveraddress_user" &&
        webphone_api.common.GetParameter("serverinputisupperserver") === "true"
      ) {
        document.getElementById("sett_comment_" + settname).innerHTML =
          GetSettComment(settname) + " " + GetSettFormattedValue(settname);
        return;
      }
      ...
      if (webphone_api.common.isNull(commentfield)) {
        webphone_api.common.PutToDebugLog(
          2,
          "WARNING, _settings: ShowSettValue commentfield is NULL",
        );
        return;
      }
  ...
  function GetSettComment(settname) {
    ...
    try {
      if (webphone_api.common.isNull(settname) || settname.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "WARNING, _settings: GetSettComment settname is NULL",
        );
        return "";
      }
      ...
      if (
        !webphone_api.common.isNull(settCommentShort) &&
        settCommentShort.length > 0
      ) {
  ...
  function GetSettFormattedValue(settname) {
    // returns displayable value of settings; ex: if sett value = 'true' -> it retuens 'enabeld'
    var fval = "";
    try {
      if (webphone_api.common.isNull(settname) || settname.length < 1) {
        webphone_api.common.PutToDebugLog(
      ...
      if (settname === "password" && settval.length > 0) {
        ...
        if (type === "0") {
          ...
          if (
            settname === "ringtimeout" ||
            settname === "calltimeout" /*&& settval.length > 2*/
          ) {
          ...
          if (
            webphone_api.common.isNull(allnames) ||
            allnames.length < 1 ||
            webphone_api.common.isNull(allvalues) ||
            allvalues.length < 1
          ) {
            return "";
          }
          ...
          if (settval.indexOf(",") > 0) {
            valarray = settval.split(",");
          } else {
            valarray[0] = settval;
          }
          ...
          if (
            ...
            webphone_api.common.isNull(allvalues) ||
            allvalues.length < 1 ||
            webphone_api.common.isNull(valarray)
          ) {
            return "";
          }
          ...
          if (settname === "aec") {
            var allnames = value[webphone_api.common.SETT_ALLNAMES].split(",");
            var allvalues =
              value[webphone_api.common.SETT_ALLVALUES].split(",");
            var valarray = [];
            ...
            if (
              webphone_api.common.GetParameter("serverinputisupperserver") ===
                "true" &&
              //OPSSTART
              (webphone_api.common.GetParameter("autoprovisioning").length <
                1 ||
                webphone_api.common.GetParameter("autoprovisioning") === "0")
              //OPSEND
            ) {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: GetSettFormattedValue",
        err,
      );
    }
    ...
    try {
      ...
      if (
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_SIP) ||
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_MEDIA) ||
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_CALLDIVERT) ||
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_GENERAL) ||
        currGroup ===
          webphone_api.common.StrToInt(webphone_api.common.GROUP_PROFILE)
      ) {
        currGroup = webphone_api.common.StrToInt(
          webphone_api.common.GROUP_MAIN,
        );
        PopulateList();
        //--        webphone_api.$('#btn_back_settings').hide();
        //--        webphone_api.$('#app_name_settings').show();
        ...
        currGroup = webphone_api.common.StrToInt(
          webphone_api.common.GROUP_LOGIN,
        );
        ...
        if (startedfrom === "app") {
          webphone_api
            .$("#btn_back_settings")
            .html(
              "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("btn_cancel"),
            );
        } else {
          webphone_api.$("#btn_back_settings").hide();
          webphone_api.$("#app_name_settings").show();
        ...
        currGroup = webphone_api.common.StrToInt(
          webphone_api.common.GROUP_GENERAL,
        );
        PopulateList();
        ...
        webphone_api
          .$("#settings_page_title")
          .html(
            webphone_api.stringres.get("sett_display_name_submenu_general"),
          );
        webphone_api
          .$("#btn_back_settings")
          .html(
        ...
        currGroup ===
        webphone_api.common.StrToInt(webphone_api.common.GROUP_VIDEO)
      ) {
        currGroup = webphone_api.common.StrToInt(
          webphone_api.common.GROUP_MEDIA,
        );
        PopulateList();

        webphone_api
          .$("#settings_page_title")
          .html(webphone_api.stringres.get("sett_display_name_submenu_media"));
        webphone_api
          .$("#btn_back_settings")
          .html(
            "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("settings_title"),
        ...
        currGroup ===
        webphone_api.common.StrToInt(webphone_api.common.GROUP_SCRSHARE)
      ) {
        currGroup = webphone_api.common.StrToInt(
          webphone_api.common.GROUP_MEDIA,
        );
  ...
  function ShowLoginPage() {
    // whether to show standard login page or settings list
    try {
      ...
      if (useloginpage > 8) {
        useloginpage = webphone_api.common.GetParameterInt("useloginpage", -1);
      }
      ...
      if (currfeatureset > 15) {
        currfeatureset = webphone_api.common.GetParameterInt("featureset", 10);
      }
  ...
  function SaveSettings(usrstart) {
    try {
      ...
      if (
        currautoprovsrv.toLowerCase() !==
        webphone_api.common.GetParameter("serveraddress_user").toLowerCase()
      ) {
        // means opcode changed
        webphone_api.common.SaveParameter("lastautoprov", "");
      }
      //OPSEND

      if (
        webphone_api.common.RequestUserServerInput() &&
        webphone_api.common.GetParameter("serverinputisupperserver") === "true"
      ) {
        // must have: sipusername, password, upperserver
        //--MODIFYINGUPPERSERVER
        var uppersrv = webphone_api.common.GetParameter("upperserver");
        var usersrv = webphone_api.common.GetParameter("serveraddress_user");

        if (
          (uppersrv.length < 0 || usersrv.indexOf(".") > 0) &&
          webphone_api.common.ShowServerInput()
        ) {
          webphone_api.common.SaveParameter("iswebrtcuppersrvfromuser", "true");
        ...
        if (
          ...
          settDisplayName = webphone_api.stringres.get(
            "sett_display_name_serveraddress_user",
          );
          settComment = webphone_api.stringres.get(
          ...
          if (webphone_api.common.isNull(settValue) || settValue.length <= 0) {
            ...
            webphone_api.common.ShowToast(
              webphone_api.stringres.get("please_enter") + " " + settComment,
            );
        ...
        if (webphone_api.common.isNull(settValue) || settValue.length <= 0) {
          ...
          webphone_api.common.ShowToast(
            webphone_api.stringres.get("username_warning"),
          );
          return;
        }
        if (
          webphone_api.common.GetConfigBool("mizuserveronly", false) === true &&
          settValue.length < 3
        ) {
          webphone_api.common.PutToDebugLog(
            1,
            "WARNING, " +
              webphone_api.stringres.get("username_warning2") +
              "_1",
          );
          webphone_api.common.ShowToast(
            webphone_api.stringres.get("username_warning2"),
        ...
        if (
          ...
          webphone_api.common.PutToDebugLog(
            2,
            "WARNING, " +
              webphone_api.stringres.get("password_warning2") +
              "_1",
          );
          webphone_api.common.ShowToast(
            webphone_api.stringres.get("password_warning2"),
        ...
        if (
          ...
          if (
            (webphone_api.common.GetConfigBool("mizuserveronly", false) ===
              true) ===
              true &&
            settValue.length < 3
          ) {
      ...
      setTimeout(function () {
        ...
        if (webphone_api.common.IsWindowsSoftphone()) {
          if (
            webphone_api.common.ParameterIsDefault("startwithos") === false &&
            webphone_api.common.GetParameterBool(
              "startwithos_was_sent",
              true,
            ) === false
          ) {
            var url =
              ...
              "?extcmd_startwithos=" +
              webphone_api.common.GetParameter("startwithos");
            webphone_api.common.WinSoftphoneHttpReq(
              url,
              "GET",
              "",
              ...
              function (resp) {
                webphone_api.common.PutToDebugLog(
                  2,
                  "EVENT, _settings: startwithos response: " + resp,
                );
              },
            );
      ...
      webphone_api
        .$("#settings_page_title")
        .html(webphone_api.stringres.get("loading"));
      webphone_api.common.PutToDebugLogSpecial(
        1,
        "EVENT, _settings: SaveSettings set Loading... page title",
        false,
        "",
      );
      ...
      if (accountsavailable === false) {
        // true, if there is at least one account created. If "false". means we have to add an account at SaveSettings()
        ...
        var settfilename = webphone_api.common.Md5Hash(
          accname +
            webphone_api.common.GetParameter("serveraddress_user") +
            webphone_api.common.GetParameter("sipusername") +
            webphone_api.global.SALT +
            webphone_api.common.GetParameter("password") +
            webphone_api.common.GetLocationPathName(),
        );

        var extra = webphone_api.common.GetConfig("brandid");
        if (
          webphone_api.common.isNull(extra) ||
          extra.length < 1 ||
          extra == "-1"
        ) {
        ...
        acctemp[webphone_api.common.AC_SIPUSERNAME] =
          webphone_api.common.GetSipusername();
      ...
      webphone_api.common.SaveSettingsFile(
        7,
        webphone_api.common.GetActiveAccSettingsFilename(),
        function (retval) {
          ...
          if (window.location.href.indexOf("isdemopage=true") > 0) {
            try {
              var stmp = "";
              stmp = webphone_api.common.GetParameter("sipusername");
              if (stmp.length > 0) {
                webphone_api.setparameter("sipusername", stmp);
              }
              stmp = webphone_api.common.GetParameter("username");
              if (stmp.length > 0) {
                webphone_api.setparameter("username", stmp);
              }
              stmp = webphone_api.common.GetParameter("password");
              if (stmp.length > 0) {
                webphone_api.setparameter("password", stmp);
              }
              stmp = webphone_api.common.GetParameter("sippassword");
              if (stmp.length > 0) {
                webphone_api.setparameter("sippassword", stmp);
              }
              stmp = webphone_api.common.GetParameter("serveraddress_user");
              if (stmp.length > 0) {
                webphone_api.setparameter("serveraddress_user", stmp);
              }
              stmp = webphone_api.common.GetParameter("serveraddress_orig");
              if (stmp.length > 0) {
                webphone_api.setparameter("serveraddress_orig", stmp);
              }
              stmp = webphone_api.common.GetParameter("serveraddress");
              if (stmp.length > 0) {
                webphone_api.setparameter("serveraddress", stmp);
              }
              stmp = webphone_api.common.GetParameter("upperserver");
              if (stmp.length > 0) {
                webphone_api.setparameter("upperserver", stmp);
              }
              stmp = webphone_api.common.GetParameter("callto");
              if (stmp.length > 0) {
                webphone_api.setparameter("callto", stmp);
              }
              stmp = webphone_api.common.GetParameter("destination");
              if (stmp.length > 0) {
                webphone_api.setparameter("destination", stmp);
              }
            } catch (e) {
              webphone_api.common.PutToDebugLogException(
                2,
                "_settings: SaveSettings inner1",
                e,
              );
            }
          }
            ...
            webphone_api.common.SaveAccountsFile(function (success) {
              if (webphone_api.common.CanLog(2)) {
                if (success) {
                  webphone_api.common.PutToDebugLog(
                    3,
                    "EVENT, _settings: SaveSettings Accounts file saved successfully",
                  );
                } else {
                  webphone_api.common.PutToDebugLog(
                    2,
                    "ERROR, _settings: SaveSettings Accounts file save failed",
                  );
                }
              }
            });
  ...
  function BeforeStart(usrstart) {
    //-- handle autoprovisioning
    try {
      StartPhone(usrstart);
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: BeforeStart",
        err,
      );
    }
  }
    ...
    try {
      ...
      setTimeout(function () {
        ...
        if (usrstart === true) {
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, mlogic API_Start _settings StartPhone",
          );
          webphone_api.start(
            webphone_api.common.GetSipusername(),
            webphone_api.common.GetParameter("password"),
    ...
    try {
      ...
      webphone_api.common.WinSoftphoneHttpReq(
        ...
        function (resp) {
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, _settings: extcmd_qrcode response: " + resp,
          );
        },
      );
    } catch (err) {
...
*/
      ...
      webphone_api.$.mobile.changePage("#page_newuser", {
        transition: "pop",
        role: "page",
      });
    } catch (err) {
  ...
  function CreateOptionsMenu(menuId) {
    // adding items to menu, called from html
    try {
      // remove data transition for windows softphone, because it's slow
      if (
        webphone_api.common.GetParameter("devicetype") ===
        webphone_api.common.DEVICE_WIN_SOFTPHONE()
      ) {
        webphone_api.$("#btn_settings_menu").removeAttr("data-transition");
      }

      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings: CreateOptionsMenu menuid null",
        );
        return;
      }

      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _settings: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_SPEEDTEST +
            '"><a data-rel="back">' +
            webphone_api.stringres.get("menu_speedtest") +
            "</a></li>",
        )
        .listview("refresh");
      webphone_api
        .$("#" + MENUITEM_SPEEDTEST)
        .attr("title", webphone_api.stringres.get("hint_speedtest"));
      ...
      if (
        ...
        webphone_api
          ...
          .append(
            '<li id="' +
              MENUITEM_SHOW_SETTINGS +
              '"><a data-rel="back">' +
              webphone_api.stringres.get("menu_showsettings") +
              "</a></li>",
          )
          .listview("refresh");
      } else {
      ...
      if (webphone_api.common.GetConfigInt("brandid", -1) === 60) {
        ...
        if (startedfrom === "app") {
          webphone_api
            ...
            .append(
              '<li id="' +
                MENUITEM_LOGS_CUSTOM +
                '"><a data-rel="back">' +
                webphone_api.stringres.get("logview_title") +
                "</a></li>",
            )
            .listview("refresh"); // add logs to menu if advanced settinsg are displayed
        }
        ...
        webphone_api
          ...
          .append(
            '<li id="' +
              MENUITEM_EXIT +
              '"><a data-rel="back">' +
              webphone_api.stringres.get("menu_exit") +
              "</a></li>",
          )
          .listview("refresh");
      }
  ...
  function MenuItemSelected(itemid) {
    try {
      if (webphone_api.common.isNull(itemid) || itemid.length < 1) {
        return;
      }
      ...
      webphone_api.$("#settings_menu").on("popupafterclose", function (event) {
        ...
        switch (itemid) {
          ...
          case MENUITEM_LOGS_CUSTOM:
            webphone_api.$.mobile.changePage("#page_logview", {
              transition: "pop",
              role: "page",
            });
            break;
  ...
  function SwitchBetweenBasicAdvanced() {
    try {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: SwitchBetweenBasicAdvanced",
        err,
      );
    }
  }
  ...
  function AdvancedSettProtected() {
    try {
      ...
      if (webphone_api.common.isNull(pwd) || pwd.length < 1) {
        pwd = webphone_api.common.GetParameter2("advancedsettpwd");
      }
      ...
      var template =
        "" +
        '<div id="advancedsett_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        '<div data-role="header" data-theme="b">' +
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' +
        webphone_api.stringres.get("unlockadvancedsett_title") +
        "</h1>" +
        "</div>" +
        '<div role="main" class="ui-content adialog_content">' +
        "<span>" +
      ...
      webphone_api.$("#adialog_positive").on("click", function (event) {
        webphone_api.common.PutToDebugLog(
          5,
          "EVENT, _settings AdvancedSettProtected ok onclick",
        );
        ...
        webphone_api
          ...
          .on("popupafterclose", function (event) {
            if (
              ...
              webphone_api.common.PutToDebugLog(
                5,
                "EVENT, _settings AdvancedSettProtected ok onclick password match",
              );
              settpwdmatche = true;
              ShowAdvancedSettings();
            } else {
              webphone_api.common.PutToDebugLog(
                5,
                "EVENT, _settings AdvancedSettProtected ok onclick password NOT match: " +
                  textboxval,
              );
              webphone_api.common.ShowToast(
                webphone_api.stringres.get("err_msg_2"),
              );
            }
          });
      });
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_settings: AdvancedSettProtected",
        err,
      );
    }
    ShowAdvancedSettings();
    ...
    try {
      if (filtervisible) {
        ...
        webphone_api
          .$("#btn_back_settings")
          .html(
            "<b>&LT;</b>&nbsp;" + webphone_api.stringres.get("hide_search"),
          );
  ...
  function IsUsernameFixed() {
    try {
      ...
      if (
        (!webphone_api.common.isNull(usr1) &&
          usr1.length > 0 &&
          usr1 !== "USERNAME" &&
          usr1 !== "SIPUSERNAME") ||
        (!webphone_api.common.isNull(usr2) &&
          usr2.length > 0 &&
          usr2 !== "USERNAME" &&
          usr2 !== "SIPUSERNAME") ||
        (!webphone_api.common.isNull(usr3) &&
          usr3.length > 0 &&
          usr3 !== "USERNAME" &&
          usr3 !== "SIPUSERNAME") ||
        (!webphone_api.common.isNull(usr4) &&
          usr4.length > 0 &&
          usr4 !== "USERNAME" &&
          usr4 !== "SIPUSERNAME")
      ) {
  ...
  function IsPasswordFixed() {
    try {
      ...
      if (
        (!webphone_api.common.isNull(pwd1) &&
          pwd1.length > 0 &&
          pwd1 !== "PASSWORD" &&
          pwd1 !== "SIPPASSWORD" &&
          pwd1 !== "SIPASSWORD") ||
        (!webphone_api.common.isNull(pwd2) &&
          pwd2.length > 0 &&
          pwd2 !== "PASSWORD" &&
          pwd2 !== "SIPPASSWORD" &&
          pwd2 !== "SIPASSWORD")
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_settings: onStop", err);
    }
  }
