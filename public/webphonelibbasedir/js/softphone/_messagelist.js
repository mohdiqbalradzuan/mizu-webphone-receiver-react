webphone_api._messagelist = (function () {
  function onCreate(event) {
    ...
    try {
      ...
      {
        if (
          webphone_api.$.mobile.activePage.attr("id") === "page_messagelist"
        ) {
          MeasureMessagelist();
        }
      });
  ...
  function onStart(event) {
    try {
      ...
      webphone_api
        .$("#btn_newmessage")
        .html(webphone_api.stringres.get("btn_new_message"));
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#page_messagelist"),
            -30,
          ),
        );

      if (
        !webphone_api.common.isNull(document.getElementById("msglist_title"))
      ) {
        document.getElementById("msglist_title").innerHTML =
          webphone_api.stringres.get("msglist_title");
      }
      webphone_api
        .$("#msglist_title")
      ...
      if (
        !webphone_api.common.isNull(document.getElementById("msglist_btnback"))
      ) {
        document.getElementById("msglist_btnback").innerHTML =
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_messagelist: onStart",
        err,
      );
    }
  }

  function MeasureMessagelist() {
    ...
    try {
      ...
      var notwidth =
        ...
        webphone_api.$("#messagelist_additional_header_left").width() -
        webphone_api.$("#messagelist_additional_header_right").width();
      var margin = webphone_api.common.StrToIntPx(
        webphone_api
          .$("#messagelist_additional_header_left")
          .css("margin-left"),
      );
      ...
      var heightTemp =
        ...
        webphone_api.$("#messagelist_header").height() -
        webphone_api.$("#btn_newmessage_container").height();
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_messagelist: MeasureMessagelist",
        err,
      );
    }
  }
  ...
  function PopulateList() {
    ...
    try {
      if (
        webphone_api.common.isNull(document.getElementById("messagelist_list"))
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _messagelist: PopulateList listelement is null",
        );
        return;
      }
      ...
      if (webphone_api.common.isNull(files) || files.length < 3) {
        webphone_api.common.PutToDebugLog(
          3,
          "EVENT, _messagelist: PopulateList no message files",
        );
        return;
      }
      ...
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _messagelist Starting populate list",
      );
      ...
      for (var i = 0; i < msglist.length; i++) {
        ...
        if (pos > 0) {
          ...
          try {
            nrMissed = webphone_api.common.StrToInt(
              webphone_api.common.Trim(tmp),
            );
          } catch (errin) {}
        }
        ...
        if (webphone_api.common.isNull(name) || name.length < 1) {
          name = number;
        }
        ...
        var listitem =
          ...
          '<li id="msgitem_' +
          i +
          '"><a class="msg_anchor mlistitem" data-transition="slide">' +
          '<div class="msg_item_container">' +
          '<div class="msg_name">' +
          name +
          ' - <span id="msgitemnumber_' +
          i +
          '">' +
          number +
          "</span>" +
          ...
          i +
          '" class="msg_type">' +
          webphone_api.stringres.get(type) +
          "</div>" +
          "</div>" +
          "</a></li>";
  ...
  function OnListItemClick(id) {
    ...
    try {
      if (webphone_api.common.isNull(id) || id.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _messagelist OnListItemClick id is NULL",
        );
        return;
      }
      ...
      if (pos < 2) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _messagelist OnListItemClick invalid id",
        );
        return;
      }
      ...
      if (webphone_api.common.isNull(to)) {
        to = "";
      } else {
        to = webphone_api.common.Trim(to);
      }

      var typestr = webphone_api.$("#msgtype_" + msgid).html();
      if (webphone_api.common.isNull(typestr)) {
        typestr = webphone_api.stringres.get("chat");
      } else {
        typestr = webphone_api.common.Trim(typestr);
      }
      ...
      webphone_api.common.PutToDebugLog(
        4,
        "EVENT, _messagelist: OnListItemClick: action: " +
          action +
          "; to: " +
          to,
      );
      ...
      if (
        ...
        webphone_api.$.mobile.changePage("#page_message", {
          transition: "none",
          role: "page",
        });
      } else {
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _messagelist: CreateOptionsMenu menuid null",
        );
        return;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _messagelist: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_MESSAGELIST_NEWMESSAGE +
            '"><a data-rel="back">' +
            webphone_api.stringres.get("btn_new_message") +
            "</a></li>",
        )
        .listview("refresh");
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_messagelist: CreateOptionsMenu",
        err,
      );
    }
  ...
  function ClearAllHistory(popupafterclose) {
    try {
      ...
      var template =
        ...
        "<span> " +
        webphone_api.stringres.get("delete_all_msg_alert") +
        " </span>" +
        ...
        for (var i = 0; i < msglist.length; i++) {
          ...
          (function () {
            webphone_api.File.DeleteFile(cfile, function (success) {
              webphone_api.common.PutToDebugLog(
                3,
                "EVENT, _messagelist: ClearAllHistory DeleteFile: " +
                  cfile +
                  " status: " +
                  success.toString(),
              );
