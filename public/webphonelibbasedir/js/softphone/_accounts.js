...
webphone_api._accounts = (function () {
  function onCreate(event) {
    ...
    try {
      ...
      webphone_api.$("#btn_accounts_menu").on("click", function () {
        CreateOptionsMenu("#accounts_menu_ul");
      });
      webphone_api
        .$("#btn_accounts_menu")
      ...
      webphone_api
        .$("#btn_add_acc")
        .attr("title", webphone_api.stringres.get("accounts_add_hint"));

      webphone_api.$("#page_accounts").keyup(function (event) {
        HandleKeyUp(event);
      });
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_accounts: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api.$("#btn_add_acc").on("click", function () {
        AddAccount();
      });

      webphone_api
        .$("#acc_add_p")
        .html(webphone_api.stringres.get("add_account"));
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#page_accounts"),
            -30,
          ),
        );
      ...
      if (
        ...
        webphone_api.global.sipaccounts.length > 0
      ) {
        // get active account
        var activeacc_name = "";
        for (var i = 0; i < webphone_api.global.sipaccounts.length; i++) {
          ...
          if (sipitem.ismain === true) {
            ...
            if (
              !webphone_api.common.isNull(sipitem.serveraddress) &&
              sipitem.serveraddress.length > 0
            ) {
        ...
        if (
          !webphone_api.common.isNull(activeacc_name) &&
          activeacc_name.length > 0
        ) {
          mainAcc =
            webphone_api.stringres.get("active_account") +
            " " +
            activeacc_name +
            ". ";
        }
      ...
      var textInstr =
        webphone_api.stringres.get("accounts_instructions1") +
        " " +
        mainAcc +
        webphone_api.stringres.get("accounts_instructions2");
      ...
      document.getElementById("accounts_title").innerHTML =
        webphone_api.stringres.get("accounts_title");
      webphone_api
        .$("#accounts_title")
      ...
      webphone_api.common.PutToDebugLogException(2, "_accounts: onStart", err);
    }
  }

  function MeasureAccountst() {
  ...
  var atemplate =
    ...
    '<input type="checkbox" class="acc_checkbox" id="acc_checkbox_[NR]" title="' +
    webphone_api.stringres.get("accounts_endis_hint") +
    '" name="acc_checkbox_[NR]" [ENABLE_CHECKED]>' +
    '<input type="radio" class="acc_radio" id="acc_radio_[NR]" title="' +
    webphone_api.stringres.get("accounts_main_hint") +
    '" name="acc_active_selection" [MAIN_CHECKED]>' +
    "</div>" +
    '<div class="acc_username" id="acc_username_[NR]">[USERNAME]</div>' +
    '<div class="acc_remove"><button id="btn_remove_account_[NR]" title="' +
    webphone_api.stringres.get("accounts_remove_hint") +
    '" class="btn_remove_account noshadow ui-btn-inline ui-btn ui-btn-corner-all ui-btn-b ui-icon-minus ui-btn-icon-notext">Remove</button></div>' +
  ...
  function PopulateData() {
    try {
      ...
      if (
        ...
        webphone_api
          .$("#acc_list")
          .html("<br /><br />" + webphone_api.stringres.get("noaccounts"));
        return;
      }

      for (var i = 0; i < webphone_api.global.sipaccounts.length; i++) {
        var sipitem = webphone_api.global.sipaccounts[i];
        var htmlitem = webphone_api.common.ReplaceAll(
          atemplate,
          "[NR]",
          i.toString(),
        );
      ...
      webphone_api.$(".btn_remove_account").on("click", function (e) {
        try {
          ...
          if (!webphone_api.common.isNull(id) && id.lastIndexOf("_") > 0) {
            ...
            if (
              !webphone_api.common.isNull(idxstr) &&
              webphone_api.common.IsNumber(idxstr) === true
            ) {
            ...
            if (
              ...
              if (!anychecked) {
                if (webphone_api.global.sipaccounts.length > 1) {
                  for (
                    var i = 0;
                    i < webphone_api.global.sipaccounts.length;
                    i++
                  ) {
                    if (i !== idx) {
                      webphone_api.global.sipaccounts[i].enabled = true;
                      ...
                      for (
                        var j = 0;
                        j < webphone_api.global.sipaccounts.length;
                        j++
                      ) {
                        webphone_api.global.sipaccounts[j].ismain = false;
                      }
                ...
                if (
                  checked === false &&
                  webphone_api.global.sipaccounts[idx].ismain === true
                ) {
                  ...
                  for (
                    ...
                    if (
                      i === idx ||
                      webphone_api.global.sipaccounts[i].enabled === false
                    ) {
                      continue;
                    }
      ...
      webphone_api.$(".acc_radio").change(function () {
        try {
          for (var i = 0; i < webphone_api.global.sipaccounts.length; i++) {
            if (webphone_api.$("#acc_radio_" + i).is(":checked")) {
              // reset ismain
              for (var j = 0; j < webphone_api.global.sipaccounts.length; j++) {
                webphone_api.global.sipaccounts[j].ismain = false;
              }
  ...
  function DeleteAccountPopup(idx, popupafterclose) {
    try {
      ...
      var template =
        "" +
        '<div id="adialog_deleteaccount" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        ...
        "<span> " +
        webphone_api.stringres.get("accounts_remove_msg") +
        ": " +
        acc_username +
        " </span>" +
      ...
      webphone_api.$("#btn_adialog_ok").on("click", function () {
        try {
          ...
          webphone_api.common.PutToDebugLog(
            2,
            "EVENT, _accounts, account removed: " + idx,
          );
          ...
          webphone_api.common.PutToDebugLogException(
            2,
            "_accounts: DeleteAccountPopup inner OK",
            err,
          );
        }
      });
  ...
  function AddAccount(popupafterclose) {
    try {
      ...
      var accounts_hint_server = webphone_api.stringres.get(
        "accounts_hint_server",
      );
      var accounts_hint_user = webphone_api.stringres.get("accounts_hint_user");
      var accounts_hint_password = webphone_api.stringres.get(
        "accounts_hint_password",
      );
      var accounts_hint_callerid = webphone_api.stringres.get(
        "accounts_hint_callerid",
      );
      var accounts_hint_displayname = webphone_api.stringres.get(
        "accounts_hint_displayname",
      );
      var accounts_hint_proxy = webphone_api.stringres.get(
        "accounts_hint_proxy",
      );
      var accounts_hint_realm = webphone_api.stringres.get(
        "accounts_hint_realm",
      );
      var accounts_hint_regival = webphone_api.stringres.get(
        "accounts_hint_regival",
      );
      ...
      if (webphone_api.common.ShowServerInput() === true) {
        ...
        var val = "";
        if (!webphone_api.common.isNull(srv) && srv.length > 0) {
          val = 'value="' + srv + '"';
        }

        serverHtml =
          '<label for="addacc_serveraddress">' +
          webphone_api.stringres.get("addacc_server") +
          ":</label>" +
      ...
      var template =
        "" +
        '<div id="addaccountpopup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        '<div data-role="header" data-theme="b">' +
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' +
        webphone_api.stringres.get("addaccount_title") +
        "</h1>" +
        "</div>" +
        ...
        "<span>" +
        webphone_api.stringres.get("addaccount_msg") +
        "</span>" +
        serverHtml +
        '<label for="addacc_username">' +
        webphone_api.stringres.get("addacc_user") +
        ":</label>" +
        '<input type="text" id="addacc_username" name="addacc_username" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_user +
        '" />' +
        '<label for="addacc_password">' +
        webphone_api.stringres.get("addacc_password") +
        ":</label>" +
        '<input type="text" id="addacc_password" name="addacc_password" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_password +
        '" />' +
        "<br />" +
        '<a href="javascript:;" id="show_hide_advanced">' +
        webphone_api.stringres.get("ac_showadvanced") +
        "</a>" +
        "<br />" +
        "<br />" +
        '<div id="addaccountpopup_advanced" style="display: none; margin-left: 2em;">' +
        '<label for="addacc_callerid">' +
        webphone_api.stringres.get("sett_display_name_username") +
        ":</label>" +
        '<input type="text" id="addacc_callerid" name="addacc_callerid" value="" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_callerid +
        '" />' +
        '<label for="addacc_displayname">' +
        webphone_api.stringres.get("sett_display_name_displayname") +
        ":</label>" +
        '<input type="text" id="addacc_displayname" name="addacc_displayname" value="" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_displayname +
        '" />' +
        '<label for="addacc_proxyaddress">' +
        webphone_api.stringres.get("sett_display_name_proxyaddress") +
        ":</label>" +
        '<input type="text" id="addacc_proxyaddress" name="addacc_proxyaddress" value="" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_proxy +
        '" />' +
        '<label for="addacc_realm">' +
        webphone_api.stringres.get("sett_display_name_realm") +
        ":</label>" +
        '<input type="text" id="addacc_realm" name="addacc_realm" value="" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_realm +
        '" />' +
        '<br /><label for="addacc_ival">' +
        webphone_api.stringres.get("addacc_ival") +
        ":</label>" +
        '<input type="text" id="addacc_ival" name="addacc_ival" value="3600" data-theme="a" autocapitalize="off" placeholder="' +
        accounts_hint_regival +
        '" />' +
        "</div>" +
        "</div>" +
      ...
      webphone_api.$("#adialog_positive").on("click", function (event) {
        ...
        webphone_api
          ...
          .on("popupafterclose", function (event) {
            ...
            webphone_api.common.PutToDebugLog(
              5,
              "EVENT, accounts AddAccount ok onclick",
            );
            ManuallyClosePopup(
            ...
            if (webphone_api.common.ShowServerInput() === true) {
              if (webphone_api.common.isNull(srv) || srv.length < 1) {
                webphone_api.common.ShowToast(
                  webphone_api.stringres.get("addacc_invalid") +
                    " " +
                    webphone_api.stringres.get("addacc_server"),
                );
                return;
              ...
              if (webphone_api.common.isNull(srv) || srv.length < 1) {
                srv = webphone_api.common.GetParameter("serveraddress_user");
              }
            }
            ...
            if (
              webphone_api.common.isNull(ivalstr) ||
              ivalstr.length < 1 ||
              !webphone_api.common.IsNumber(ivalstr)
            ) {
              webphone_api.common.ShowToast(
                webphone_api.stringres.get("addacc_invalid") +
                  " " +
            ...
            if (webphone_api.common.isNull(callerid)) {
              callerid = "";
            }
            if (webphone_api.common.isNull(displayname)) {
              displayname = "";
            }
            if (webphone_api.common.isNull(proxyaddress)) {
              proxyaddress = "";
            }
            if (webphone_api.common.isNull(realm)) {
              realm = "";
            }
            ...
            if (
              webphone_api.common.isNull(ivalstr) ||
              webphone_api.common.IsNumber(ivalstr) == false
            ) {
              ivalstr = "3600";
            }
      ...
      webphone_api.$("#show_hide_advanced").on("click", function (event) {
        if (webphone_api.$("#addaccountpopup_advanced").is(":visible")) {
          ...
          webphone_api
            .$("#show_hide_advanced")
            .html(webphone_api.stringres.get("ac_showadvanced"));
          ...
          if (
            webphone_api.$(".messagePopup .ui-content").height() >
            maxHeight + 100
          ) {
            webphone_api.$(".messagePopup .ui-content").height(maxHeight);
          }
          ...
          webphone_api
            .$("#show_hide_advanced")
            .html(webphone_api.stringres.get("ac_hideadvanced"));
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _accounts: CreateOptionsMenu menuid null",
        );
        return;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _accounts: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_accounts: onStop", err);
    }
  }
