...
*/
  ...
  function onCreate(event) {
    try {
      ...
      webphone_api.$("#btn_extra2_menu").on("click", function () {
        CreateOptionsMenu("#extra2_menu_ul");
      });

      // set page title
      webphone_api.$("#extra2_title").html("Page Extra 2");
    } catch (err) {
      webphone_api.common.PutToDebugLogException(2, "_extra2: onCreate", err);
    }
  }
  ...
  function onStart(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra2: onStart", err);
    }
  }
  ...
  function MeasureExtra2() {
    ...
    var contentHeight =
      ...
      webphone_api.$("#extra2_header").height() -
      3;
    ...
    try {
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra2: CreateOptionsMenu menuid null",
        );
        return;
      }
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _extra2: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
 ...
 */
  function MenuItemSelected(itemid) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_extra2: MenuItemSelected",
        err,
      );
    }
  }

  function MenuActionGoBack() {
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(2, "_extra2: onStop", err);
    }
  }
