...
webphone_api._contactslist = (function () {
  function onCreate(event) {
    ...
    try {
      ...
      {
        if (
          webphone_api.$.mobile.activePage.attr("id") === "page_contactslist"
        ) {
          MeasureContacslist();
        }
      });
      ...
      webphone_api
        .$("#contactslist_menu_ul")
        .on("click", "li", function (event) {
          MenuItemSelected(webphone_api.$(this).attr("id"));
        });
      webphone_api.$("#btn_contactslist_menu").on("click", function () {
        CreateOptionsMenu("#contactslist_menu_ul");
      });
      webphone_api
        .$("#btn_contactslist_menu")
      ...
      if (
        webphone_api.common.GetParameterBool("contacttoggle", false) === true
      ) {
      ...
      if (webphone_api.common.UsePresence2() === true) {
        webphone_api
          .$("#contactslist_additional_header_left")
          .on("click", function () {
  ...
  function onStart(event) {
    try {
      ...
      setTimeout(
        function () // otherwise keyup will not work, because the element is not focused
        {
      ...
      if (
        ...
        document.getElementById("app_name_contactslist").innerHTML =
      ...
      webphone_api
        ...
        .attr(
          ...
          webphone_api.stringres.get("ct_search_hint"),
        );

      if (
        !webphone_api.common.isNull(document.getElementById("ctlist_title"))
      ) {
        document.getElementById("ctlist_title").innerHTML =
          webphone_api.stringres.get("ctlist_title");
      }
      webphone_api
        .$("#ctlist_title")
      ...
      if (!webphone_api.common.isNull(curruser) && curruser.length > 0) {
        webphone_api.$("#curr_user_contactslist").html(curruser);
      }
      // set status width so it's uses all space to curr_user
      var statwidth =
        webphone_api.common.GetDeviceWidth() -
        webphone_api.$("#curr_user_contactslist").width() -
        25;
      if (
        !webphone_api.common.isNull(statwidth) &&
        webphone_api.common.IsNumber(statwidth)
      ) {
      ...
      webphone_api
        ...
        .css(
          ...
          webphone_api.common.HoverCalc(
            webphone_api.common.getBgColor("#page_contactslist"),
            -30,
          ),
        );
      ...
      webphone_api
        .$("#contactslist_notification_list")
      ...
      if (webphone_api.common.GetColortheme() === 22) {
        webphone_api
          .$("#nav_ct_dialpad IMG")
          .attr("src", "images/tab_dialpad_grey.png");
        webphone_api
          .$("#nav_ct_contacts IMG")
          .attr("src", "images/tab_contacts_blue.png");
        webphone_api
          .$("#nav_ct_callhistory IMG")
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: onStart",
        err,
      );
    }
  }

  function MeasureContacslist() {
    ...
    try {
      ...
      var notwidth =
        webphone_api.common.GetDeviceWidth() -
        webphone_api.$("#contactslist_additional_header_left").width() -
        webphone_api.$("#contactslist_additional_header_right").width();
      ...
      webphone_api
        ...
        .height(
          Math.floor(
            webphone_api.$("#contactslist_additional_header_left").height(),
          ),
        );
      ...
      var heightTemp =
        ...
        webphone_api
          .$("#contactslist_header")
          .height() /* - webphone_api.$("form.ui-filterable").height()*/ -
        webphone_api.common.StrToIntPx(
          webphone_api.$(".ui-input-search").css("border-top-width"),
        ) -
        webphone_api.common.StrToIntPx(
          webphone_api.$(".ui-input-search").css("border-bottom-width"),
        );
      ...
      var searchmargin = webphone_api.common.StrToIntPx(
        webphone_api.$(".ui-input-search").css("margin-top"),
      );
      heightTemp = heightTemp - searchmargin - searchmargin;
      heightTemp = heightTemp - 3;

      if (webphone_api.$("#footertext_contactslist").is(":visible")) {
        heightTemp =
          heightTemp - webphone_api.$("#footertext_contactslist").height();
      }

      if (webphone_api.$("#advert_contactslist").is(":visible")) {
        heightTemp =
          heightTemp - webphone_api.$("#advert_contactslist").height();
      }

      if (webphone_api.$("#togglecontact_container").is(":visible")) {
        heightTemp =
          heightTemp - webphone_api.$("#togglecontact_container").height();
      }
      ...
      if (
        ...
        webphone_api.common.GetBrandName().length >
      ...
      var brandW =
        webphone_api.$("#contactslist_additional_header_left").width() - 5;
      if (webphone_api.$("#contactslist_presence").is(":visible")) {
        brandW = brandW - webphone_api.$("#contactslist_presence").width();
      }
      webphone_api.$("#app_name_contactslist").width(brandW);
    } catch (err) {
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: MeasureContacslist",
        err,
      );
    }
  }
  ...
  function LoadContacts() {
    try {
      ...
      if (
        ...
        webphone_api.global.ctlist.length < 1
      ) {
        ...
        webphone_api.common.GetContacts(function (success) {
          if (!success) {
            webphone_api.common.PutToDebugLog(
              2,
              "ERROR, _contactslist: LoadContacts failed",
            );
          }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: LoadContacts",
        err,
      );
    }
  }

  function AddDefContacts() {
    // add custom(def) contacts to contactslist and reload contacts list
    try {
      ...
      if (!webphone_api.common.isNull(webphone_api.parameters["defcontacts"])) {
        defct = webphone_api.parameters["defcontacts"];
      }
      if (!webphone_api.common.isNull(defct) && defct.length > 2) {
        ...
        if (!webphone_api.common.isNull(darr)) {
          for (var i = 0; i < darr.length; i++) {
            ...
            if (webphone_api.common.isNull(onect) || onect.length < 2) {
              continue;
            }
            ...
            if (
              ...
              webphone_api.common.PutToDebugLog(
                3,
                "ERROR, plhandler AddContact invalid number: " + number,
              );
              return false;
            }
            number = webphone_api.common.Trim(number);
            if (webphone_api.common.isNull(fullname)) {
              fullname = "";
            }
            ...
            var norm = webphone_api.common.GetParameterInt(
              "normalize_contact",
              1,
            );

            if (norm > 0) {
              fullname = webphone_api.common.NormalizeInput2(fullname, 2);
            }

            if (webphone_api.common.isNull(email)) {
              email = "";
            }
            if (norm > 0) {
              email = webphone_api.common.NormalizeInput2(email, 3);
              email = webphone_api.common.Trim(email);
            }

            if (webphone_api.common.isNull(address)) {
              address = "";
            }
            if (norm > 0) {
              address = webphone_api.common.NormalizeInput2(address, 3);
              address = webphone_api.common.Trim(address);
            }

            if (webphone_api.common.isNull(notes)) {
              notes = "";
            }
            if (norm > 0) {
              notes = webphone_api.common.NormalizeInput2(notes, 3);
              notes = webphone_api.common.Trim(notes);
            }

            if (webphone_api.common.isNull(website)) {
              website = "";
            }
            if (norm > 0) {
              website = webphone_api.common.NormalizeInput2(website, 2);
              website = webphone_api.common.Trim(website);
            }

            var type = "sip";
            if (webphone_api.common.IsNumber(number)) {
              type = "phone";
            }
            ...
            webphone_api.plhandler.SaveTheContact(
              fullname,
              number,
              "0",
              type,
  ...
  function PopulateList(onlyserver) {
    // :no return value;  // onlyonserver: display only contacts on server. Controlled by toggle button
    try {
      if (
        webphone_api.common.isNull(document.getElementById("contactslist_list"))
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist: PopulateList listelement is null",
        );
        return;
      }
      ...
      if (
        ...
        var htmlcontent =
          '<span style="text-shadow:0 0 0;">' +
          webphone_api.stringres.get("no_contacts_1") +
          "</span>";

        // display sync contacts button
        if (
          webphone_api.common.GetParameterInt("showsynccontactsmenu", 0) === 0
        ) {
          htmlcontent =
            htmlcontent +
            "<br /><br /><br />" +
            '<span style="text-shadow:0 0 0;">' +
            webphone_api.stringres.get("no_contacts_2") +
            "</span>";
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "EVENT, _contactslist: PopulateList no contacts",
        );
        return;
      }
      ...
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _contactslist Starting populate list",
      );
      ...
      if (onlyserver) {
        ...
        if (
          webphone_api.common.isNull(servercontacts) ||
          servercontacts.length < 1
        ) {
          return;
        }
        ...
        if (webphone_api.common.isNull(serverctL) || serverctL.length < 1) {
          ...
          for (var i = 0; i < webphone_api.global.ctlist.length; i++) {
            ...
            if (webphone_api.common.isNull(nrtmp) || nrtmp.length < 1) {
              continue;
            }
      ...
      } // display all contacts, not only server contacts
      else {
        // copy array
      ...
      var template =
        '<li id="contact_[ID]"><a data-transition="slide" class="mlistitem">[NAME]</a></li>';
      var listview = "";
      ...
      for (var i = 0; i < ctlistLocal.length; i++) {
        ...
        if (webphone_api.common.isNull(ctname) || ctname.length < 1) {
          ...
          if (
            webphone_api.common.isNull(nrlist) ||
            nrlist.length < 1 ||
            webphone_api.common.isNull(nrlist[0]) ||
            nrlist[0].length < 1
          ) {
            continue;
          }
  ...
  function OnListItemClick(id) {
    ...
    try {
      if (webphone_api.common.isNull(id) || id.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist OnListItemClick id is NULL",
        );
        return;
      }
      ...
      if (pos < 2) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist OnListItemClick invalid id",
        );
        return;
      }
        ...
        if (webphone_api.common.isNull(nrlist) || nrlist.length < 1) {
          webphone_api.common.SetVoiceMailNumber(function (vmnr) {
            if (!webphone_api.common.isNull(vmnr) && vmnr.length > 0) {
              ...
              for (var i = 0; i < webphone_api.global.ctlist.length; i++) {
                if (
                  webphone_api.global.ctlist[i][webphone_api.common.CT_NAME] ===
                  "Voicemail"
                ) {
              ...
              ctTemp[webphone_api.common.CT_NAME] =
                ctlistLocal[ctid][webphone_api.common.CT_NAME];
              ctTemp[webphone_api.common.CT_NUMBER] = [vmnr];
              ctTemp[webphone_api.common.CT_PTYPE] =
                ctlistLocal[ctid][webphone_api.common.CT_PTYPE];
              ctTemp[webphone_api.common.CT_USAGE] =
                ctlistLocal[ctid][webphone_api.common.CT_USAGE];
              ctTemp[webphone_api.common.CT_LASTMODIF] =
                ctlistLocal[ctid][webphone_api.common.CT_LASTMODIF];
              ctTemp[webphone_api.common.CT_DELFLAG] =
                ctlistLocal[ctid][webphone_api.common.CT_DELFLAG];
              ctTemp[webphone_api.common.CT_FAV] =
                ctlistLocal[ctid][webphone_api.common.CT_FAV];
              ctTemp[webphone_api.common.CT_EMAIL] =
                ctlistLocal[ctid][webphone_api.common.CT_EMAIL];
              ctTemp[webphone_api.common.CT_ADDRESS] =
                ctlistLocal[ctid][webphone_api.common.CT_ADDRESS];
              ctTemp[webphone_api.common.CT_NOTES] =
                ctlistLocal[ctid][webphone_api.common.CT_NOTES];
              ctTemp[webphone_api.common.CT_WEBSITE] =
                ctlistLocal[ctid][webphone_api.common.CT_WEBSITE];
              ctTemp[webphone_api.common.CT_LASTACTIVE] =
                ctlistLocal[ctid][webphone_api.common.CT_LASTACTIVE];
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: OnListItemClick",
        err,
      );
    }
  }

  function OnListItemLongClick(id) {
    // :no return value
    try {
      if (webphone_api.common.isNull(id) || id.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist OnListItemLongClick id is NULL",
        );
        return;
      }
      ...
      if (pos < 2) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist OnListItemLongClick invalid id 1",
        );
        return;
      }
      ...
      ctid = webphone_api.common.Trim(id.substring(pos + 1));
      if (
        webphone_api.common.isNull(ctid) ||
        ctid.length < 1 ||
        !webphone_api.common.IsNumber(ctid)
      ) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist OnListItemLongClick invalid id 2: " + ctid,
        );
        return;
      }
      ...
      for (var i = 0; i < webphone_api.global.ctlist.length; i++) {
        if (
          webphone_api.global.ctlist[i][webphone_api.common.CT_NAME] === name
        ) {
          globalid = i.toString();
          break;
        }
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: OnListItemLongClick",
        err,
      );
    }
  }
  ...
  function CreateContextmenu(ctid, popupafterclose) {
    try {
      ...
      if (
        ...
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist: CreateContextmenu invalid contact id: " + ctid,
        );
      }
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_deletecontact"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      if (
        ...
        (webphone_api.common.GetConfigBool("usingmizuserver", false) === true ||
          webphone_api.common.IsMizuPublicWebRTCGateway() === true)
      ) {
        //OPSSTART
      ...
      var template =
        "" +
        '<div id="ct_contextmenu" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        "px; min-width: " +
        ...
        ctentry[webphone_api.common.CT_NAME] +
        "</h1>" +
        "</div>" +
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: CreateContextmenu",
        err,
      );
    }
  }

  function FileTrasnf(ctidstr) {
    // opens page message with the contact's number
    try {
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _contactslist: FileTrasnf contact id: " + ctidstr,
      );
      ...
      if (webphone_api.common.isNull(numbers) || numbers.length < 1) {
        webphone_api.common.PutToDebugLog(
          1,
          "ERROR," + webphone_api.stringres.get("ct_menu_error"),
        );
        webphone_api.common.ShowToast(
          "ERROR," + webphone_api.stringres.get("ct_menu_error"),
        );
      ...
      if (numbers.length === 1) {
        ...
        webphone_api.common.PickContactNumber(
          ctid,
          function (pick_nr, pick_name) {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: FileTrasnf",
        err,
      );
    }
  }

  function InitiateCall(ctidstr, isvideo) {
    // opens page message with the contact's number
    try {
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _contactslist: InitiateCall contact id: " + ctidstr,
      );
      var ctid = webphone_api.common.StrToInt(ctidstr);
      var contact = webphone_api.global.ctlist[ctid];
      var numbers = contact[webphone_api.common.CT_NUMBER];
      ...
      if (numbers.length === 1) {
        ...
        if (isvideo === true) {
          webphone_api.common.PutToDebugLog(
            4,
            "EVENT, _contactslist initiate video call to: " + number,
          );
          webphone_api.videocall(number);
        } else {
          webphone_api.common.PutToDebugLog(
            4,
            "EVENT, _contactslist initiate call to: " + number,
          );
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: InitiateCall",
        err,
      );
    }
  }

  function SendMessage(ctidstr) {
    // opens page message with the contact's number
    try {
      webphone_api.common.PutToDebugLog(
        2,
        "EVENT, _contactslist: SendMessage contact id: " + ctidstr,
      );
      var ctid = webphone_api.common.StrToInt(ctidstr);
      var contact = webphone_api.global.ctlist[ctid];
      var numbers = contact[webphone_api.common.CT_NUMBER];
  ...
  function DeleteContact(ctid, popupafterclose) {
    try {
      ...
      var template =
        "" +
        '<div id="ct_delete_contact_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        '<div data-role="header" data-theme="b">' +
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        ...
        "<span> " +
        webphone_api.stringres.get("contact_delete_msg") +
        " </span>" +
        //        '<a href="javascript:;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back">' + webphone_api.stringres.get('btn_close') + '</a>' +
        //        '<a href="javascript:;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back" data-transition="flow">Delete</a>' +
        "</div>" +
      ...
      webphone_api.$("#btn_adialog_ok").on("click", function () {
        ...
        webphone_api.common.SaveContactsFile(function (issaved) {
          webphone_api.common.PutToDebugLog(
            4,
            "EVENT, _contactslist: DeleteContact SaveContactsFile: " +
              issaved.toString(),
          );
        });
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.isNull(menuId) || menuId.length < 1) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist: CreateOptionsMenu menuid null",
        );
        return;
      }
      ...
      if (webphone_api.$(menuId).length <= 0) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist: CreateOptionsMenu can't get reference to Menu",
        );
        return;
      }
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_CONTACTSLIST_NEWCT +
            '"><a data-rel="back">' +
            webphone_api.stringres.get("menu_new_contact") +
            "</a></li>",
        )
        .listview("refresh");
      ...
      if (webphone_api.common.IsWindowsSoftphone() === true) {
          ...
          webphone_api
            ...
            .append(
              '<li id="' +
                MENUITEM_SYNC +
                '"><a data-rel="back">' +
                webphone_api.stringres.get("menu_import_list") +
                "...</a></li>",
            )
            .listview("refresh");
        }
        ...
        if (
          featureset > 0 &&
          (webphone_api.common.GetParameterInt("showsynccontactsmenu", 0) ===
            0 ||
            webphone_api.common.GetParameterInt("showsynccontactsmenu", 0) ===
              1)
        ) {
          webphone_api
            ...
            .append(
              '<li id="' +
                MENUITEM_SYNC +
                '"><a data-rel="back">' +
                webphone_api.stringres.get("menu_sync") +
                "</a></li>",
            )
            .listview("refresh");
        }
      ...
      if (
        featureset > 0 &&
        (webphone_api.common.GetParameterInt("showsynccontactsmenu", 0) === 0 ||
          webphone_api.common.GetParameterInt("showsynccontactsmenu", 0) === 1)
      ) {
        webphone_api
          .$(menuId)
          .append(
            '<li id="' +
              MENUITEM_SORTCONTACTS +
              '"><a data-rel="back">' +
              webphone_api.stringres.get("menu_sortct") +
              "</a></li>",
          )
          .listview("refresh");
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_DELETEALLCONTACTS +
            '"><a data-rel="back">' +
            webphone_api.stringres.get("menu_delallct") +
            "</a></li>",
        )
        .listview("refresh");
      ...
      webphone_api
        ...
        .append(
          '<li id="' +
            MENUITEM_CONTACTSLIST_SETTINGS +
            '"><a data-rel="back">' +
      ...
      if (
        ...
        webphone_api
          ...
          .append(
            '<li id="' +
              MENUITEM_CONTACTS_CUSTOM_LINK2 +
              '"><a data-rel="back">' +
              linktext2 +
              "</a></li>",
          )
      ...
      if (
        ...
        webphone_api
          ...
          .append(
            '<li id="' +
              MENUITEM_CONTACTS_CUSTOM_LINK3 +
              '"><a data-rel="back">' +
              linktext3 +
              "</a></li>",
          )
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: CreateOptionsMenu",
        err,
      );
    }
  ...
  function MenuItemSelected(itemid) {
    try {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: MenuItemSelected",
        err,
      );
    }
  }
  ...
  function DeleteAllContacts(popupafterclose) {
    try {
      ...
      var template =
        "" +
        '<div id="delete_allcontacts_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        'px;">' +
        '<div data-role="header" data-theme="b">' +
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        ...
        '<div role="main" class="ui-content adialog_content adialog_alert">' +
        "<span> " +
        webphone_api.stringres.get("contact_delete_all_msg") +
        " </span>" +
        //        '<a href="javascript:;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back">' + webphone_api.stringres.get('btn_close') + '</a>' +
        //        '<a href="javascript:;" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back" data-transition="flow">Delete</a>' +
        "</div>" +
      ...
      webphone_api.$("#btn_adialog_ok").on("click", function () {
        webphone_api.common.PutToDebugLog(
          2,
          "EVENT, _contactslist: DeleteAllContacts OK clicked",
        );
        ...
        webphone_api.common.SaveContactsFile(function (issaved) {
          webphone_api.common.PutToDebugLog(
            4,
            "EVENT, _contactslist: DeleteAllContacts SaveContactsFile: " +
              issaved.toString(),
          );
        });
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: DeleteAllContacts",
        err,
      );
    }
  }
  ...
  function FilterContactsByPopup(popupafterclose) {
    try {
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ct_filter_all"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ct_filter_important"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ct_filter_online"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      var template =
        "" +
        '<div id="filterct_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        "px; min-width: " +
        Math.floor(popupWidth * 0.6) +
        'px;">' +
        ...
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' +
        webphone_api.stringres.get("ct_filter_title") +
        "</h1>" +
        "</div>" +
        '<div role="main" class="ui-content adialog_content" style="padding: 0; margin: 0;">' +
      ...
      webphone_api.$("#filterct_ul").on("click", "li", function (event) {
        ...
        if (itemid === "#item_filter_all") {
          webphone_api.common.SaveParameter(
            "lastfilterval",
            webphone_api.common.GetParameter("filtercontacts"),
          );
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: FilterContactsByPopup",
        err,
      );
    }
  }
  ...
  function SortContactsPopup(popupafterclose) {
    try {
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ct_sort_name"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ct_sort_importance"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("ct_sort_status"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      var template =
        "" +
        '<div id="sortct_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        "px; min-width: " +
        Math.floor(popupWidth * 0.6) +
        'px;">' +
        ...
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' +
        webphone_api.stringres.get("ct_sort_title") +
        "</h1>" +
        "</div>" +
        '<div role="main" class="ui-content adialog_content" style="padding: 0; margin: 0;">' +
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: SortContactsPopup",
        err,
      );
    }
  }
  ...
  function ImportContactsPopup(popupafterclose) {
    try {
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_import_file"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_import_outlook"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_import_skype"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_import_google"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_import_android"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      itemTemp = itemTemp.replace(
        "[ITEMTITLE]",
        webphone_api.stringres.get("menu_export_file"),
      );
      list = list + itemTemp;
      itemTemp = "";
      ...
      var template =
        "" +
        '<div id="import_contacts_window" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' +
        popupWidth +
        "px; min-width: " +
        Math.floor(popupWidth * 0.6) +
        'px;">' +
      ...
      webphone_api
        .$("#import_contacts_window_ul")
        .on("click", "li", function (event) {
          var itemid = webphone_api.$(this).attr("id");

          webphone_api
            .$("#import_contacts_window")
            .on("popupafterclose", function (event) {
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_contactslist: ImportContactsPopup",
        err,
      );
    }
  }
  ...
  function ImportContacts(id) {
    try {
      if (webphone_api.common.isNull(id) || id.length < 3) {
        webphone_api.common.PutToDebugLog(
          2,
          "ERROR, _contactslist ImportContacts invalid entry: " + id,
        );
        return;
      }
      ...
      if (id === "#item_import_android") {
        ...
        webphone_api.common.WinSoftphoneHttpReq(
          ...
          function (resp) {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, Import contacts from file: " + resp,
            );
          },
        );
      } else if (id === "#item_export_file") {
        // file export
        var url =
          webphone_api.common.AddJscommport(
            webphone_api.global.WIN_SOFTPHONE_URL,
        ...
        webphone_api.common.WinSoftphoneHttpReq(
          ...
          function (resp) {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, Export contacts to file: " + resp,
            );
          },
        );
      } else if (id === "#item_import_outlook") {
        // outlook import
        var url =
          webphone_api.common.AddJscommport(
            webphone_api.global.WIN_SOFTPHONE_URL,
          ) + "?extcmd_import_outlook";
        webphone_api.common.WinSoftphoneHttpReq(
          url,
          "GET",
          "",
          ...
          function (resp) {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, Import contacts from outlook: " + resp,
            );
          },
        );
      } else if (id === "#item_import_skype") {
        // skype import
        var url =
          webphone_api.common.AddJscommport(
            webphone_api.global.WIN_SOFTPHONE_URL,
        ...
        webphone_api.common.WinSoftphoneHttpReq(
          ...
          function (resp) {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, Import contacts from skype: " + resp,
            );
          },
        );
      } else if (id === "#item_import_skype") {
        ...
        webphone_api.common.WinSoftphoneHttpReq(
          ...
          function (resp) {
            webphone_api.common.PutToDebugLog(
              2,
              "EVENT, Import contacts from google: " + resp,
            );
          },
        );
      }
  ...
  function onStop(event) {
    try {
      ...
      webphone_api.common.SaveContactsFile(function (issaved) {
        webphone_api.common.PutToDebugLog(
          4,
          "EVENT, _contactslist: onDestroy SaveContactsFile: " +
            issaved.toString(),
        );
      });
    } catch (err) {
