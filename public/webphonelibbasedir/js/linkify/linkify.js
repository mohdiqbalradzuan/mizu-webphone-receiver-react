...
var linkify = (function () {
  ...
  function Convert() {
    try {
      ...
      for (var i = elementCount; i >= 0; i--) {
        if (common.isNull(dom[i])) {
          continue;
        }
        if (dom[i].nodeName.toUpperCase() === "BODY") {
          break;
        }

        if (
          common.isNull(dom[i].nodeName) ||
          dom[i].nodeName.toUpperCase() === "BR" ||
          dom[i].nodeName.toUpperCase() === "IMG" ||
        ...
        while (j < content.length) {
          ...
          if (common.IsNumber(content.charAt(j))) {
            ...
            if (
              j > 1 &&
              (content.charAt(j - 2) === "+" || content.charAt(j - 2) === "(")
            ) {
            ...
            for (var k = j; k < limit; k++) {
              if (common.IsChar(content.charAt(k))) {
                break;
              }
            ...
            if (isPhoneNumber > 7) {
              ...
              if (!wasAlreadyReplaced) {
                ...
                var linked =
                  '<a href="javascript:;" title="Click to call" onclick="linkify.GoToWebphone(\'' +
                  replaceval +
                  "');\">" +
                  replaceval +
                  "</a>";
      ...
      common.PutToDebugLogException(2, "linkify Convert", err);
    }
  }
  ...
  var common = {
    PutToDebugLogException: function (level, msg, err) {
      common.PutToDebugLog(level, msg + " " + err);
    },
    PutToDebugLog: function (level, msg) {
      log(msg);
    },
    isNull: function (variable) {
      try {
        ...
        common.PutToDebugLogException(2, "common: isNull", err);
      }
    ...
    Trim: function (str) {
      try {
        ...
        common.PutToDebugLogException(2, "common: Trim", err);
      }
    ...
    IsNumber: function (value) {
      try {
        ...
        common.PutToDebugLogException(2, "common: IsNumber", err);
      }

      return false;
    },
    IsChar: function (
      value, // !!! IMPORTANT used only for finding phone numbers
    ) {
      try {
        if (common.isNull(value)) {
          return false;
        ...
        common.PutToDebugLogException(2, "common: IsChar", err);
      }
    ...
    EscapeRegExp: function (str) {
      try {
        ...
        common.PutToDebugLogException(2, "common: EscapeRegExp", err);
      }
    ...
    ReplaceAll: function (str, what, value) {
      try {
        ...
        common.PutToDebugLogException(2, "common: ReplaceAll", err);
      }
      return "";
    },
  };
  ...
  return {
    ...
    GoToWebphone: function (number) {
      try {
        var url =
          linkify.webphone_url +
          "?wp_serveraddress=" +
          linkify.serveraddress +
          "&wp_username=" +
          linkify.username +
          "&wp_password=" +
          linkify.password +
        ...
        window.open(
          url,
          "_blank",
          "scrollbars=yes, resizable=yes, top=100, left=200, width=400, height=500",
        );
      } catch (err) {
        common.PutToDebugLogException(2, "common: GoToWebphone", err);
      }
