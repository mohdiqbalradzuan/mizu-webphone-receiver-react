...
webphone_api.onCallStateChange(
  function (status, direction, peername, peerdisplayname, line) {
    var lastoop = 0;
    try {
      if (status === "setup") {
        lastoop = 1;
      ...
      PutToDebugLogException(
        2,
        "click2call: onCallStateChange (" + lastoop.toString() + ")",
        err,
      );
    }
  },
...
function Click2Call(nr, isvideo) {
  ...
  if (save_completed === false) {
    ...
    if (isNull(calltonr) || calltonr.length < 1) {
      if (isNull(serveraddress) || serveraddress.length < 3) {
        alert(
          'VoIP serveraddress is not set.\n It can be set in "click2call.html"',
        );
        return;
      }
      if (isNull(username) || username.length < 3) {
        alert(
          'VoIP accounts username is not set.\n It can be set in "click2call.html"',
        );
        return;
      }
      if (ELIMIT < 2 && (isNull(callto) || callto.length < 3)) {
        alert(
          'Destination number (callto) is not set.\n It can be set in "click2call.html"',
        );
        return;
      }
    }
  ...
  if (
    !isNull(cdiv) &&
    !isNull(cdiv.getAttribute("title")) &&
    cdiv.getAttribute("title").length > 0
  ) {
  ...
  PutToDebugLog(
    2,
    "EVENT, Click2Call called, id: " + nr + "; number: " + destnr,
  );
...
function DisplayStatus(msg_orig) {
  try {
    ...
    if (isNull(loglevel)) {
      loglevel = 1;
      try {
        loglevel = parseInt(webphone_api.parameters["loglevel"], 10);
      } catch (err) {}
    }
    ...
    if (msg.indexOf("Credit:") > 0) {
      msg = Trim(msg.substring(0, msg.indexOf("Credit:")));
    }
    ...
    if (
      msg.toLowerCase().indexOf("registered") >= 0 ||
      msg.toLowerCase().indexOf("authenticated successfully") >= 0
    ) {
      started = 2;
    }
    ...
    for (var i = 0; i < ELIMIT; i++) {
      ...
      if (!isNull(chat_status)) {
        chat_status.innerHTML = msg;
      }
    }
    ...
    PutToDebugLogException(2, "live_demo: DisplayStatus", err);
  }
}
...
function GetAutoaction() {
  try {
    ...
    if (
      !isNull(webphone_api.parameters) &&
      !isNull(webphone_api.parameters["autoaction"]) &&
      !isNaN(webphone_api.parameters["autoaction"])
    ) {
    ...
    if (!isNull(ret) && !isNaN(ret)) {
      return ret;
    }
  } catch (err) {
    PutToDebugLogException(2, "live_demo: GetAutoaction", err);
  }
  return 0;
}
...
webphone_api.onAppStateChange(function (state) {
  try {
    ...
    for (var i = 0; i < ELIMIT; i++) {
      var chtml =
        '<br /><button id="c2k_btn_' +
        i +
        '" class="cl_c2k_btn" onmousedown="Btnc2kMousedown(event, ' +
        i +
        ')" oncontextmenu="Btnc2kContextmenu(event, ' +
        i +
        ')" onmouseup="Btnc2kOnmouseup(event, ' +
        i +
        ')">' +
        '<span id="c2k_status_' +
        i +
        '" class="cl_c2k_status"></span><br>' +
        '<span id="c2k_label_' +
        i +
        '" class="cl_c2k_label">Call</span></button>';

      var c2k_container = document.getElementById("c2k_container_" + i);

      var dispfullchatw = "block";
      if (!isNull(chatwindow_default_state) && chatwindow_default_state > 0) {
        dispfullchatw = "none";
      }
      if (isNull(chatwindow_title)) {
        chatwindow_title = "";
      }
      var chwindow = "";
      var contextmenu = "";
      if (!isNull(c2k_container)) {
        chwindow =
          ...
          '<div id="chat_box_' +
          i +
          '" class="cl_chat_box" style="display: ' +
          dispchat +
          ';">' +
          '<span class="cl_chat_w_title">' +
          chatwindow_title +
          "</span>" +
          '<button class="cl_btn_chat_collapse" onclick="ChatCollapse(' +
          i +
          ')" title="Open/Collapse"><img src="' +
          webphone_api.getbasedir2() +
          'images/icon_collapse.png" width="12" height="8" style="border: 0;" /></button>' +
          '<div id="chat_box_container_' +
          i +
          '" class="cl_chat_box_container" style="display: ' +
          dispfullchatw +
          ';">' +
          '<span id="chat_status_' +
          i +
          '" class="cl_chat_status">&nbsp;</span>' +
        ...
        chwindow =
          chwindow + '<div id="video_container" style="display: none;"></div>';

        contextmenu =
          "" +
          '<div id="optionsmenu_popup_' +
          i +
          '" class="cl_optionsmenu_popup" style="display: none;">' +
          '<ul class="cl_optionsmenu_list">' +
          '<li onclick="OptionsMenuClick(' +
          i +
          ', 1)"><span>Chat</span></li>' +
          '<li onclick="OptionsMenuClick(' +
          i +
      ...
      if (!isNull(c2k_btn)) {
        ...
        if (button_width > 0) {
          c2k_btn.style.width = button_width + "px";
        }
        if (button_height > 0) {
          c2k_btn.style.height = button_height + "px";
        }
        if (button_radius >= 0) {
          c2k_btn.style.borderRadius = button_radius + "px";
        }

        // if it's a floating button
        if (
          (float_button == true || float_button == "true") &&
          !isNull(c2k_container)
        ) {
          ...
          var fset = false;
          if (
            !isNull(float_distance_from_right) &&
            float_distance_from_right >= 0
          ) {
            c2k_container.style.right = float_distance_from_right + "px";
            fset = true;
          }
          if (
            !isNull(float_distance_from_left) &&
            float_distance_from_left >= 0
          ) {
            c2k_container.style.left = float_distance_from_left + "px";
            fset = true;
          }
          if (
            !isNull(float_distance_from_bottom) &&
            float_distance_from_bottom >= 0
          ) {
            c2k_container.style.bottom = float_distance_from_bottom + "px";
            fset = true;
          }
          if (
            !isNull(float_distance_from_top) &&
            float_distance_from_top >= 0
          ) {
    ...
    */
      ...
      if (!isNull(chat_status)) {
        chat_status.style.color = status_text_color;
      }
    }
    ...
    PutToDebugLogException(2, "live_demo: onload", err);
  }

  webphone_api.onChat(function (from, msg, line) {
    if (isNull(from) || isNull(msg)) {
      return;
    }
...
function Btnc2kContextmenu(ev, nr) {
  // handle right click
  ev = ev || window.event;
  ev.preventDefault();
...
function Btnc2kOnmouseup(ev, nr) {
  // handle long click/tap
  ev = ev || window.event;
  ev.preventDefault();
  ...
  if (ev.which == 1) {
    if (GetTickCount() - longclick_initiated > LONG_CLICK_DELAY) {
      document.getElementById("optionsmenu_popup_" + nr).style.display =
        "block"; // open options menu
    } else {
      Click2Call(nr);
    }
  }
...
function Btnc2kMousedown(ev, nr) {
  // handle right click
  ev = ev || window.event;
  ev.preventDefault();
...
function AcceptRejectPopup(from) {
  try {
    ...
    PutToDebugLogException(2, "click2call: AcceptRejectPopup", err);
  }
}
...
function LoadLocalstorageConfig() {
  try {
    if (!isNull(demotext)) {
      // means it's demo version
      var srv = webphone_api.getparameter("serveraddress");
      if (isNull(srv) || srv.length < 1) {
        srv = webphone_api.getparameter("serveraddress_user");
      }
      ...
      if (!isNull(callto) && callto.length > 0) {
        ...
        if (!isNull(calltoF)) {
          calltoF.value = callto;
        }
      }
    }
  } catch (err) {
    PutToDebugLogException(2, "click2call: LoadLocalstorageConfig", err);
  }
}
...
function SaveLocalstorageConfig() {
  try {
    if (!isNull(demotext)) {
      ...
      if (!isNull(srvF) && !isNull(srvF.value)) {
        srv = Trim(srvF.value);
      }
      if (!isNull(usrF) && !isNull(usrF.value)) {
        usr = Trim(usrF.value);
      }
      if (!isNull(pwdF) && !isNull(pwdF.value)) {
        pwd = Trim(pwdF.value);
      }
      if (!isNull(calltoF) && !isNull(calltoF.value)) {
        callto = Trim(calltoF.value);
      }
      ...
      if (usr.length > 0) {
        webphone_api.setparameter("username", usr);
        webphone_api.parameters["username"] = usr;
      }
      if (pwd.length > 0) {
        webphone_api.setparameter("password", pwd);
        webphone_api.parameters["password"] = pwd;
      }
      if (callto.length > 0) {
        webphone_api.setparameter("callto", callto);
        webphone_api.parameters["callto"] = callto;
      }
    }
  } catch (err) {
    PutToDebugLogException(2, "click2call: SaveLocalstorageConfig", err);
  }
}
...
function GetDateForMessage() {
  try {
    ...
    PutToDebugLogException(2, "_message: GetDateForMessage", err);
  }
  return "";
}
      ...
      if (isincall === true || forcehangup === true) {
        ...
        if (button_width < button_minwidth) {
          btn.innerHTML =
            '<img src="' +
            webphone_api.getbasedir2() +
            'images/icon_hangup.png" border="0" />';
        } else {
          if (!isNull(label)) {
            label.innerHTML = hangup_button_text;
          }
        }
      } else {
        ...
        if (button_width < button_minwidth) {
          btn.innerHTML =
            '<img src="' +
            webphone_api.getbasedir2() +
            'images/icon_call.png" border="0" />';
        } else {
          if (!isNull(label)) {
            label.innerHTML = call_button_text;
          }
        }
      }
    }
  } catch (err) {
    PutToDebugLogException(2, "live_demo: SetButtonState", err);
  }
}
...
function isNull(variable) {
  try {
    ...
    PutToDebugLogException(2, "common: isNull", err);
  }
  return true;
}
function Trim(str) {
...
function GetTickCount() {
  try {
    ...
    PutToDebugLogException(2, "common: GetTickCount", err);
  }
  return 0;
}
...
function PutToDebugLog(level, msg) {
  try {
    ...
    if (currMilli < 10) {
      currMilli = "00" + currMilli;
    } else if (currMilli < 100) {
      currMilli = "0" + currMilli;
    }

    var eventtime =
      "[" + currHour + ":" + currMin + ":" + currSec + "." + currMilli + "] ";

    if (msg.indexOf("ERROR") > -1) {
      try {
        console.error("webphone: " + eventtime + msg);
      } catch (e1) {}
    } else {
      try {
        console.log("webphone: " + eventtime + msg);
      } catch (e2) {}
    }
  } catch (e) {}
}
