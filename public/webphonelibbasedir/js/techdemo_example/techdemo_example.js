...
function Start() {
  // function called on "Register" button click to start the webphone
  td_started = true;
...
webphone_api.onAppStateChange(function (state) {
  if (state === "loaded") {
    ...
    if (sipusername.length > 0) {
      sipusername_input.value = sipusername;
    }
    ...
    if (isNull(rcolumn) || isNull(btnadv)) {
      return;
    }
...
function ProcessNotifications(not) {
  try {
    ...
    if (pos > 0) {
      notifywordcontent = Trim(notifywordcontent.substring(0, pos));
    }
...
function AcceptRejectPopup(from) {
  try {
    ...
    PutToDebugLogException(2, "live_demo: AcceptRejectPopup", err);
  }
}
...
function DisplayStatus(msg_orig) {
  try {
    ...
    if (msg.toLowerCase().indexOf("subscribe") >= 0) {
      return;
    }
...
function PopulateSettings() {
  try {
    ...
    if (
      isNull(webphone_api.global.settmap2) ||
      isNull(webphone_api.global.settmap2["magicnumber"])
    ) {
      PutToDebugLog(
        2,
        "live_demo: PopulateSettings, webphone_api.global.settmap is NULL",
      );
      return;
    }
    // callerid, displayname, proxyaddress, webrtcserveraddress
    var callerid_description =
    ...
    var displayname_description =
      "Specify display name. This is an optional setting and it should be your full name";
    var proxyaddress_description =
      "Outbound SIP proxy address. Leave it empty if you don’t have a stateless/outbound proxy";
    var webrtcserveraddress_description =
      "Optional setting to indicate the domain name or IP address of your websocket service used for WebRTC if any.";
    var content =
      "" +
      '<div class="sett_form">' +
      '<label for="username" title="' +
      callerid_description +
      '">Caller ID:</label>' +
      '<input type="text" placeholder="Caller ID" id="username" value="' +
      webphone_api.getparameter("username") +
      '" title="' +
      callerid_description +
      '" autocapitalize="off" />' +
      "</div>" +
      '<div class="sett_form">' +
      '<label for="displayname" title="' +
      displayname_description +
      '">Display name:</label>' +
      '<input type="text" placeholder="Display name" id="displayname" value="' +
      webphone_api.getparameter("displayname") +
      '" title="' +
      displayname_description +
      '" autocapitalize="off" />' +
      "</div>" +
      '<div class="sett_form">' +
      '<label for="proxyaddress" title="' +
      proxyaddress_description +
      '">Proxy address:</label>' +
      '<input type="text" placeholder="Proxy address" id="proxyaddress" value="' +
      webphone_api.getparameter("proxyaddress") +
      '" title="' +
      proxyaddress_description +
      '" autocapitalize="off" />' +
      "</div>" +
      '<div class="sett_form">' +
      '<label for="webrtcserveraddress" title="' +
      webrtcserveraddress_description +
      '">WebRTC Server:</label>' +
      '<input type="text" placeholder="WebRTC Server" id="webrtcserveraddress" value="' +
      webphone_api.getparameter("webrtcserveraddress") +
      '" title="' +
      webrtcserveraddress_description +
      '" autocapitalize="off" />' +
    ...
    if (!isNull(settings_list)) {
      ...
      PutToDebugLog(
        5,
        "EVENT, live_demo PopulateSettings list element is NULL",
      );
    }
  } catch (err) {
    PutToDebugLogException(2, "live_demo: PopulateSettings", err);
  }
}
...
function SaveSettings() {
  try {
    ...
    if (
      (isNull(sipusername) || Trim(sipusername).length < 1) &&
      !isNull(username_input)
    ) {
    ...
    PutToDebugLogException(2, "live_demo: SaveSettings", err);
  }
}
...
function GetBrowser() {
  try {
    if (isNull(browserName) || isNull(browserName2)) {
      ...
      PutToDebugLog(
        2,
        "EVENT, common: GetBrowser Detected browser name: " +
          browserName +
          ";  " +
          browserName2,
      );
    ...
    PutToDebugLogException(2, "common: GetBrowser", err);
  }
