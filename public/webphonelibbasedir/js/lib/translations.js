...
webphone_api.translations = (function () {
  ...
  function TranslateInnerHun(str) {
    try {
      ...
      if (str === "Enter phone number of SIP URI")
        return "Telefonszám vagy SIP URI";
      if (str === "Close") return "Bezár";
      if (str === "Authenticated") return "Hitelesítve";
      if (str === "Add people to the conference")
        return "Adjon hozzá személyeket a konferenciához";
      if (str === "Activated") return "Aktiválva";
      if (
        str ===
        "Save my settings on this computer locally (including password!)"
      )
      ...
      if (
        ...
        return "Mentse le beállitásaim ezen a számitógépen (jelszót leleértve)";
  ...
  function TranslateInnerSpanish(str) {
    try {
      ...
      if (str === "Cannot open mic (audio in) device")
        return "No puede abrir el micrófono (entrada de audio)";
      ...
      if (str === "Proxy Authentication Required")
        return "Necesaria autenticación en el proxy";
      ...
      if (str === "Automatic reconfiguration")
        return "Reconfiguración automática";
      if (str === "init encryption. please wait...")
        return "iniciando encriptación. por favor espere...";
      ...
      if (str === "Register expired / Manual reregister needed")
        return "Registro expiró / Necesita re-registro manual";
      if (str === "No network") return "Sin red";
      if (str === "No network / No network connection")
        return "Sin red / Sin conexión de red";
      if (str === "Register failed") return "Registro fallido";
      if (str === "Unknown / Currently registered")
        return "Desconocido / Actualmente registrado";
      if (str === "Connection lost") return "perdida la Conexión";
      if (str === "Server is offline, your network is offline")
        return "Servidor fuera de línea, su red esta desconectada";
      if (str === "Wrong username/password, rejected by the server")
        return "Nombre de usuario / contraseña incorrectos, el servidor ha rechazado la conexión";
      //noinspection JSConstructorReturnsPrimitive
      if (str === "Forbidden by the server -probably wrong username/password")
        return "Conexión prohibida por el servidor - probablemente el nombre de usuario / contraseña sea erróneo";
      ...
      if (str === "No more voicemail message")
        return "No más mensajes de buzón de voz";
      ...
      if (str === "No phone instance found")
        return "Ningún teléfono encontrado en la instancia";
      if (str === "disconnected from server")
      ...
      if (str === "transfered initiated successfully")
        return "transferencia iniciada con éxito";
      if (str === "wrong username/password")
  ...
  function TranslateInnerRus(str) {
    try {
      ...
      if (str === "Proxy Authentication Required")
        return "Необходима авторизация для прокси";
  ...
  function TranslateInnerItalian(str) {
    try {
      ...
      if (str === "Proxy Authentication Required")
        return "E' necessario autentificarsi al proxy";
  ...
  function TranslateInnerPr(str) {
    try {
      ...
      if (str === "Proxy Authentication Required")
        return "Requer autenticação no Proxy";
  ...
  function TranslateInnerFr(str) {
    try {
      ...
      if (
        ...
        return "Sauver les données localement(mot de passe inclu";
      ...
      if (str === "Call Is Being Forwarded")
        return "L'appel est en cours de transfert";
      ...
      if (str === "Unsupported Media Type")
        return "Type de média non supportée";
      ...
      //noinspection JSConstructorReturnsPrimitive
      if (str === "Response Cannot Be Sent Safely")
        return "La réponse ne peut etre envoyée de maniere securisée";
      if (str === "Requires congestion management")
        return "Necessité d'un systeme de management de congestion";
      if (str === "Would induce fragmentation")
        return "Devrait introduire de la fragmentation";
      ...
      if (str === "Your authentication username")
        return "Nom utilisateur de connexion";
      ...
      if (str === "cannot save config")
        return "impossible de sauver la configuration";
      if (str === "Callback") return "Rappeler";
      ...
      if (str === "Too many wrong requests")
        return "Trop de requete avec erreur";
  ...
  function TranslateInner(lang, str) {
    switch (lang) {
      case "ru":
        return TranslateInnerRus(str);
      case "hu":
        return TranslateInnerHun(str);
      case "ro":
        return TranslateInnerRo(str);
      case "de":
        return TranslateInnerDe(str);
      case "it":
        return TranslateInnerItalian(str);
      case "es":
        return TranslateInnerSpanish(str);
      case "tr":
        return TranslateInnerTr(str);
      case "pr":
        return TranslateInnerPr(str);
      case "pt":
        return TranslateInnerPr(str);
      case "br":
        return TranslateInnerPr(str);
      case "jp":
        return TranslateInnerJp(str);
      case "ja":
        return TranslateInnerJp(str);
      case "fr":
        return TranslateInnerFr(str);
  ...
  function Translate(str) {
    ...
    if (ret.length < 1 && webphone_api.stringres.getlangshort() > 0)
      ret = TranslateInner(webphone_api.stringres.getlangshort(), str);
    if (ret.length < 1) {
      // && str[str.length - 1] == '.'
      var str2 = TrimEx(str);
      if (str2 !== str) {
        ...
        if (ret.length < 1 && webphone_api.stringres.getlangshort() > 0)
          ret = TranslateInner(webphone_api.stringres.getlangshort(), str2);
        if (
          ret.length > 0 &&
          str.length > 3 &&
          str[str.length - 1] == "." &&
      ...
      if (ret.length < 1) {
        if (str.indexOf("Registered. Credit") == 0)
          return (
        ...
        if (str.indexOf("Registered") >= 0)
          return str.replace("Registered", Translate("Registered"));
        if (str.indexOf("Call duration") >= 0)
          return str.replace("Call duration", Translate("Call duration"));
      }
    }
