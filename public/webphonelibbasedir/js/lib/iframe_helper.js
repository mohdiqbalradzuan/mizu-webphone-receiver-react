...
webphone_api.start = function () {
  SendMessageToFrame("webphone_api.start(webphone_api.parameters)");
};
webphone_api.stop = function () {
  SendMessageToFrame("webphone_api.stop()");
};
webphone_api.register = function () {
  SendMessageToFrame("webphone_api.register(webphone_api.parameters)");
};
...
webphone_api.hangup = function (isinternal) {
  if (isNull(isinternal)) SendMessageToFrame("webphone_api.hangup()");
  else
    SendMessageToFrame('webphone_api.hangup("' + isinternal.toString() + '")');
};
webphone_api.accept = function () {
  SendMessageToFrame("webphone_api.accept()");
};
webphone_api.reject = function () {
  SendMessageToFrame("webphone_api.reject()");
};
webphone_api.ignore = function () {
  SendMessageToFrame("webphone_api.ignore()");
};
...
webphone_api.getavailablecallfunc = function () {
  ...
  PutToDebugLog(
    2,
    logtag + "ERROR, getavailablecallfunc webphoneframe is NULL",
  );
};
...
webphone_api.forward = function (number) {
  SendMessageToFrame('webphone_api.forward("' + number.toString() + '")');
};
webphone_api.conference = function (number, add) {
  SendMessageToFrame(
    'webphone_api.conference("' + number.toString() + "," + add + '")',
  );
};
webphone_api.transfer = function (number) {
  SendMessageToFrame('webphone_api.transfer("' + number.toString() + '")');
};
webphone_api.dtmf = function (message) {
  SendMessageToFrame('webphone_api.dtmf("' + message.toString() + '")');
};
webphone_api.mute = function (mute, direction) {
  SendMessageToFrame("webphone_api.mute(" + mute + ", " + direction + ")");
};
...
webphone_api.hold = function (state) {
  SendMessageToFrame("webphone_api.hold(" + state + ")");
};
...
webphone_api.sendchat = function (number, msg) {
  SendMessageToFrame(
...
webphone_api.sendsms = function (number, msg, from) {
  SendMessageToFrame(
    'webphone_api.sendsms("' +
      number.toString() +
      '","' +
      msg.toString() +
      '","' +
...
webphone_api.voicerecord = function (bool, url, line) {
  SendMessageToFrame(
    'webphone_api.voicerecord("' + bool + '","' + url + '","' + line + '")',
  );
};
webphone_api.audiodevice = function () {
  SendMessageToFrame("webphone_api.audiodevice()");
};
webphone_api.devicepopup = function () {
  SendMessageToFrame("webphone_api.devicepopup()");
};
...
webphone_api.setaudiodevice = function (dev, devicename, immediate) {
  SendMessageToFrame(
    'webphone_api.setaudiodevice("' +
      dev +
      '", "' +
      devicename +
      '", "' +
...
webphone_api.setdevice = function (dev, devicename, immediate) {
  SendMessageToFrame(
    'webphone_api.setdevice("' +
      dev +
      '", "' +
      devicename +
      '", "' +
...
webphone_api.setvolume = function (dev, volume) {
  SendMessageToFrame('webphone_api.setvolume("' + dev + '", "' + volume + '")');
};
...
webphone_api.setparameter = function (param, value) {
  SendMessageToFrame(
...
webphone_api.setline = function (line, keeplastreal) {
  SendMessageToFrame(
    'webphone_api.setline("' + line + '", "' + keeplastreal + '")',
  );
};
...
webphone_api.unregister = function () {
  SendMessageToFrame("webphone_api.unregister()");
};
webphone_api.reregister = function () {
  SendMessageToFrame("webphone_api.reregister()");
};
webphone_api.checkpresence = function (userlist) {
  SendMessageToFrame('webphone_api.checkpresence("' + userlist + '")');
};
webphone_api.setpresencestatus = function (statustring) {
  SendMessageToFrame('webphone_api.setpresencestatus("' + statustring + '")');
};
...
webphone_api.setsipheader = function (header) {
  SendMessageToFrame('webphone_api.setsipheader("' + header + '")');
};
...
webphone_api.getsipmessage = function (dir, type, callback) {
  ...
  SendMessageToFrame(
    "webphone_api.getsipmessage#" + dir + "," + type + "," + callback,
  );
};
...
webphone_api.delsettings = function (level) {
  SendMessageToFrame("webphone_api.delsettings(" + level + ")");
};
...
function SendMessageToFrame(msg) {
  try {
    if (isNull(msg) || msg.length < 1) {
      PutToDebugLog(
        2,
        logtag + "ERROR, SendMessageToFrame invalid message: " + msg,
      );
      return;
    }
    ...
    if (isNull(wpiframe)) {
      PutToDebugLog(
        2,
        logtag + "ERROR, SendMessageToFrame [webphoneframe] not found",
      );
      return;
    }

    if (isNull(webphoneframe)) {
      webphoneframe = wpiframe.contentWindow;
    }
    if (isNull(webphoneframe)) {
      PutToDebugLog(
        2,
        logtag + "ERROR, SendMessageToFrame webphoneframe is NULL",
      );
      return;
    }
    ...
    if (
      typeof mesage_queue_cache === "undefined" ||
      mesage_queue_cache === null
    ) {
      mesage_queue_cache = [];
    }
    ...
    if (
      webphoneframe &&
      wp_frame_is_loaded_and_ready === false &&
      msg === "initialize_connection"
    ) {
      webphoneframe.postMessage(msg, allowed_domain_i); //send the message and target URI
    } else if (webphoneframe && wp_frame_is_loaded_and_ready === true) {
      // && webphoneframe.webphone_api && webphoneframe.webphone_api.HandleEventMessage)
      webphoneframe.postMessage(msg, allowed_domain_i); //send the message and target URI
    } else {
      PutToDebugLog(
        2,
        logtag +
          "EVENT, SendMessageToFrame webphone not loaded yet, queue message: " +
          msg,
      );
      ...
      wpavailability_timer = setInterval(function () {
        try {
          ...
          if (webphoneframe && wp_frame_is_loaded_and_ready === true) {
            ...
            if (mesage_queue_cache.length > 0) {
              for (var i = 0; i < mesage_queue_cache.length; i++) {
                if (
                  typeof mesage_queue_cache[i] === "undefined" ||
                  mesage_queue_cache[i] === null ||
                  mesage_queue_cache[i].length < 1
                ) {
                  continue;
                }
                ...
                PutToDebugLog(
                  2,
                  logtag +
                    "EVENT, SendMessageToFrame handle queued message: " +
                    mesage_queue_cache[i],
                );
                webphoneframe.postMessage(
                  mesage_queue_cache[i],
                  allowed_domain_i,
          ...
          PutToDebugLogException(
            2,
            logtag + "SendMessageToFrame wpavailability_timer",
            errin,
          );
          StopQueueTimer();
        }
    ...
    PutToDebugLogException(2, logtag + "SendMessageToFrame", err);
  }
}

function StopQueueTimer() {
  if (
    typeof wpavailability_timer !== "undefined" &&
...
function StartInitCommTimer() {
  // after page load, run this timer until we have access to iFrame
  StopInitCommTimer();

  wp_comminint_timer = setInterval(function () {
    var wpfrm = document.getElementById("webphoneframe");
    if (
      typeof wpfrm !== "undefined" &&
      wpfrm !== null &&
      typeof wpfrm.contentWindow !== "undefined" &&
      wpfrm.contentWindow !== null
    ) {
      StopInitCommTimer();
      InitializeHandler();
      return;
...
function StopInitCommTimer() {
  if (
    typeof wp_comminint_timer !== "undefined" &&
    wp_comminint_timer !== null
  ) {
    clearInterval(wp_comminint_timer);
  }
  wp_comminint_timer = null;
}
...
function InitializeHandler() {
  try {
    if (flag_inithandlercalled === true) {
      return;
    }
    var wpfrm = document.getElementById("webphoneframe");
    if (
    ...
    PutToDebugLog(
      2,
      logtag + "EVENT, InitializeHandler, eventlistener attached to window",
    );
      ...
      try {
        try {
          ...
          if (
            ...
            event.data !== null &&
            event.data.toString().indexOf("_IFRAME") >= 0
          ) {
            if (event.data.toString().indexOf("wploadedandready_IFRAME") >= 0) {
              PutToDebugLog(
                2,
                logtag +
                  "EVENT, onMessage: received message: webphone is loaded and ready",
              );
          ...
          PutToDebugLogException(
            2,
            logtag + "window.onmessage " + event.data,
            err,
          );
        } //ERROR,iframe_webphone: window.onmessage ReferenceError: wploadedandready_IFRAME is not defined
      } catch (err2) {
        PutToDebugLogException(2, logtag + "window.onmessage ex", err2);
      }
    }
    ...
    PutToDebugLogException(2, logtag + "InitializeHandler", err);
  }
}
...
function isNull(variable) {
  try {
    ...
    PutToDebugLogException(2, logtag + "isNull", err);
  }
  return true;
}
function Trim(str) {
  try {
    ...
    PutToDebugLogException(2, logtag + "Trim", err);
  }
  return str;
}
function PutToDebugLogException(level, msg, err) {
  PutToDebugLog(level, "ERROR," + msg + " " + err);
}
